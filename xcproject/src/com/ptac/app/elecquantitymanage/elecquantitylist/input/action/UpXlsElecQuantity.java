package com.ptac.app.elecquantitymanage.elecquantitylist.input.action;
...
import com.ptac.app.elecquantitymanage.elecquantitylist.input.dao.InputElecQuantityImp;
...
public class UpXlsElecQuantity extends HttpServlet {
	private static final long serialVersionUID = 1L;

	public UpXlsElecQuantity() {
	}
	 ...
	 * @param response
	 * @throws IOException
	 * @update WangYiXiao 2014-4-24 excel列数32改为26
	 */
	public void readXls(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			if (gs) {
				...
				if ((content.size() - 1) <= 300) { // 条数判断
					...
					Vector wrong = new InputElecQuantityImp().input(content,
					...
					if (!wrong.isEmpty()) {
						...
						wr.Write(wrong, account.getAccountName()
								+ "电量批量导入不成功的数据.xls", "电量批量导入不成功的数据",
								"电量批量导入不成功数据", 26, dir2);//32
					} else {
		...
		session.setAttribute("url", path 
				+ "/web/dianliang/input.jsp");
		session.setAttribute("wfile", path + "/wrongdata/" 
				+ account.getAccountName() + "电量批量导入不成功的数据.xls");
