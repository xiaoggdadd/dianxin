...
import com.ptac.app.elecquantitymanage.elecquantitylist.tool.CheckInputAvailable;

public class InputElecQuantityImp implements InputElecQuantity {
	...
	public Vector input(Vector content, CountForm cform, Account account,
		...
		for (int temp = 1; temp < rows.length; temp++) {
			...
			wcell = new CheckInputAvailable().inputCheck(cells, account);
			if (!wcell.isEmpty()) {
