...
public class CountryHeadDaoImp implements CountryHeadDao{
	 ...
	 * update WangYiXiao 2014-8-8 不能查询出加强版的数据
	 * update WangYiXiao 2014-8-11 去掉tbtz != 1
	 */
...
//				bean.setBzydl(String.valueOf(bzydl));//报账用电量
	...
	public CountryHeadBean xiangxi(String whereStr) {
		...
		sql.append("SELECT E.UNITPRICE,E.BEILV,A.MEMO AMEMO,E.MEMO EMEMO, " 
				+"A.FLOATDEGREE,A.BLHDL,E.FLOATPAY, E.ACTUALPAY, A.ACTUALDEGREE, E.YDDF," 
				+"(SELECT IND.NAME FROM INDEXS IND WHERE IND.CODE=E.LINELOSSTYPE AND IND.TYPE='xslx') LINELOSSTYPE," 
				+"E.LINELOSSVALUE,E.CHANGEVALUE,A.LASTDEGREE,A.THISDEGREE,E.ACTUALLINELOSSVALUE"
		...
		try {
			...
			if(rs.next()){
				...
				bean.setActualdegree(rs.getString("ACTUALDEGREE") != null ? rs.getString("ACTUALDEGREE"): "0");//实际用电量
	...
	public String save(CountryHeadBean bean,Share share) {
    	...
    	String sql1 = "update AMMETERDEGREES SET ACTUALDEGREE="+bean.getDbydl()+", FLOATDEGREE="+bean.getFloatdegree()+", MEMO='"+bean.getAmmemo()+"', BLHDL='"+bean.getBlhdl()+"', DEDHDL='"+bean.getDedhdl()+"', CSDB='"+bean.getCsdb()+"',"
