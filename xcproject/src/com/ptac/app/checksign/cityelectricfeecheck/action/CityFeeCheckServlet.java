...
import com.ptac.app.checksign.cityelectricfeecheck.dao.CityFeeCheckDao;
import com.ptac.app.checksign.cityelectricfeecheck.dao.CityFeeCheckDaoImpl;
 ...
 */
public class CityFeeCheckServlet extends HttpServlet {
	 ...
	 * @throws IOException
	 * @update WangYiXiao 2014-8-12 供电方式过滤条件 zd.gdfs
	 * @update WangYiXiao 2014-8-13 过滤条件 录入标志 getlrbz
	 */
	public void queryexport(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		//------dao------
		CityFeeCheckDao dao = new CityFeeCheckDaoImpl();
		//------获取登录账户信息------
		...
		if("chaxun".equals(command)){//传到查询页面 
			request.getRequestDispatcher("/web/appJSP/checksign/cityelectricfeecheck/cityfeecheck.jsp").forward(request, response);
		}else if("daochu".equals(command)){//传到导出页面
			request.getRequestDispatcher("/web/appJSP/checksign/cityelectricfeecheck/人工电费二级审核.jsp").forward(request, response);
		}
	}
	/**
	 * @param request
	...
	public void check(HttpServletRequest request, HttpServletResponse response)throws ServletException, IOException {
		...
		if("checkcity".equals(command)){//通过
	        ...
	        url = path + "/web/appJSP/checksign/cityelectricfeecheck/cityfeecheck.jsp" ;
	      	String cityaudit = "1";//审核通过
	        msg = dao.CheckCityFees(personnal,cityaudit,chooseIdStr1,chooseIdStr2,"1");
	        ...
	        msg = dao.CheckCityFees(personnal,cityaudit,chooseIdStr1,chooseIdStr2,"0");
	        log.write(msg, account.getName(), request.getRemoteAddr(), "市级电费审核");
	        String m="";
	        if(msg=="审核电费信息失败！"){
	        	m="0";
