package com.ptac.app.sitemanage.stationmanage.action;
...
import com.ptac.app.sitemanage.stationmanage.dao.InputStationDaoImpl;


/**
 * 站点导入 xls 临时文件 生成
 * 
 * @author WangYiXiao 2014-9-19
 * 
 */
@SuppressWarnings("deprecation")
public class StationInputServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;

	public StationInputServlet() {
	}
	...
	public void readXls(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			if (gs) {
				msg = "站点导入模板格式不正确，请在系统中下载模板！";
			} else {
				// 生成 读取文件的工具类 进行使用
				ReadFile temp = new ReadFileFactory().getReadFile(filename);
				// 开始读取文件
				Vector content = temp.getContentzd(filename);
				...
				if ((content.size() - 1) <= 300) { // 条数判断
					...
					Vector wrong = new InputStationDaoImpl().input(content,
		...
		session.setAttribute("url", path
				+ "/web/jizhan/sitemanage.jsp");
		session.setAttribute("wfile", path + "/wrongdata/"
				+ account.getAccountName() + "站点导入不成功的数据.xls");
