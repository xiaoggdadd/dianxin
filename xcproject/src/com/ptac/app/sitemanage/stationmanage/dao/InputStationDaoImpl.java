...
import com.ptac.app.sitemanage.stationmanage.util.CheckStationInputAvailable;

public class InputStationDaoImpl implements InputStationDao {
	...
	public Vector input(Vector content, CountForm cform, Account account,
		...
		for (int temp = 1; temp < rows.length; temp++) {
			...
			wcell = new CheckStationInputAvailable().inputCheck(cells, account);
			if (!wcell.isEmpty()) {
