package com.ptac.app.statisticcollect.city.electricanalyse.action;
...
import com.ptac.app.statisticcollect.city.electricanalyse.dao.ElectricAnalyseDao;
import com.ptac.app.statisticcollect.city.electricanalyse.dao.ElectricAnalyseDaoImpl;
import com.ptac.app.statisticcollect.util.YearYueUtil;

public class ElectricAnalyseServlet extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		ElectricAnalyseDao dao = new ElectricAnalyseDaoImpl();
		List<ElectricAnalyseBean> list = new ArrayList<ElectricAnalyseBean>();
		list = dao.queryDetails(yearyue, property, status, zzs, shicode, loginid);
		int num = list.size();
		...
		if("chaxun".equals(action)){
			request.getRequestDispatcher("/web/appJSP/statisticscollect/city/electricanalyse/electricanalyse.jsp").forward(request, response);
		}else{
			request.getRequestDispatcher("/web/appJSP/statisticscollect/city/electricanalyse/electricanalyseExport.jsp").forward(request, response);
		}
	}
