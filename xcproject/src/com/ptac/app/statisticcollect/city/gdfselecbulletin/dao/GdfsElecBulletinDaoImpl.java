...
public class GdfsElecBulletinDaoImpl implements GdfsElecBulletinDao{
	...
	public List<GdfsElecBulletinBean> queryDetails(String yearyue,
			String property, String status, String shicode, String loginid) {
		...
		sql.append("SELECT RNDIQU(A.SHI) SHI, RNDIQU(A.XIAN) XIAN, C.ZDL, C.ZHIGD, C.ZHIZB, C.ZHUANGD, C.ZHUANZB FROM (SELECT FAGCODE SHI, AGCODE XIAN FROM D_AREA_GRADE WHERE FAGCODE = '").append(shicode).append("' AND AGCODE IN (SELECT T.AGCODE FROM PER_AREA T WHERE T.ACCOUNTID = '").append(loginid).append("') ORDER BY AGCODE) A,"
        +" (SELECT B.XIAN XIAN, B.ZDL, B.ZHIGD, ROUND(B.ZHIGD / B.ZDL * 100, 2) ZHIZB, B.ZDL - B.ZHIGD ZHUANGD,ROUND((B.ZDL - B.ZHIGD) / B.ZDL * 100, 2) ZHUANZB"
        +" FROM (SELECT ROUND(SUM(").append(dl).append(") / 10000,2) ZDL,ROUND(SUM(DECODE(MH.GDFS,'gdfs06',").append(dl).append(",'gdfs07',").append(dl).append(",0)) / 10000,2) ZHIGD, MH.XIAN FROM MQY_ZH MH WHERE TO_CHAR(MH.ACCOUNTMONTH, 'YYYY-MM') = '").append(yearyue).append("'")
        .append("0".equals(property)?"":(" AND MH.ZPROPERTY = '"+property+"'")).append(" AND MH.XIAN IN (SELECT T.AGCODE FROM PER_AREA T WHERE T.ACCOUNTID = '").append(loginid).append("') GROUP BY MH.XIAN) B) C  WHERE A.XIAN = C.XIAN(+) ORDER BY A.XIAN");
		logger.info("市级供电方式电量通报查询："+sql.toString());
		...
		try {
			...
			e.printStackTrace();
			logger.error("市级供电方式电量通报查询出错："+e.getMessage());
		}finally{
			db.free(rs, ps, conn);
		}
		return list;
