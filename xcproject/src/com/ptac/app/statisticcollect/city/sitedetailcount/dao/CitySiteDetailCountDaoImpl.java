 ...
 * @see 报账单审核统计dao层实现类
 */
public class CitySiteDetailCountDaoImpl implements CitySiteDetailCountDao {

	/**
	 * @author lijing
	 * @see 报账单审核统计通过信息查询
	 * @param whereStr String 过滤条件
	 * @return list 返回一个CitySiteDetailCountBean的list
	 */
	@Override
	public List<CitySiteDetailCountBean> quarySiteDetailCount(
			StringBuffer whereStr, StringBuffer str, String loginId) {

		Connection conn = null;
		PreparedStatement ps = null;
		ResultSet rs = null;
		List<CitySiteDetailCountBean> list = new ArrayList<CitySiteDetailCountBean>();
		...
		sql.append("SELECT COUNT(ZD.ID) ZHAN,B.DIANBIAO,C.JIAOFEI,(SELECT D.AGNAME FROM D_AREA_GRADE D WHERE D.AGCODE=B.SHI) AS SHI,C.ZIDONG,C.RENGONG,C.SHIJI,C.CAIWU,C.LIUCHENGHAO,B.SHI,C.QXZR,C.SZR,C.RG,C.diszdid "
				+" FROM ZHANDIAN ZD,(SELECT Z.SHI,COUNT(*) DIANBIAO FROM ZHANDIAN Z,DIANBIAO D WHERE Z.ID=D.ZDID AND Z.SHSIGN='1' AND Z.QYZT='1' AND D.DBYT='dbyt01' AND D.DBQYZT='1' GROUP BY SHI)B,"
		...
		System.out.println("站点明细统计审核通过:"+sql);
		...
		try {
			...
			while(rs.next()){
				...
				CitySiteDetailCountBean bean=new CitySiteDetailCountBean();
...
//	            }
	 ...
	 * @see 报账单审核统计不通过信息查询
	 * @param whereStr String 过滤条件
	 * @return list 返回一个CitySiteDetailCountBean1的list
	 */
	@Override
	public List<CitySiteDetailCountBean1> quarySiteDetailCount1(
			StringBuffer whereStr, StringBuffer str, String loginId) {

		Connection conn = null;
		PreparedStatement ps = null;
		ResultSet rs = null;
		List<CitySiteDetailCountBean1> list1 = new ArrayList<CitySiteDetailCountBean1>();
		...
		System.out.println("站点明细统计审核不通过:"+sql);
		DataBase db = new DataBase();
		try {
			db.connectDb();
			conn = db.getConnection();
			...
			while(rs.next()){
				...
				CitySiteDetailCountBean1 bean1 = new CitySiteDetailCountBean1();
...
//					bean1.setDyl1(dyl);
	...
	/**
	 * @author lijing
	 * @see 报账单审核统计详细查询（审核通过）
	 * @param str String 过滤条件
	 * @return list 返回一个Info的list
	 */
	@Override
	public List<Info> info(String loginId,StringBuffer str1) {
		...
		StringBuffer sql = new StringBuffer();
		sql.append("SELECT A.ELECTRICFEE_ID,Z.JZNAME,A.BLHDL,A.ACTUALPAY"
				+" FROM ZHANDIAN Z,DIANBIAO D,dianfeidandayin A"
				+" WHERE Z.ID = D.ZDID AND D.DBID = A.AMMETERID_FK"
				+" AND Z.SHSIGN = '1' AND Z.QYZT = '1' AND D.DBYT = 'dbyt01'"
				+str1
				+" and Z.xiaoqu in(select t.agcode from per_area t where t.accountid = '"+loginId+"')");
		
		System.out.println("报账单审核统计详细查询:"+sql);
		DataBase db = new DataBase();
		...
		try {
			...
			while(rs.next()){
				...
				bean.setZdname(rs.getString(2) != null ? rs.getString(2):"");
				bean.setBlhdl(rs.getString(3) != null ? rs.getString(3):"");
				bean.setActualpay(rs.getString(4) != null ? rs.getString(4):"");
				bean.setShr("");
				bean.setShtime("");
				bean.setPassly("");
				list.add(bean);
	 ...
	 */
	@Override
	public List<Info> info1(String loginId,StringBuffer str,StringBuffer str1) {
		...
		sql.append("SELECT A.ELECTRICFEE_ID,Z.JZNAME,A.BLHDL,A.ACTUALPAY,"+str
				+" FROM ZHANDIAN Z,DIANBIAO D,dianfeidandayin A"
				+" WHERE Z.ID = D.ZDID AND D.DBID = A.AMMETERID_FK"
				+" AND Z.SHSIGN = '1' AND Z.QYZT = '1' AND D.DBYT = 'dbyt01' "
				+str1
		...
		try {
			...
			while(rs.next()){
				...
				bean.setBlhdl(rs.getString(3) != null ? rs.getString(3):"");
				bean.setActualpay(rs.getString(4) != null ? rs.getString(4):"");
				bean.setShr(rs.getString(5) != null ? rs.getString(5):"");
				bean.setShtime(rs.getString(6) != null ? rs.getString(6):"");
				bean.setPassly(rs.getString(7) != null ? rs.getString(7):"");
				list.add(bean);
			}
		} catch (Exception e) {
			// TODO Auto-generated catch block
	...
	/**
	 * @author lijing
	 * @param str1 str String 查询过滤条件
	 * @param loginId 
	 * @return List 报账单审核统计详细查询(市级、人工审核：不通过，未审核)
	 */
	@Override
	public List<Info> info2(String loginId, StringBuffer str, StringBuffer str1) {
		...
		try {
			...
			while(rs.next()){
				...
				bean.setShr(rs.getString(5) != null ? rs.getString(5):"");
				bean.setShtime(rs.getString(6) != null ? rs.getString(6):"");
				String shzt = rs.getString(7) != null ? rs.getString(7):"";
				if("0".equals(shzt)){
					bean.setShzt("未审核");
	 ...
	 */
	@Override
	public List<Info> info3(String loginId, StringBuffer str, StringBuffer str1) {
		...
		try {
			...
			while(rs.next()){
				...
				bean.setShr("");
				bean.setShtime("");
				String shzt = rs.getString(5) != null ? rs.getString(5):"";
				if("0".equals(shzt)){
					bean.setShzt("未审核");
				}
				list.add(bean);
	 ...
	 */
	@Override
	public List<Info> info4(String loginId, StringBuffer str, StringBuffer str1) {
	 ...
	 */
	@Override
	public List<Info> info5(String loginId, StringBuffer str, StringBuffer str1) {
		...
		try {
			...
			while(rs.next()){
				...
				bean.setShtime(rs.getString(6) != null ? rs.getString(6):"");
				String shzt = rs.getString(7) != null ? rs.getString(7):"";
				if("0".equals(shzt)||"1".equals(shzt)||"-2".equals(shzt)){
					bean.setShzt("未审核");
	 ...
	 */
	@Override
	public List<Info> info11(String loginId, StringBuffer str, StringBuffer str1) {
