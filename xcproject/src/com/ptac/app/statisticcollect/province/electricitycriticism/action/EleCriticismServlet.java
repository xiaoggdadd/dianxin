package com.ptac.app.statisticcollect.province.electricitycriticism.action;
...
import com.ptac.app.statisticcollect.province.electricitycriticism.dao.EleCriticismDao;
import com.ptac.app.statisticcollect.province.electricitycriticism.dao.EleCriticismDaoImpl;
/**
 * 
...
public class EleCriticismServlet extends HttpServlet {
	...
	throws ServletException, IOException{
		...
		if("elec".equals(action)){
			...
			EleCriticismDao dao = new EleCriticismDaoImpl();
			List<EleCriticismBean> list = dao.checkPrice(wherestr, cbbly, cbble);
			int num = list.size();
			String[] sum = dao.getSum(list);
			EleCriticismBean sumbean = new EleCriticismBean();
			...
			if("chaxun".equals(command)){
				request.getRequestDispatcher("/web/appJSP/statisticscollect/province/electricitycriticism/electricitycriticism.jsp").forward(request, response);
			}else{
				request.getRequestDispatcher("/web/appJSP/statisticscollect/province/electricitycriticism/electricityexport.jsp").forward(request, response);
			}
		}
	}
