...
public class UnitAveShengDaoImp implements UnitAveShengDao {
	...
	public List<UnitAveShengBean> queryElectric(String whereStr, String loginId, String shi, String bzyfstart, String bzyfend) {
		List<UnitAveShengBean> list = new ArrayList<UnitAveShengBean>();//声明结果集
		...
		try {
			...
			System.out.println("省级单价平均与平均单价:" + sql);//后台打印sql
			...
			while (rs.next()) {
				UnitAveShengBean bean = new UnitAveShengBean();//库里每有一行数据便生成一个bean
				//------接收查询到的数据(并判断是否为空)------
				String shiCh = rs.getString("SHI") != null ? rs.getString("SHI") : "";	//市
