package com.ptac.app.statisticcollect.province.unitAveSheng.action;
...
import com.ptac.app.statisticcollect.province.unitAveSheng.dao.UnitAveShengDao;
import com.ptac.app.statisticcollect.province.unitAveSheng.dao.UnitAveShengDaoImp;

/**
 * @see 省级单价平均与平均单价.Servlet
 * @author ZengJin 2014-3-30 
 */
public class UnitAveShengServlet extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if (bzyfstart != null && !bzyfstart.equals("") && !bzyfstart.equals("0") 
		 && bzyfend != null && !bzyfend.equals("") && !bzyfend.equals("0")) {
			...
			// ------获得结果集和结果条数------
			UnitAveShengDao dao = new UnitAveShengDaoImp();
			...
			if ("chaxun".equals(command)) {// 传到查询页面
				request.getRequestDispatcher(
						"/web/appJSP/statisticscollect/province/unitAveSheng/unitAveShengQuery.jsp")
						.forward(request, response);
			} else if ("daochu".equals(command)) {// 传到导出页面
				request.getRequestDispatcher(
						"/web/appJSP/statisticscollect/province/unitAveSheng/unitAveShengExport.jsp")
						.forward(request, response);
			}
		}
	}
