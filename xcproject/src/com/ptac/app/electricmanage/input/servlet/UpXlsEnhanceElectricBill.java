...
import com.ptac.app.electricmanage.input.dao.InputEnhanceElectricBillImp;
...
public class UpXlsEnhanceElectricBill extends HttpServlet {
	private static final long serialVersionUID = 1L;

	public UpXlsEnhanceElectricBill() {
	}
	...
	public void readXls(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			if (gs) {
				...
				if ((content.size() - 1) <= 300) { // 条数判断
					...
					Vector wrong = new InputEnhanceElectricBillImp().input(content,
		...
		session.setAttribute("url", path
				+ "/web/appJSP/electricmanage/enhanceelectricitybill/EnhanceInputEleBill.jsp");
