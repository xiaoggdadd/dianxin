...
import com.ptac.app.lease.audit.dao.LeaseFeesCountryAuditDao;
import com.ptac.app.lease.audit.dao.LeaseFeesCountryAuditDaoImpl;

/** 租赁费用区县审核
 * @author WangYiXiao 2014-9-24
 *
 */
public class LeaseFeesCountryAuditServlet extends HttpServlet {
	...
	public void doChaXun(HttpServletRequest request, HttpServletResponse response)
		...
		LeaseFeesCountryAuditDao dao = new LeaseFeesCountryAuditDaoImpl();
		...
		String countryauditstatus = request.getParameter("shenhestatus");//区县审核状态
		String whereStr = "";
		if(shi != null){
		...
		if(countryauditstatus !=null &&! "-1".equals(countryauditstatus)){
			whereStr = whereStr + " AND LF.COUNTRYAUDITSTATUS ='" + countryauditstatus + "'";
		}
		List<LeasefeesCountryAudit> list = new ArrayList<LeasefeesCountryAudit>(); 
		list = dao.getCountryAuditLeaseFees(whereStr, loginId);
		int num = list.size();//结果条数
		request.setAttribute("num", num);//结果条数
		request.setAttribute("list", list);//结果集
		request.getRequestDispatcher("/web/appJSP/lease/audit/leaseFeesCountryAudit.jsp")
	...
	throws ServletException, IOException {
		...
		if("tongguo".equals(action)){//通过正常部分
			...
			url = path + "/web/appJSP/lease/audit/leaseFeesCountryAudit.jsp" ;
		  	String audit = "1";//通过
		    msg = dao.CheckCountryFees(personnal,audit,chooseIdStr1,"1");
			...
			url = path + "/web/appJSP/lease/audit/leaseFeesCountryAudit.jsp" ;
		  	String audit = "2";//不通过
		    msg = dao.CheckCountryFees(personnal,audit,chooseIdStr1,"2");
			...
			url = path + "/web/appJSP/lease/audit/leaseFeesCountryAudit.jsp" ;
		  	String audit = "0";//通过
		    msg = dao.CheckCountryFees(personnal,audit,chooseIdStr1,"0");
