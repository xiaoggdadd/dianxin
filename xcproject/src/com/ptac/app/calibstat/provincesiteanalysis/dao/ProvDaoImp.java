 ...
 * @see 全省站点超标分析实现层
 */
public class ProvDaoImp implements ProvDao {

	/**
	 * @see 全省站点超标分析 查询、导出
	 */
	public List<ProvBean> getQueryExport(String str, String loginId) {
		
		List<ProvBean> list = new ArrayList<ProvBean>();
		String sql = "";
		...
		try {
			...
			sql = "SELECT SUM(ZD.RJDL1),SUM(ZD.RJDL2),SUM(ZD.RJDL3),SUM(ZD.RJDL4),SUM(ZD.RJDL5),SUM(ZD.RJDL6),SUM(ZD.RJDL7),"
				 ...
				 + "SUM(Z.RJDL10),SUM(Z.RJDL11),SUM(Z.RJDL12),RNDIQU(ZD.SHI),ZD.SHI"
			...
			System.out.println("全省站点超标分析："+sql);
			...
			while(rs.next()){
				...
				String city = rs.getString(25)!= null ? rs.getString(25) : "";//市(汉字)
				String shi = rs.getString(26)!= null ? rs.getString(26) : "";//市(code码)
	 ...
	 * @see 全省站点超标分析具体图表信息
	 */
	public ProvBean getXiangXi(String str, String loginId) {
		...
		try {
			...
			sql = "SELECT SUM(ZD.RJDL1),SUM(ZD.RJDL2),SUM(ZD.RJDL3),SUM(ZD.RJDL4),SUM(ZD.RJDL5),SUM(ZD.RJDL6),SUM(ZD.RJDL7),"
				 + "SUM(ZD.RJDL8),SUM(ZD.RJDL9),SUM(ZD.RJDL10),SUM(ZD.RJDL11),SUM(ZD.RJDL12),SUM(Z.RJDL1),SUM(Z.RJDL2),"
				 + "SUM(Z.RJDL3),SUM(Z.RJDL4),SUM(Z.RJDL5),SUM(Z.RJDL6),SUM(Z.RJDL7),SUM(Z.RJDL8),SUM(Z.RJDL9),"
				 + "SUM(Z.RJDL10),SUM(Z.RJDL11),SUM(Z.RJDL12),RNDIQU(ZD.SHI)"
				 + " FROM  T_ZDDBHZ@UNEBD_81 ZD, T_ZDDBSDB@UNEBD_81 Z,ZHANDIAN ZZ"
				 + " WHERE ZD.ZDID = Z.ZDID AND Z.ZDID = ZZ.ID "
				 + str
				 + " and exists (select t.agcode from per_area t where t.agcode=zd.xiaoqu and t.accountid = '"
			...
			System.out.println("全省站点超标分析具体图表信息："+sql);
