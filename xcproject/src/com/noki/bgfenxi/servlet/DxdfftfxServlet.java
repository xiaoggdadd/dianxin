...
public class DxdfftfxServlet extends HttpServlet {
...
//			} 
				...
				DxdffxBean bean = new DxdffxBean();
				bean.setDw(dw);
				bean.setQx(qx);
				bean.setZDMC(zdmc);
				bean.setZDBM(zdbm);
				...
				bean.setJSFS(jsfs);
				bean.setDH(dh);
				bean.setSccbsj(jfqsrq);
				bean.setBccbsj(jfzzrq);
				bean.setQM(qm);
				bean.setZM(zm);
				...
				Dxdffxmanage dfManage = new Dxdffxmanage();
				boolean flag = dfManage.adddxfx(bean);
			...
			String url = path + "/web/bgfenxi/dxdfftfx_search.jsp";
			...
			Dxdffxmanage dfManage = new Dxdffxmanage();
			boolean flag = dfManage.saveDxdffx(b);
			...
			HttpSession session = request.getSession();
			String path = request.getContextPath();
			String url = path + "/web/bgfenxi/tt_dxdfftfx_search.jsp";
			session.setAttribute("url", url);
			session.setAttribute("msg", msg);
			response.sendRedirect(path + "/web/msg.jsp");
		}
