...
public class HB {
  ...
  private String kjnd = CTime.formatShortDate().substring(0, 4);

  private int month = Integer.parseInt(CTime.formatShortDate().substring(4, 6));
  private DataBase db;
  ...
  public synchronized Vector getHuanbi_all(String dwdm, String qijian,
    ...
    try {
      ...
      String sqlx_sub1 = "SELECT SUM(ROUND(YD" + String.valueOf(month) +
          ...
          "' AND (kmdm like '401%' or kmdm like '404%' or kmdm like '407%')";
      // AND CN_DWDM IN (SELECT DWDM FROM CN_DWSUB WHERE GL_DWXZ='0')" + l;
      String sqlx_sub2 = "SELECT SUM(ROUND(YD" + String.valueOf(smonth) +
          ",2)-ROUND(yj" + String.valueOf(smonth) +
          ",2)) FROM GL_YEB WHERE KJND='" + skjnd +
          "' AND (kmdm like '401%' or kmdm like '404%' or kmdm like '407%')";
      // AND CN_DWDM IN (SELECT DWDM FROM CN_DWSUB WHERE GL_DWXZ='0')" + l;
      ...
      String sqls_sub3 = "select sum(round(yd" + String.valueOf(month) +
          ",2)-ROUND(yj" + String.valueOf(month) +
          ",2)) from gl_yeb where  ";
      // CN_DWDM IN (SELECT DWDM FROM CN_DWSUB WHERE GL_DWXZ='1')
      sqls_sub3 += " kjnd='" +
      ...
      String sqls_t1 = "SELECT (" + sqlx_sub1 + "),(" + sqlx_sub2 + "),(" +
          ...
          ",2)-ROUND(yj" + String.valueOf(smonth) +
          ",2)) FROM GL_YEB  where ";
      //  CN_DWDM IN (SELECT DWDM FROM CN_DWSUB WHERE GL_DWXZ='1') AND
      sqls_t1 += " kjnd='" +
