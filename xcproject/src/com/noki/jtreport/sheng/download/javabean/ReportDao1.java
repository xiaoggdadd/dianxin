...
public class ReportDao1 {
	...
	public List<String> execute1(List<String> errors) throws Exception{
		...
		String sql = "select t.content,t.shicode from jt_report_upload t where t.shicode in("+cityStr+") and t.actualmonth = '"+getMonth()+"' and t.filename = '"+getName()+"'";
		ResultSet rs = db.queryAll(sql);
		System.out.println("查询要下载的报表"+sql.toString());
		while(rs.next()){
		...
		for(InputStream is:streamList){
			...
			System.out.println("wwwwwwwwww:"+newBook.getNumberOfSheets()+"  tmpPath"+ tmpPath);
