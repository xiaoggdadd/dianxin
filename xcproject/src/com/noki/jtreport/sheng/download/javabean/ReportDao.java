...
public class ReportDao {
	 ...
	 * @throws Exception
	 */
	public List<String> execute(List<String> errors) throws Exception{
		...
		String sql = "select t.content,t.shicode from jt_report_upload t " +
				"where t.shicode in("+cityStr+") and t.actualmonth = '"+getMonth()+"' " +
				"and t.filename = '"+getName()+"' order by t.shicode";//按照地市编号排序 （ 与新疆的区别）
		ResultSet rs = db.queryAll(sql);
		System.out.println("查询报表---"+sql.toString());
		while(rs.next()){
			System.out.println("获取的报表--"+rs.getBinaryStream(1));
			streamList.add(rs.getBinaryStream(1));
			shiCode.add(rs.getString(2));
		}
		System.out.println("查询出的报表长度streamList-"+streamList.size());
