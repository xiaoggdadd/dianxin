package com.noki.zwhd.model;

public class DianbiaoBean {
	private String ID;
	private String ZDNAME;
	private String ZDID;
	private String AREA;
	private String JZTYPE;
	private String BGSIGN;
	private String BIEMING;
	private String DBID;
	private String SSZY;
	private String DBYT;
	private String DLLX;
	private String YDSB;
	private String CSDSOLD;
	private String CSSYTIMEOLD;
	private String BEILVOLD;
	private String DBXH;
	private String MEMO;
	private String SCODE;
	private String SHICODE;
	private String XCODE;
	private String NETPOINT_ID;
	private String NETPOINT_NAME;
	private String DBNAME;
	private String YHDLOLD;
	private String DFZFLX;
	private String LINELOSSTYPE;
	private String LINELOSSVALUEOLD;
	private String ENTRYPENSONNEL;
	private String ENTRYTIMEOLD;
	private String BZW;
	private String DBQYZT;
	private String DBZBDYHH;
	private String YDBID;
	private String DANJIAOLD;
	private String CHANGEVALUEOLD;
	private String DBDS;
	private String XGBZW;
	private String ZGDJ;
	private String ZZSL;
	private String CSLRR;
	private String CSLRSJ;
	private String CSDS;
	private String CSSYTIME;
	private String BEILV;
	private String YHDL;
	private String LINELOSSVALUE;
	private String ENTRYTIME;
	private String DANJIA;
	private String CHANGEVALUE;
	private String SHISHBZ;
	private String SHISHSJ;
	private String SHISHR;
	private String SHISHLY;
	private String SHENGSHBZ;
	private String SHENGSHSJ;
	private String SHENGSHR;
	private String SHENGSHLY;
	private String XZDBLY;
	private String DBCOUNTBZW;
	private String SCCBSJ;
	private String CBSJBZW;
	private String DBBM;
	private String SHZT;

	public String getSHZT() {
		return SHZT;
	}

	public void setSHZT(String sHZT) {
		SHZT = sHZT;
	}

	public String getID() {
		return ID;
	}

	public void setID(String iD) {
		ID = iD;
	}

	public String getZDNAME() {
		return ZDNAME;
	}

	public void setZDNAME(String zDNAME) {
		ZDNAME = zDNAME;
	}

	public String getZDID() {
		return ZDID;
	}

	public void setZDID(String zDID) {
		ZDID = zDID;
	}

	public String getAREA() {
		return AREA;
	}

	public void setAREA(String aREA) {
		AREA = aREA;
	}

	public String getJZTYPE() {
		return JZTYPE;
	}

	public void setJZTYPE(String jZTYPE) {
		JZTYPE = jZTYPE;
	}

	public String getBGSIGN() {
		return BGSIGN;
	}

	public void setBGSIGN(String bGSIGN) {
		BGSIGN = bGSIGN;
	}

	public String getBIEMING() {
		return BIEMING;
	}

	public void setBIEMING(String bIEMING) {
		BIEMING = bIEMING;
	}

	public String getDBID() {
		return DBID;
	}

	public void setDBID(String dBID) {
		DBID = dBID;
	}

	public String getSSZY() {
		return SSZY;
	}

	public void setSSZY(String sSZY) {
		SSZY = sSZY;
	}

	public String getDBYT() {
		return DBYT;
	}

	public void setDBYT(String dBYT) {
		DBYT = dBYT;
	}

	public String getDLLX() {
		return DLLX;
	}

	public void setDLLX(String dLLX) {
		DLLX = dLLX;
	}

	public String getYDSB() {
		return YDSB;
	}

	public void setYDSB(String yDSB) {
		YDSB = yDSB;
	}

	public String getCSDSOLD() {
		return CSDSOLD;
	}

	public void setCSDSOLD(String cSDSOLD) {
		CSDSOLD = cSDSOLD;
	}

	public String getCSSYTIMEOLD() {
		return CSSYTIMEOLD;
	}

	public void setCSSYTIMEOLD(String cSSYTIMEOLD) {
		CSSYTIMEOLD = cSSYTIMEOLD;
	}

	public String getBEILVOLD() {
		return BEILVOLD;
	}

	public void setBEILVOLD(String bEILVOLD) {
		BEILVOLD = bEILVOLD;
	}

	public String getDBXH() {
		return DBXH;
	}

	public void setDBXH(String dBXH) {
		DBXH = dBXH;
	}

	public String getMEMO() {
		return MEMO;
	}

	public void setMEMO(String mEMO) {
		MEMO = mEMO;
	}

	public String getSCODE() {
		return SCODE;
	}

	public void setSCODE(String sCODE) {
		SCODE = sCODE;
	}

	public String getSHICODE() {
		return SHICODE;
	}

	public void setSHICODE(String sHICODE) {
		SHICODE = sHICODE;
	}

	public String getXCODE() {
		return XCODE;
	}

	public void setXCODE(String xCODE) {
		XCODE = xCODE;
	}

	public String getNETPOINT_ID() {
		return NETPOINT_ID;
	}

	public void setNETPOINT_ID(String nETPOINT_ID) {
		NETPOINT_ID = nETPOINT_ID;
	}

	public String getNETPOINT_NAME() {
		return NETPOINT_NAME;
	}

	public void setNETPOINT_NAME(String nETPOINT_NAME) {
		NETPOINT_NAME = nETPOINT_NAME;
	}

	public String getDBNAME() {
		return DBNAME;
	}

	public void setDBNAME(String dBNAME) {
		DBNAME = dBNAME;
	}

	public String getYHDLOLD() {
		return YHDLOLD;
	}

	public void setYHDLOLD(String yHDLOLD) {
		YHDLOLD = yHDLOLD;
	}

	public String getDFZFLX() {
		return DFZFLX;
	}

	public void setDFZFLX(String dFZFLX) {
		DFZFLX = dFZFLX;
	}

	public String getLINELOSSTYPE() {
		return LINELOSSTYPE;
	}

	public void setLINELOSSTYPE(String lINELOSSTYPE) {
		LINELOSSTYPE = lINELOSSTYPE;
	}

	public String getLINELOSSVALUEOLD() {
		return LINELOSSVALUEOLD;
	}

	public void setLINELOSSVALUEOLD(String lINELOSSVALUEOLD) {
		LINELOSSVALUEOLD = lINELOSSVALUEOLD;
	}

	public String getENTRYPENSONNEL() {
		return ENTRYPENSONNEL;
	}

	public void setENTRYPENSONNEL(String eNTRYPENSONNEL) {
		ENTRYPENSONNEL = eNTRYPENSONNEL;
	}

	public String getENTRYTIMEOLD() {
		return ENTRYTIMEOLD;
	}

	public void setENTRYTIMEOLD(String eNTRYTIMEOLD) {
		ENTRYTIMEOLD = eNTRYTIMEOLD;
	}

	public String getBZW() {
		return BZW;
	}

	public void setBZW(String bZW) {
		BZW = bZW;
	}

	public String getDBQYZT() {
		return DBQYZT;
	}

	public void setDBQYZT(String dBQYZT) {
		DBQYZT = dBQYZT;
	}

	public String getDBZBDYHH() {
		return DBZBDYHH;
	}

	public void setDBZBDYHH(String dBZBDYHH) {
		DBZBDYHH = dBZBDYHH;
	}

	public String getYDBID() {
		return YDBID;
	}

	public void setYDBID(String yDBID) {
		YDBID = yDBID;
	}

	public String getDANJIAOLD() {
		return DANJIAOLD;
	}

	public void setDANJIAOLD(String dANJIAOLD) {
		DANJIAOLD = dANJIAOLD;
	}

	public String getCHANGEVALUEOLD() {
		return CHANGEVALUEOLD;
	}

	public void setCHANGEVALUEOLD(String cHANGEVALUEOLD) {
		CHANGEVALUEOLD = cHANGEVALUEOLD;
	}

	public String getDBDS() {
		return DBDS;
	}

	public void setDBDS(String dBDS) {
		DBDS = dBDS;
	}

	public String getXGBZW() {
		return XGBZW;
	}

	public void setXGBZW(String xGBZW) {
		XGBZW = xGBZW;
	}

	public String getZGDJ() {
		return ZGDJ;
	}

	public void setZGDJ(String zGDJ) {
		ZGDJ = zGDJ;
	}

	public String getZZSL() {
		return ZZSL;
	}

	public void setZZSL(String zZSL) {
		ZZSL = zZSL;
	}

	public String getCSLRR() {
		return CSLRR;
	}

	public void setCSLRR(String cSLRR) {
		CSLRR = cSLRR;
	}

	public String getCSLRSJ() {
		return CSLRSJ;
	}

	public void setCSLRSJ(String cSLRSJ) {
		CSLRSJ = cSLRSJ;
	}

	public String getCSDS() {
		return CSDS;
	}

	public void setCSDS(String cSDS) {
		CSDS = cSDS;
	}

	public String getCSSYTIME() {
		return CSSYTIME;
	}

	public void setCSSYTIME(String cSSYTIME) {
		CSSYTIME = cSSYTIME;
	}

	public String getBEILV() {
		return BEILV;
	}

	public void setBEILV(String bEILV) {
		BEILV = bEILV;
	}

	public String getYHDL() {
		return YHDL;
	}

	public void setYHDL(String yHDL) {
		YHDL = yHDL;
	}

	public String getLINELOSSVALUE() {
		return LINELOSSVALUE;
	}

	public void setLINELOSSVALUE(String lINELOSSVALUE) {
		LINELOSSVALUE = lINELOSSVALUE;
	}

	public String getENTRYTIME() {
		return ENTRYTIME;
	}

	public void setENTRYTIME(String eNTRYTIME) {
		ENTRYTIME = eNTRYTIME;
	}

	public String getDANJIA() {
		return DANJIA;
	}

	public void setDANJIA(String dANJIA) {
		DANJIA = dANJIA;
	}

	public String getCHANGEVALUE() {
		return CHANGEVALUE;
	}

	public void setCHANGEVALUE(String cHANGEVALUE) {
		CHANGEVALUE = cHANGEVALUE;
	}

	public String getSHISHBZ() {
		return SHISHBZ;
	}

	public void setSHISHBZ(String sHISHBZ) {
		SHISHBZ = sHISHBZ;
	}

	public String getSHISHSJ() {
		return SHISHSJ;
	}

	public void setSHISHSJ(String sHISHSJ) {
		SHISHSJ = sHISHSJ;
	}

	public String getSHISHR() {
		return SHISHR;
	}

	public void setSHISHR(String sHISHR) {
		SHISHR = sHISHR;
	}

	public String getSHISHLY() {
		return SHISHLY;
	}

	public void setSHISHLY(String sHISHLY) {
		SHISHLY = sHISHLY;
	}

	public String getSHENGSHBZ() {
		return SHENGSHBZ;
	}

	public void setSHENGSHBZ(String sHENGSHBZ) {
		SHENGSHBZ = sHENGSHBZ;
	}

	public String getSHENGSHSJ() {
		return SHENGSHSJ;
	}

	public void setSHENGSHSJ(String sHENGSHSJ) {
		SHENGSHSJ = sHENGSHSJ;
	}

	public String getSHENGSHR() {
		return SHENGSHR;
	}

	public void setSHENGSHR(String sHENGSHR) {
		SHENGSHR = sHENGSHR;
	}

	public String getSHENGSHLY() {
		return SHENGSHLY;
	}

	public void setSHENGSHLY(String sHENGSHLY) {
		SHENGSHLY = sHENGSHLY;
	}

	public String getXZDBLY() {
		return XZDBLY;
	}

	public void setXZDBLY(String xZDBLY) {
		XZDBLY = xZDBLY;
	}

	public String getDBCOUNTBZW() {
		return DBCOUNTBZW;
	}

	public void setDBCOUNTBZW(String dBCOUNTBZW) {
		DBCOUNTBZW = dBCOUNTBZW;
	}

	public String getSCCBSJ() {
		return SCCBSJ;
	}

	public void setSCCBSJ(String sCCBSJ) {
		SCCBSJ = sCCBSJ;
	}

	public String getCBSJBZW() {
		return CBSJBZW;
	}

	public void setCBSJBZW(String cBSJBZW) {
		CBSJBZW = cBSJBZW;
	}

	public String getDLONGITUDE() {
		return DLONGITUDE;
	}

	public void setDLONGITUDE(String dLONGITUDE) {
		DLONGITUDE = dLONGITUDE;
	}

	public String getDLATITUDE() {
		return DLATITUDE;
	}

	public void setDLATITUDE(String dLATITUDE) {
		DLATITUDE = dLATITUDE;
	}

	public String getDBBM() {
		return DBBM;
	}

	public void setDBBM(String dBBM) {
		DBBM = dBBM;
	}
	//鐢佃〃缁忕含搴�
	String DLONGITUDE;
	String DLATITUDE;

}
