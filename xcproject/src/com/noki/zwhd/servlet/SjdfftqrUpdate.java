...
public class SjdfftqrUpdate extends HttpServlet {
...
//		SimpleDateFormat mdFormat = new SimpleDateFormat("MM.dd");
		...
		String t_tzje = request.getParameter("t_tzje");
		String t_tzr = request.getParameter("t_tzr");
		String t_tzyy = request.getParameter("t_tzyy");
		
		WydfftBean wydfft = new WydfftBean();
		wydfft.setTZHJE(t_tzje);
		wydfft.setTZR(t_tzr);
		wydfft.setTZYY(t_tzyy);
...
//		}
		...
		if(wydfft.getID()!=null&&!wydfft.getID().equals("")&&!wydfft.getID().equals("null")){
			flag2 = wyManage.updateWydfft_yxg(wydfft);
		}
		if(flag2){
			msg = "市级电费分摊确认修改失败！";
		}else{
		...
		String url = path + "/web/wyftdfsh/tt_sjdfftqr_search.jsp";
