...
function getDictList(maxrow,minrow){
	...
	var params ="&key=value";
	 ...
	 Url = getPublicPath()+"/servlet/ProjectinfoServlet?action=getSelData&roleFlg=PERMISSION_SEARCH";
	 				 ...
	 				 if (rowDate!=""){
		 				  for(var i=0;i<rowDate.length;i++){
			 					...
			 					var cellDate = rowDate[i].split("|");//有多少列
			 						...
			 						var dirNode2 = document.createElement("div");//创建一个层
			 			    		dirNode2.setAttribute("align","center");//层居中
			 						var divText=document.createTextNode(cellDate[3]);//创建文本节点
			 			    		dirNode2.appendChild(divText);//层中加入文本
			 			    		tdNode2.appendChild(dirNode2);//将层节点加入列
			 			    		trNode.appendChild(tdNode2);
...
//增加数据
 function add(){
     window.location.href=getPublicPath()+"/"+"web/projectinfo/add.jsp";
   ...
   function open1(url){
        window.location.href=getPublicPath()+"/"+"web/projectinfo/"+url;
...
//
   function del(id){
     if(confirm("确定删除")){
       var params = "?&xmmch="+id;
       var Url=getPublicPath()+"/servlet/ProjectinfoServlet?action=del&roleFlg=PERMISSION_DELETE";
	    new Ajax.Request(Url,{
	      		method : 'post',
				asynchronous: false,
			    parameters : params,
													...
													<td height="49" bgcolor="#FFFFFF">
													     ...
													     <table width="100%" border="0" cellspacing="1" cellpadding="1" bgcolor="#cbd5de" id ="dict">
													         <tr>
													             ...
													             <td nowrap height="23" bgcolor="#888888"><div align="center" class="bttcn">项目负责人</div></td>
