...
function queryDegree() {
	document.form1.action = path
			+ "/web/jzcbnewfunction/zgcwsh.jsp?command=chaxun";
	...
	<body class="body" style="overflow-x: hidden;">
		<form action="" name="form1" method="POST">
			...
			<tr>
			    	<td colspan="4" width="50" >
						 <div style="width:700px;height:50px">
						  ...
						  <span style="font-size:14px;font-weight:bold;position: absolute;left:25px;top:15px;color:black">财务审核</span>	
						 </div>
					</td>
				</tr>
				<tr>
				...
				out.print(request.getParameter("zdname"));%>"
								...
								<td>
									<select name="cwshzt" class="selected_font"
										onchange="javascript:document.form1.cwshzt2.value=document.form1.cwshzt.value">
										<option value="-1">
											请选择
										</option>
										<option value="1">
			...
			<%
				...
				if (cwshzt != null && !cwshzt.equals("") && !cwshzt.equals("-1")) {
					whereStr = whereStr + " and s.cwshzt='" + cwshzt + "'";
				}
			...
			<div
				...
				<table width="1200px" height="80%" border="0" cellspacing="1"
					...
					<%
						...
						if ("daochu".equals(request.getParameter("command"))
							...
							list = bean.getCaiWu(whereStr, loginId);
						String id = "";
						String zdid = "";//站点编号
						String shi1 = "";//市
						String xian1 = "";//县
						...
						DecimalFormat mat2=new DecimalFormat("0.00");
							for (zgsj bean2 : list) {
								...
								if(null==cwshzt1 || "null".equals(cwshzt1) ||"".equals(cwshzt1)|| cwshzt1.equals(" ") ||cwshzt1==""){
									cwshzt1="";
								}else if("0".equals(cwshzt1)){
									cwshzt1="未审核";
								}else if("1".equals(cwshzt1)){
									cwshzt1="通过";
								}else if("2".equals(cwshzt1)){
									cwshzt1="未通过";
								}
								...
								sdb = mat2.format(Double.parseDouble(sdb));
								dj = mat1.format(Double.parseDouble(dj));
								zgdf = mat2.format(Double.parseDouble(zgdf));
...
function passCheck() {
	...
	if (count != 0) {
		...
		document.form1.action = path
				+ "/servlet/ZGsjServlet?action=cwzdshyes&chooseIdstr1="
...
function passCheckNo() {
	...
	for ( var i = 0; i < l; i++) {
		...
		if (count != 0) {
			...
			document.form1.action = path
				+ "/servlet/ZGsjServlet?action=cwzdshno&chooseIdstr1="
				+ chooseIdStr1 + "&loginName=" + loginName;
			document.form1.submit();
...
function passCheckqx() {
	...
	for ( var i = 0; i < l; i++) {
		...
		if (count != 0) {
			...
			chooseIdStr1 = chooseIdStr1.substring(0, chooseIdStr1.length - 1);
			document.form1.action = path
				+ "/servlet/ZGsjServlet?action=cwzdshqx&chooseIdstr1="
				+ chooseIdStr1 + "&loginName=" + loginName;
			document.form1.submit();
		} else {
			alert("请选择信息！");
...
document.form1.cwshzt.value = '<%=cwshzt%>';
</script>
</html>
