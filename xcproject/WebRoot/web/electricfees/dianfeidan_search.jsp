...
<%@ page import="com.noki.dldfgl.DFDLbean"%>
<%@ page import="com.noki.dldfgl.DFDLManage"%>
<%
	...
	String cbzt = request.getParameter("cbzt")!=null?request.getParameter("cbzt") : "0";
...
<script language="javascript">
	...
	function chaxun() {
		document.form1.action = path + "/web/electricfees/dianfeidan_search.jsp";
	...
	function adddegree1(){
          document.form1.action=path+"/web/electricfees/ltadddianfeidan.jsp";
          document.form1.submit();
        }
        function adddegree2(){
          document.form1.action=path+"/web/electricfees/dxadddianfeidan.jsp";
          document.form1.submit();
        }
	function daorujz() {
	...
	$(function() {
		$("#xiugaiBtn").click(function(){
			...
			if (i > 0) {			
				document.form1.action = path + "/web/electricfees/quxianchaobiao.jsp?id="+_id;
				document.form1.submit();			
				//var kh = $("#kh").html();
				//var stationname = $("#stationname").html();
				//var ftje = $("#ftje").html();
				//document.form1.action = path + "/web/electricfees/quxianchaobiao.jsp?stationname='"+stationname+"'&id="+_id+"&ftje="+ftje+"";
				//document.form1.submit();			
			} else {
				alert("请选择要修改的条目");
				return;
		...
		$("#updateBtn").click(function(){
			...
			for ( var j = 0; j < document.form1.elements.length; j++) {
				...
				//alert(stationname);
					if("已抄表"==cbzt){
				//alert(stationname);
					document.form1.action = path + "/web/electricfees/quxianxiugai.jsp?stationname='"+stationname+"'&id="+_id+"&ftje="+ftje+"";
					document.form1.submit();			
				}else {
				alert("只有已抄表的才可以修改");
				}				
			...
			var id;
				...
				if (document.form1.elements[j].checked) {
					i++;
					id = document.form1.elements[j].value;
					alert(id);
					if(confirm("您确认要删除本条电表信息？")){
		     		document.form1.action=path+"/servlet/dianbiao?action=del2&id="+id;
		      		document.form1.submit();
		      		showdiv("请稍等..............");
		      	}
				}
		...
		$("#ltzengjia").click(function(){
			adddegree1();	
		});
		$("#dxzengjia").click(function(){
			adddegree2();	
		});
...
function nextpage() {
	...
	document.form1.action = path + "/web/electricfees/dianfeidan_search.jsp?curpage="
			+ curpage;
	document.form1.submit();
}
...
function gopagebyno(pageno) {
	...
	document.form1.action = path + "/web/electricfees/dianfeidan_search.jsp?curpage=" + pageno;
	document.form1.submit();
}
</script>
			...
			<tr>
				<td colspan="4">
					<div style="width:700px;height=50px">
						<!--    <img alt="" src="<%=path%>/images/btt.bmp" width="100%" height="100%" />-->
						<span
							style="font-size:14px;font-weight:bold;position: absolute;left:25px;top:15px;color:black">区县抄表查询</span>
					</div></td>
			</tr>
			<tr>
...
<div style="width:88%;" > 
			...
			<table border='0'>
				<tr class="form_label">
				    ...
				    <td>
				    	<select name="cbzt">
				        	<option value="0">请选择</option>
				            <option value="已核对">已抄表</option>
				            <option value="未核对">未抄表</option>
				    	</select>
					</td>
					<td>批次：</td>
		            <td>
		...
		<table width="2000px" height="60%" border="0" cellspacing="1" cellpadding="1" bgcolor="#cbd5de" class="form_label">
			<tr bgcolor="#DDDDDD">
				...
				<td width="100" height="23"><div align="center" class="bttcn">批次</div></td>
				<td width="80" height="23"><div align="center" class="bttcn">抄表状态</div></td>
				<td width="200" height="23"><div align="center" class="bttcn">省分公司</div>
				</td>
				<td width="200" height="23"><div align="center" class="bttcn">地市分公司</div>
				</td>
				<td nowrap height="23"><div align="center" class="bttcn">站点名称</div>
				</td>
				<td nowrap height="23"><div align="center" class="bttcn">站点编码</div>				
				</td>
				<td nowrap height="23"><div align="center" class="bttcn">供电方式</div>				
				</td>
				<td nowrap height="23"><div align="center" class="bttcn">单号</div>
				</td>
				<td nowrap height="23"><div align="center" class="bttcn">账期</div>
				</td>
				<td nowrap height="23"><div align="center" class="bttcn">缴费金额</div>
				</td>
				<td nowrap height="23"><div align="center" class="bttcn">销账标识</div>
				</td>
				<td nowrap height="23"><div align="center" class="bttcn">缴费票据类型</div>
				</td>
				<td width="200" height="23"><div align="center" class="bttcn">供电方名称</div>
				</td>
				...
				<td nowrap height="23"><div align="center" class="bttcn">开票类型</div>
				</td>
				<td nowrap height="23"><div align="center" class="bttcn">缴费开始日期</div>
				</td>
				<td nowrap height="23"><div align="center" class="bttcn">缴费终止日期</div>
				</td>
				<td nowrap height="23"><div align="center" class="bttcn">起码</div>
				</td>
				<td nowrap height="23"><div align="center" class="bttcn">止码</div>
				</td>
				<td nowrap height="23"><div align="center" class="bttcn">电量</div>
				</td>
				<td nowrap height="23"><div align="center" class="bttcn">电流</div>
				</td>
				<td nowrap height="23"><div align="center" class="bttcn">损耗</div>
				</td>
				<td nowrap height="23"><div align="center" class="bttcn">上次抄表时间</div>
				</td>
				<td nowrap height="23"><div align="center" class="bttcn">本次抄表时间</div>
				</td>
			</tr>
			<%
				DFDLManage wyManage = new DFDLManage();
				allpage = wyManage.searchCountWydfft()/pageSize+1;
				List<DFDLbean> wydfftList = wyManage.searchWydfft_fy1(loginName,loginId,pageSize,curpage,sheng,shi,xian,xiaoqu,"已确认",cbzt,pc,zdbm);
				for(int i=0;i<wydfftList.size();i++) {
					DFDLbean wydfft = wydfftList.get(i);
					if(i%2==0){
		 				color="#FFFFFF";
					}else{
					   color="#DDDDDD";
			...
			<tr bgcolor="<%=color%>">
				<td>
					<div align="center" name="itemSelected"><input type="checkbox" 
					value="<%=wydfft.getID()%>" /></div></td>
				<td>
					<div align="center"><%=wydfft.getRownum()%></div></td>
				<td>
					<div id="cbzt" align="center"><%=wydfft.getYEARMONTH()%></div></td>
				<td>
					<div id="cbzt" align="center"><%=wydfft.getCBZT()%></div></td>
				<td>
					<div align="left"><%=wydfft.getSFGS()%></div></td>
				<td>
					<div align="left"><%=wydfft.getDSFGS()%></div></td>
				<td>
					<div id="stationname" align="left"><%=wydfft.getZDMC()%></div></td>
				<td>
				...
				<td>
					<div id="kh" align="center"><%=wydfft.getKH()%></div></td>
				<td>
					<div align="center"><%=wydfft.getFTBL()%></div></td>
				<td>
					<div align="center"><%=wydfft.getFSYZ()%></div></td>
				<td>
					<div id="ftje" align="center"><%=wydfft.getFTJE()%></div></td>
				<td>
					<div align="center"><%=wydfft.getKPLX()%></div></td>
				<td>
					<div align="center"><%=wydfft.getCB_JFQSRQ()%></div></td>
				<td>
					<div align="center"><%=wydfft.getCB_JFZZRQ()%></div></td>
				<td>
					<div align="center"><%=wydfft.getCB_QM()%></div></td>
				<td>
					<div align="center"><%=wydfft.getCB_ZM()%></div></td>	
				<td>
					<div align="center"><%=wydfft.getCB_DLIANG()%></div></td>
				<td>
					<div align="center"><%=wydfft.getCB_DLIU()%></div></td>
				<td>
					<div align="center"><%=wydfft.getSUNHAO()%></div></td>
				<td>
					<div align="center"><%=wydfft.getLASTCBSJ()%></div></td>
				<td>
					<div align="center"><%=wydfft.getTHISCBSJ()%></div></td>	
			</tr>
...
<!-- 					</div>					  -->
					...
					<div id="xiugaiBtn"
						...
						<img src="<%=path%>/images/daoru.png" width="100%" height="100%">
						<span
							style="font-size:12px;position: absolute;left:27px;top:3px;color:white">抄表</span>
					</div>
					<%
					 		}
					 	}
...
function updateZd(res){
	...
	document.form1.cbzt.value='<%=cbzt%>';
