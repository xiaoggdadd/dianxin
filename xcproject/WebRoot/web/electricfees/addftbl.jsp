	...
	function ShowHideSearchRegion(trObject,SelfObject)
{
	...
	function saveAccount(){
          			var f = parseInt(document.form1.ftbl.value);		
          			var ff = parseInt(document.form1.ftblq.value);
          			var fff = parseInt(document.form1.ftblqw.value);
          			var a = f+ff+fff;	
          			var b = f+ff;
          			var c = ff+fff;
          			var d = f+fff;
	     	   if(a=="100"|b=="100"|c=="100"|d=="100"|f=="100"|ff=="100"|fff=="100"){
	     	   	addzhandian();   
	     	   }else{
	     	   	alert("分摊比例总和必须为100");
	     	   }
...
//    function Zdmc(){
//    		var zdmc = document.form1.zdmc.value;
   		
//    		document.form1.action=path+"web/electricfees/addftbl.jsp?zdmc="+zdmc;
//    }
        	function addzhandian(){
        		if(confirm("您将要添加分摊比例信息！确认信息正确！")){
        		
          		document.form1.action=path+"/servlet/DFDLservlet?action=addftbl";
	   ...
	   function fanhui(){       
              document.form1.action=path+"/web/electricfees/ftbl_search.jsp";
              document.form1.submit();
        }
        function queding(){
...
function reset(){
	document.form1.zdmc.value="";
	document.form1.kehu.value="";
	document.form1.ftbl.value="";
}
...
<form action="" name="form1" method="POST">
		...
		<tr class="form_label">
         ...
         <select name="shi" style="width:130;" onchange="changeCity()">
	         	...
	         	shenglist = commBean.getAgcode(shengId,"0",loginName);//获取该登录用户负责的城市集合 
	         	if(shenglist!=null){
	         		String sfid="",sfnm="";
         ...
         <td width="26%">
         <select name="xiaoqu" id="xiaoqu" style="width:130" onchange="changezdm()">
         		<option value="0">请选择</option>
         	</select><span class="style1">&nbsp;*</span>
         </td>
      </tr>								
	<tr class="form_label">
     ...
     <td width="25%">
         <select name="zdmc" id="zdmc" style="width:130" onchange="changezdmc()">
         		<option value="0">请选择</option>
         		<% 
	         	ArrayList shenglist1= new ArrayList();
	         	shenglist1 = commBean.getZDNAME();//获取该登录用户负责的站点名称集合 
	         	if(shenglist1!=null){
	         		String sfid="",sfnm="";
	         		int scount = ((Integer)shenglist1.get(0)).intValue();
	                //System.out.print(shenglist.indexOf("JZNAME"));
 	         		for(int i=scount;i<shenglist1.size()-1;i+=scount)
                     {
                    	sfnm=(String)shenglist1.get(i+shenglist1.indexOf("JZNAME"));
                   	    sfid=(String)shenglist1.get(i+shenglist1.indexOf("JZCODE"));
                     	//System.out.println(sfid);
//                     %>
                    <option value="<%=sfid%>"><%=sfid%></option>
                    <% }
	         	}
	         %>
         	</select><span class="style1">&nbsp;*</span>
         </td>
         <td height="8%" bgcolor="#DDDDDD" width="15%"><div align="left">电表编码：</div>
      </td>
      <td width="25%">
         <select name="dbbm" id="dbbm" style="width:130">
         <option value="0">请选择</option>
<!--                   		<  -->
<!-- 	         	ArrayList list= new ArrayList(); -->
<!-- 	         	list = commBean.getDBBM(_zdmc);//获取该登录用户负责的站点名称集合  -->
<!-- 	         	if(list!=null){ -->
<!-- 	         		String sfid="",sfnm=""; -->
...
<!--                      { -->
<!--                     	sfid=(String)list.get(i+list.indexOf("DBBM")); -->
<!--                    	    //sfnm=(String)list.get(i+list.indexOf("JZCODE")); -->
<!--                      	//System.out.println(sfid); -->
<!-- //                     %> -->
<!--                     <option value="<=sfid%>"><=sfid%></option> -->
<!--                     < } -->
...
<!-- 	         %> -->
     ...
     <tr>
	 <td height="8%" bgcolor="#DDDDDD" width="15%"><div align="left">客户：</div>
     </td>
     <td width="25%"><input type="text" name="kehu" value="移动" readonly="true"  style="width:130px"/></td>
     <td height="8%" bgcolor="#DDDDDD" width="15%"><div align="left">分摊比例：</div>
     </td>
     <td width="25%"><input type="text" name="ftbl"  style="width:130px"/></td>
	</tr>
	 <tr>
	 <td height="8%" bgcolor="#DDDDDD" width="15%"><div align="left">客户：</div>
     </td>
     <td width="25%"><input type="text" name="kh" value="联通" readonly="true"  style="width:130px"/></td>
     <td height="8%" bgcolor="#DDDDDD" width="15%"><div align="left">分摊比例：</div>
     </td>
     <td width="25%"><input type="text" name="ftblq"  style="width:130px"/></td>
	</tr>
	 <tr>
	 <td height="8%" bgcolor="#DDDDDD" width="15%"><div align="left">客户：</div>
     </td>
     <td width="25%"><input type="text" name="kehuqw" value="电信" readonly="true"  style="width:130px"/></td>
     <td height="8%" bgcolor="#DDDDDD" width="15%"><div align="left">分摊比例：</div>
     </td>
     <td width="25%"><input type="text" name="ftblqw"  style="width:130px"/></td>
	</tr>
<tr class="form_label">
        <td colspan="6">
...
var XMLHttpReq;
	...
	function sendRequest(url,para) {
		...
		//alert("111");
			XMLHttpReq.onreadystatechange = processResponse_zdmc;//指定响应函数
		}else if(para=="zdm"){
			XMLHttpReq.onreadystatechange = processResponse_zdm;
		}
		else{
...
function changezdmc(){
 	var zdmc = document.form1.zdmc.value;
 	alert(zdmc);
 	//this.location.href = path+"/web/electricfees/addftbl.jsp?zdmc="+zdmc;
 	//document.form1.action=path+"/web/electricfees/addftbl.jsp?zdmc="+zdmc;
	alert(zdmc);
	if(zdmc==null){
		...
		sendRequest(path+"/servlet/garea?action=zdmc&code="+encodeURIComponent(zdmc),"zdmc");
	}
}
function changezdm(){
	...
	if(xiaoqu==null){
		...
		sendRequest(path+"/servlet/garea?action=zdm&code="+xiaoqu,"zdm");
	}
}
