...
<%
	...
	if (request.getParameter("shi") == null) {
		...
		if (shilist != null) {
			...
			agcode1 = (String) shilist.get(scount
					+ shilist.indexOf("AGCODE"));//获取 字段的 编码
		}
	}
	...
	String beginTimeQ = request.getParameter("beginTimeQ") != null ? request
			.getParameter("beginTimeQ")
			: "";
	String endTimeQ = request.getParameter("endTimeQ") != null ? request
			.getParameter("endTimeQ")
			: "";
	String zdbh = request.getParameter("zdbh") != null ? request
			.getParameter("zdbh") : "";
...
function delLogs() {
    ...
    function delad(electricfeeId){
       if(confirm("您确定删除此电费信息？")){
                    document.form1.action=path+"/servlet/electricfees?action=delete1&deladfee=deladfee&degreeid="+electricfeeId;
        			document.form1.submit();
        			 showdiv("请稍等..........");
       }
    }
    function modifyad(electricfeeId){
                    document.form1.action=path+"/web/electricfees/newmodifyElectricFeesDegree.jsp?degreeid="+electricfeeId;
                    document.form1.submit();
                     showdiv("请稍等..........");
    ...
    function queryDegree(){
                   document.form1.action=path+"/web/electricfees/newelectricFeesInput.jsp?command=chaxun";
                   document.form1.submit();
    ...
    //选择所有按钮
     ...
     function deletes(){
        ...
        if(bz>0){
        	if(bz<=50){
	           document.form1.action=path+"/servlet/electricfees?action=deletes&biaozhi=lao&chooseIdStr="+chooseIdStr;
	           document.form1.submit();
	           showdiv("请稍等..........");
           }else{
  ...
  function exportFData(){
          document.form1.action=path+"/web/electricfees/inputdegreefees.jsp";
                                document.form1.submit();
  }
  $(function(){
...
if(targetObj.style.display!="none"){
   ...
   if(openTip  &&  shutTip){
    sourceObj.innerHTML = openTip; 
		...
		<script src="<%=path%>/javascript/wait.js">
</script>
	</head>
		...
		<form action="" name="form1" method="POST">
			<table width="100%" border="0" cellspacing="0" cellpadding="0">
				<tr>
					<td width="50">
						<div style="width: 700px;">
							...
							<span
								style="font-size: 14px; font-weight: bold; position: absolute; left: 25px; top: 15px; color: black">电费单管理</span>
						</div>
					</td>
				</tr>
				<tr>
							...
							<tr class="form_label">
								...
								<td>
									<select name="shi" class="selected_font"
										...
										<%
											ArrayList shilist = new ArrayList();
											System.out.println("__________________________sheng:" + sheng);
											System.out
													.println("__________________________account.getAccountId():"
															+ account.getAccountId());
											shilist = commBean.getAgcode(sheng, account.getAccountId());
											System.out.println("__________________________1:" + shilist);
											if (shilist != null) {
												String agcode = "", agname = "";
												System.out.println("__________________________2:"
														+ (Integer) shilist.get(0));
												int scount = ((Integer) shilist.get(0)).intValue();
												System.out.println("__________________________3:" + scount);
							...
							<p id="box3" style="display: none">
								<table>
									<tr class="form_label">
										...
										<td>
											<input type="text" name="zdbh" class="selected_font"
												value="<%if (null != request.getParameter("zdbh"))
				out.print(request.getParameter("zdbh"));%>" />
										</td>
										<td>
											站点类型：
										</td>
										...
										<td>
											<input type="text" name="beginTimeQ" class="selected_font"
												value="<%if (null != request.getParameter("beginTimeQ"))
				out.print(request.getParameter("beginTimeQ"));%>"
										...
										<td>
											<input type="text" name="endTimeQ" class="selected_font"
												value="<%if (null != request.getParameter("endTimeQ"))
				out.print(request.getParameter("endTimeQ"));%>"
												onFocus="getDateString(this,oCalendarChs)" />
										</td>
									</tr>
									<tr class="form_label">
										...
										<td>
											<input type="text" name="beginTime1" class="selected_font"
				...
				out.print(request.getParameter("endTime1"));%>"
										...
										<td>
											<input type="text" name="accountmonth" class="selected_font"
			...
			<div
				...
				<table width="100%" height="60%" border="0" cellspacing="1"
					...
					<tr height="10" class="relativeTag">
						...
						<td width="11%" height="23" bgcolor="#DDDDDD">
							<div align="center" class="bttcn">
								所在地区
							</div>
						</td>
						...
						<td width="6%" height="15" bgcolor="#DDDDDD">
							<div align="center" class="bttcn">
								本次单价
							</div>
						</td>
						<td width="8%" height="15" bgcolor="#DDDDDD">
							<div align="center" class="bttcn">
								用电费用调整
							</div>
						</td>
						<td width="8%" height="18" bgcolor="#DDDDDD">
							<div align="center" class="bttcn">
								实际电费金额
							</div>
						</td>
						...
						<td width="5%" height="23" bgcolor="#DDDDDD" colspan="2">
							<div align="center" class="bttcn">
								操作
							</div>
						</td>
					...
					<%
						...
						if (beginTimeQ != null && beginTimeQ != ""
								&& !beginTimeQ.equals("0")) {
							whereStr = whereStr
									+ " and to_char(ad.lastdatetime,'yyyy-mm-dd')='"
									+ beginTimeQ + "'";
							str = str + " and to_char(ad.lastdatetime,'yyyy-mm-dd')='"
									+ beginTimeQ + "'";
						}
						if (endTimeQ != null && endTimeQ != "" && !endTimeQ.equals("0")) {
							whereStr = whereStr
									+ " and to_char(ad.thisdatetime,'yyyy-mm-dd')='"
									+ endTimeQ + "'";
							str = str + " and to_char(ad.thisdatetime,'yyyy-mm-dd')='"
									+ endTimeQ + "'";
						}
						...
						if (beginTime1 != null && beginTime1 != ""
								&& !beginTime1.equals("0")) {
							whereStr = whereStr + " and to_char(ad.STARTMONTH,'yyyy-mm')='"
									+ beginTime1 + "'";
						}
						if (endTime1 != null && endTime1 != "" && !endTime1.equals("0")) {
							whereStr = whereStr + " and to_char(ad.ENDMONTH,'yyyy-mm')='"
									+ endTime1 + "'";
						}
						//站点类型
						if (stationtype1 != null && !stationtype1.equals("")
								&& !stationtype1.equals("0")) {
						...
						if (accountmonth != null && !accountmonth.equals("")
							...
							whereStr = whereStr
									+ " and to_char(ef.accountmonth,'yyyy-mm')='"
									+ accountmonth + "'";
							str = str + " and to_char(ef.accountmonth,'yyyy-mm')='"
									+ accountmonth + "'";
						}
						//}
						...
						if ("daochu".equals(request.getParameter("command"))
							...
							fylist = bean.getPageDataDegreeFees(whereStr, loginId);
							allpage = bean.getAllPage();
							String electricfeeId = "", unitprice = "", floatpay = "", actualpay = "", notetypeid = "", noteno = "", payoperator = "", paydatetime = "", stationtype2 = "";
							String jzname = "", jzcode = "", manualauditstatus = "", df = "", szdq = "", ammeterdegreeid = "";
							...
							if (fylist != null) {
								...
								for (int k = fycount; k < fylist.size() - 1; k += fycount) {
									linenum++;
									//num为序号，不同页中序号是连续的
									jzname = (String) fylist.get(k
											+ fylist.indexOf("JZNAME"));
									jzname = jzname != null ? jzname : "";

									szdq = (String) fylist.get(k + fylist.indexOf("SZDQ"));
									szdq = szdq != null ? szdq : "";

									jzcode = (String) fylist.get(k
											+ fylist.indexOf("ZDCODE"));
									jzcode = jzcode != null ? jzcode : "";

									//添加站点类型
									stationtype2 = (String) fylist.get(k
											+ fylist.indexOf("STATIONTYPE"));
									stationtype2 = stationtype2 != null ? stationtype2 : "";

									ammeterdegreeid = (String) fylist.get(k
											+ fylist.indexOf("AMMETERDEGREEID"));
									ammeterdegreeid = ammeterdegreeid != null ? ammeterdegreeid
											: "";

									electricfeeId = (String) fylist.get(k
											+ fylist.indexOf("ELECTRICFEE_ID"));
									electricfeeId = electricfeeId != null ? electricfeeId
											: "";

									unitprice = (String) fylist.get(k
											+ fylist.indexOf("UNITPRICE"));
									unitprice = unitprice != null ? unitprice : "";

									floatpay = (String) fylist.get(k
											+ fylist.indexOf("FLOATPAY"));
									floatpay = floatpay != null ? floatpay : "";

									actualpay = (String) fylist.get(k
											+ fylist.indexOf("ACTUALPAY"));
									actualpay = actualpay != null ? actualpay : "";

									notetypeid = (String) fylist.get(k
											+ fylist.indexOf("NOTETYPEID"));
									notetypeid = notetypeid != null ? notetypeid : "";

									noteno = (String) fylist.get(k
											+ fylist.indexOf("NOTENO"));
									noteno = noteno != null ? noteno : "";

									payoperator = (String) fylist.get(k
											+ fylist.indexOf("PAYOPERATOR"));
									payoperator = payoperator != null ? payoperator : "";

									paydatetime = (String) fylist.get(k
											+ fylist.indexOf("PAYDATETIME"));
									paydatetime = paydatetime != null ? paydatetime : "";

									manualauditstatus = (String) fylist.get(k
											+ fylist.indexOf("MANUALAUDITSTATUS"));
									manualauditstatus = manualauditstatus != null ? manualauditstatus
											: "";

									if (actualpay == null || actualpay.equals("")
											|| actualpay.equals("null")
											|| actualpay.equals("o"))
										actualpay = "0";
									DecimalFormat mat = new DecimalFormat("0.00");
									dianfei = Double.parseDouble(actualpay);
									df = mat.format(dianfei);

									if (unitprice == null || unitprice.equals("")
											|| unitprice.equals("null")
											|| unitprice.equals("o"))
										unitprice = "0";
									DecimalFormat price = new DecimalFormat("0.0000");
									unitprice = price.format(Double.parseDouble(unitprice));

									if (floatpay == null || floatpay.equals("")
											|| floatpay.equals("null")
											|| floatpay.equals("o"))
										floatpay = "0";
					...
					<tr bgcolor="<%=color%>">
						...
						<td>
							<div align="center">
								<a href="javascript:modifyad('<%=electricfeeId%>')">修改</a>
							</div>
						</td>
						<td>
							<%
							...
							%>
							<div align="center">
								<a href="javascript:delad('<%=electricfeeId%>')">删除</a>
							</div>
							<%
								}
							%>
						...
						<td align="right">
							...
							<div id="zengjia"
								...
								<img alt="" src="<%=path%>/images/xinzeng.png" width="100%"
									height="100%" />
								<span
									style="font-size: 12px; position: absolute; left: 25px; top: 5px; color: white">增加</span>
							</div>
							...
							%>
							<div id="daochu"
								style="width: 60px; height: 23px; cursor: pointer; float: right; position: relative; right: 9px;">
								<img alt="" src="<%=path%>/images/daochu.png" width="100%"
									height="100%" />
								<span
									style="font-size: 12px; position: absolute; left: 25px; top: 5px; color: white">导出</span>
							</div>
							<%
								}
							%>
							...
							%>
							<div id="deletes"
								style="width: 76px; height: 23px; cursor: pointer; float: right; position: relative; right: 11px;">
								<img alt="" src="<%=path%>/images/quxiao.png" width="100%"
									height="100%" />
								<span
									style="font-size: 12px; position: absolute; left: 25px; top: 5px; color: white">批量删除</span>
							</div>
							<%
								}
							%>
...
function previouspage() {
	if ((parseInt(document.form1.page.value)) < 1)
		...
		document.form1.action = path
				+ "/web/electricfees/newelectricFeesInput.jsp?curpage="
				+ curpage + "<%=canshuStr%>";
		document.form1.submit();
	}
}
function nextpage() {
	...
	var curpage = '<%=(curpage + 1)%>';
	document.form1.action = path
			+ "/web/electricfees/newelectricFeesInput.jsp?curpage=" + curpage
			+ "<%=canshuStr%>";
	document.form1.submit();
}
function gopagebyno(pageno) {
	...
	document.form1.action = path
			+ "/web/electricfees/newelectricFeesInput.jsp?curpage=" + pageno
			+ "<%=canshuStr%>";
	document.form1.submit();
}
</script>
...
= <%=curpage%>;
            ...
            var zdbh ="<%=zdbh%>";
            //alert(canshuStr);
        	document.form1.action=path+"/web/electricfees/电费单.jsp?curpage="+curpage+"&whereStr="+whereStr+"&zdbh="+zdbh+"&command=daochu";
            document.form1.submit();
   }
