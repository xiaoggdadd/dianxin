...
<%
	...
	String zdid="",qq="",ww="",rr="",aa="",kehu="",ftbl="",ftid="",zz="",xx="",jzname="",dbbm="";
	StringBuffer sql = new StringBuffer();
	StringBuffer sql2 = new StringBuffer();
	StringBuffer sql3 = new StringBuffer();
	sql.append("select * from ftblquery where id="+id+"");
	...
	sql3.append("select jzname from zhandian where jzcode='"+zdid+"'");
	db3.connectDb();
	rs3=db3.queryAll(sql3.toString());
	while(rs3.next()){
		jzname=rs3.getString("jzname");
	}
	sql2.append("select * from ftblquery where zdid="+zdid+"");
	db2.connectDb();
	rs2= db2.queryAll(sql2.toString());
	ArrayList list = new ArrayList();
	while(rs2.next()){
		kehu=rs2.getString("kehu");
		ftbl = rs2.getString("ftbl");
		ftid=rs2.getString("id");
	...
	for(int i = 0;i<list.size();i++){
		...
		qq=(String)list.get(0);
		ww=(String)list.get(1);
		int ee=Integer.parseInt((String)list.get(2));
		rr=(String)list.get(3);
		aa=(String)list.get(4);
		int ss=Integer.parseInt((String)list.get(5));
		zz=(String)list.get(6);
		xx=(String)list.get(7);
		int cc=Integer.parseInt((String)list.get(8));
		session.setAttribute("ids",cc);
		session.setAttribute("id", ee);
		session.setAttribute("idd", ss);
%>
<html>
<head>
<title>
	...
	function ShowHideSearchRegion(trObject,SelfObject)
{
		...
		else
		{
			...
			function saveAccount(){
				var a= parseInt(document.form1.ftbl.value);
				var b = parseInt(document.form1.ftblq.value);
				var c = parseInt(document.form1.ftblw.value);
				var d=a+b+c;
				var f = a+b;
				var ff = a+c;
				var fff = b+c;		
	     	   
	     	   if(d=="100"|f=="100"|ff=="100"|fff=="100"|a=="100"|b=="100"|c=="100"){
	     	   	addzhandian();
	     	   }else{
	     	   	alert("分摊比例总和必须是100");	     	   	     	   	                                   
   }
        	...
        	function addzhandian(){
        		if(confirm("您将要修改信息！确认信息正确！")){
          		...
          		document.form1.action=path+"/servlet/DFDLservlet?action=updateftbl";
...
<form action="" name="form1" method="POST">
	...
	<tr class="form_label">
	  <td height="8%" bgcolor="#DDDDDD" width="15%"><div align="left">站点名称：</div>
         ...
         <td width="25%"><input type="text" name="zdid"  value=<%=jzname %> readonly="true"  style="width:130px" title="选填项，站点所在的具体物理位置 "/></td>
	 <td height="8%" bgcolor="#DDDDDD" width="15%"><div align="left">电表编码：</div>
         </td>
         <td width="25%"><input type="text" name="dbbm"  value=<%=dbbm %> readonly="true"  style="width:130px" title="选填项，站点所在的具体物理位置 "/></td>
	 </tr>
	 <tr>
	 <td height="8%" bgcolor="#DDDDDD" width="15%"><div align="left">客户：</div>
         </td>
         <td width="25%"><input type="text" name="kehu" value=<%=qq %> readonly="true"  style="width:130px" title="选填项，站点所在的具体物理位置 "/></td>
	 <td height="8%" bgcolor="#DDDDDD" width="15%"><div align="left">分摊比例：</div>
         </td>
         <td width="25%"><input type="text" name="ftbl" value=<%=ww %>   style="width:130px" title="选填项，站点所在的具体物理位置 "/></td>
	</tr>
	<tr>
	<td height="8%" bgcolor="#DDDDDD" width="15%"><div align="left">客户：</div>
         </td>
         <td width="25%"><input type="text" name="kehuq" value=<%=rr %> readonly="true"  style="width:130px" title="选填项，站点所在的具体物理位置 "/></td>
	 <td height="8%" bgcolor="#DDDDDD" width="15%"><div align="left">分摊比例：</div>
         </td>
         <td width="25%"><input type="text" name="ftblq" value=<%=aa %>  style="width:130px" title="选填项，站点所在的具体物理位置 "/></td>
	</tr>
	<tr>
	<td height="8%" bgcolor="#DDDDDD" width="15%"><div align="left">客户：</div>
         </td>
         <td width="25%"><input type="text" name="kehuw" value=<%=zz %> readonly="true"  style="width:130px" title="选填项，站点所在的具体物理位置 "/></td>
	 <td height="8%" bgcolor="#DDDDDD" width="15%"><div align="left">分摊比例：</div>
         </td>
         <td width="25%"><input type="text" name="ftblw" value=<%=xx %>  style="width:130px" title="选填项，站点所在的具体物理位置 "/></td>
	</tr>
<tr class="form_label">
        <td colspan="6">
