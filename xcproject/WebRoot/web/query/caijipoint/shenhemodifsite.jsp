...
<%
	...
	System.out.println("=====wo zai zhe li:"+id);
	...
	String shi2=request.getParameter("shi1"); //站点审核传过来的条件  再传回去
	String xian2=request.getParameter("xian1");
	String xiaoqu2=request.getParameter("xiaoqu1");
	String sname2=request.getParameter("sname1");
	String szdcode2=request.getParameter("szdcode1");
	String stationtype2=request.getParameter("stationtype1");
	String jzproperty2=request.getParameter("jzproperty1");
	String jztype2=request.getParameter("jztype1");
	String lrrq2=request.getParameter("lrrq1");
          		  					...
          		  					if( isNaN(document.form1.zpslzd.value)==false&&
          		 		 				 ...
          		 		 				 if(reg.exec(stime)||stime==null||stime==""){
			    								...
			    								alert("初始使用时间，请输入正确的日期格式");
													...
													document.form1.action=path+"/servlet/zhandian?action=shenhemodifySite"
...
<body  class="body" style="overflow-x:hidden;">
	<form action="" name="form1" method="POST" id="form">
		...
		<tr class="selected_font">
         ...
         <td width="25%">
         <select name="shi" class="selected_font" onchange="changeCity()" disabled="true">
         ...
         <td width="25%">
         <select name="xian" class="selected_font" onchange="changeCountry()" disabled="true">
         		<option value="0">请选择</option>
         		<%
         ...
         <td width="25%">
         <select name="xiaoqu" id="xiaoqu" class="selected_font" disabled="true" >
         		<option value="0">请选择</option>
         		<%
      ...
      <tr class="form_label">
         ...
         <td width="25%">
         	<select name="jzproperty" class="selected_font" onchange="JZtypes()" title="必填项，根据下拉值选择填写" disabled="true">
         		<option value="0">请选择</option>
         		<%
         ...
         <td width="25%">
         	<select name="jztype" class="selected_font"  onchange="kzinfo()" title="必填项，根据下拉值选择填写，相应类型生成对应报表，包括：IDC机房 对应报表为 IDC用电量汇总表和IDC机房节能技术应用情况汇总表，基站 对应报表为 地市基站用电量信息汇总表和基站用电量汇总分析表，通信机房 对应的报表为 通信局房用电量汇总表和通信机房环境温度管理节能情况汇总表，接入网 对应的报表为 地市接入网机房用电量信息汇总" disabled="true">
         	<option value="0">请选择</option>
         		<%
         ...
         <td width="25%">
         	<select name="stationtype" class="selected_font" onchange="kzinfo()" title="必填项，根据下拉值选择填写" disabled="true">
         		<option value="0">请选择</option>
         		<%
		...
		<tr class="form_label">
         ...
         <td width="25%"><input type="text" name="jzname" value="<%=form.getJzname() %>" class="selected_font" disabled="true" title="必填项,根据站点所在建筑物名称填写 例如：软件科技园D座楼"/></td>
          <td height="8%" bgcolor="#DDDDDD" width="15%"><div align="left">站点别名：</div></td>
         <td width="25%"><input type="text" name="bieming" value="<%=form.getBieming() %>" class="selected_font" disabled="true" title="选填项，根据站点所在建筑物名称自定义填写"/></td>				     
        <td height="8%" bgcolor="#DDDDDD" width="15%"><div align="left">归属方：</div>
				         </td>
				         <td width="25%">
				         	<select name="gsf" class="selected_font" title="必填项，根据站点的支付方确定选择下拉填写，例如：付费方不经报账有地市自行缴纳费用 则为 市本部，其他 无此种类似情况，则视为标准即可。" disabled="true">
					         		<option value="0">请选择</option>
					         		<%
       ...
       <tr class="form_label">
         ...
         <td width="25%">
         <select name="bgsign" class="selected_font" title="选填项，是否设置此站点为标杆站" disabled="true">
         ...
         <td width="25%">
         	<select name="jnglmk" class="selected_font" title="选填项，是否节能设备" disabled="true">
         		<option value="0">否</option>
         		<option value="1">是</option>
        ...
        <td height="8%" bgcolor="#DDDDDD" width="15%"><div align="left">地址：</div>
         ...
         <td width="25%">
         <input type="text" name="address" value="<%=form.getAddress() %>" class="selected_font"  title="选填项，站点所在的具体物理位置" disabled="true"/>
         </td>
      </tr>
      ...
      <tr class="form_label">
         ...
         <td width="25%">
         	<select name="gdfs" class="selected_font" title="必填项，根据下拉值选择填写" disabled="true">
         		<option value="0">请选择</option>
         		<%
         ...
         <td width="26%">  
         		<select name="yflx" class="selected_font" onchange="JZtypes()" title="根据下拉值选择填写，用房类型包括：砖混、彩钢板+直流负荷 来判断基站或接入网的22个基站类型和接入网类型" disabled="true">
         		<option value="0">请选择</option>
         		<%
         ...
         <td width="25%">
         	<input type="text" name="area" value="<%=form.getArea() %>" class="selected_font" title="选填项，根据站点所在地的具体面积填写 单位：平方米" disabled="true"/>
       ...
       <tr class="form_label">
         ...
         <td width="25%">
         	<input type="text" name="fuzeren" value="" class="selected_font" title="选填项，站点具体负责的人员" disabled="true"/>
         </td>
         <td height="8%" bgcolor="#DDDDDD" width="15%"><div align="left">单价：</div>
         </td>
         <td width="25%">
         	<input type="text" name="danjia" value="<%=form.getDanjia() %>" class="selected_font" title="必填项，根据站点的单价填写" disabled="true"/>
         </td>
        <td colspan="2">
          ...
          <table width="100%">
              <tr>
               <td bgcolor="#DDDDDD" width="47%" class="form_label">
		         ...
		         <td>  
		         		<select name="jzxlx" class="selected_font" onchange="JZtypes()" disabled="true">
		         		<option value="0">请选择</option>
		         		<%
       ...
       <tr class="form_label">
      	<td height="8%" bgcolor="#DDDDDD" width="15%"><div align="left">站点产权：</div>
				         ...
				         <td width="25%">
				         	<select name="zdcq" class="selected_font" disabled="true">
					         		<option value="0">请选择</option>
					         		<%
				             ...
				             <td width="25%">
				         	<select name="gxxx" style="width:130" disabled="true">
					         		<option value="0">请选择</option>
					         		<%
         ...
         <table width="100%">
            <tr class="form_label">
               <td height="8%" bgcolor="#DDDDDD" width="50%">
		         ...
		         <td>  
		         		<select name="jflx" class="selected_font" disabled="true">
		         		<option value="0">请选择</option>
		         		<%
          ...
          <table width="100%">
            <tr class="form_label">
               <td bgcolor="#DDDDDD" width="47%">
		         ...
		         <td>  
		         		<select name="jrwtype" class="selected_font" onchange="JZtypes()" disabled="true">
		         		<option value="0">请选择</option>
		         		<%
       ...
       <tr class="form_label">
         ...
         <td width="25%">
         <input type="text" name="edhdl" value="<%=form.getEdhdl() %>" class="selected_font" title="选填项，填写之后，市级二级审核，会根据审核条件“本次电量上下浮动超过站点额定耗电量计算值的XX%”进行判断，判断通不过会进入市级二级审核" disabled="true"/>
         </td>
         <td height="8%" bgcolor="#DDDDDD" width="15%"><div align="left">交流负荷（A）：</div>
         </td>
         <td width="25%"><input type="text" name="jlfh" value="" class="selected_font" disabled="true"/></td>
          <td height="8%" bgcolor="#DDDDDD" width="15%"><div align="left">直流负荷（A）：</div> </td>
		 <td width="25%">
		 <input type="text" name="zlfh" value="<%=form.getZlfh() %>" onchange="JZtypes()" disabled="true" class="selected_font" title="直流负荷+用房类型确定此站点的基站类型（22个类型）或者接入网（12个类型）"/>
		</td>
	 ...
	 <tr class="form_label">
         ...
         <td width="25%"><input type="text" name="sydate" value="<%=form.getSydate() %>" disabled="true" style="width:130px" onFocus="getDateString(this,oCalendarChs)"/></td>
	 	 <td height="8%" bgcolor="#DDDDDD" width="15%">楼宇交换机个数：</td>
         <td width="25%"><input type="text" name="lyjhjgs" value="<%= form.getLyjhjgs()%>" style="width:130px" disabled="true"/></td>
	 	<td height="8%" bgcolor="#DDDDDD" width="15%"><div align="left">站点启用状态：</div></td>
         <td width="25%">
         	    <select name="qyzt" style="width:130" disabled="true">
         		<option value="1">是</option>
         		<option value="0">否</option>
         	   </select>
          </td>
	  ...
	  <tr class="form_label">
        		...
        		<td width="25%">
          			<input type="text" name="jskssj" value="<%= form.getJskssj()%>" style="width:130px" disabled="true"/>
         		</td>
         		<td height="8%" bgcolor="#DDDDDD" width="15%"><div align="left">建设结束时间：</div></td>
        		<td width="25%">
          			<input type="text" name="jsjssj" value="<%= form.getJsjssj()%>" style="width:130px" disabled="true"/>
         		</td>
				<td height="8%" bgcolor="#DDDDDD" width="15%">项目号</td>
				<td width="25%">
          			<input type="text" name="xmh" value="<%= form.getXmh()%>" style="width:130px" disabled="true"/>
         		</td>
			</tr>
			<tr class="form_label">
				<td height="8%" bgcolor="#DDDDDD" width="15%"><div align="left">远供电：</div></td>
                <td width="25%">
         	    	<select name="ygd" style="width:130" disabled="true">
         	    		<option value="0">否</option>
         				<option value="1">是</option>
                ...
                <td width="25%">
         	    	<select name="ysd" style="width:130" disabled="true">
         	    		<option value="0">否</option>
         				<option value="1">是</option>
      ...
      <tr class="form_label">
        <td height="8%" bgcolor="#DDDDDD" width="15%"><div align="left">生产(%)：</div>
         ...
         <td width="25%"><input type="text" name="sc" value="<%=sc1 %>" style="width:130px" disabled="true"/></td>
         <td height="8%" bgcolor="#DDDDDD" width="15%"><div align="left">办公(%)：</div>
         </td>
         <td width="25%"><input type="text" name="bg" value="<%=bg1 %>" style="width:130px" disabled="true"/></td>
         <td height="8%" bgcolor="#DDDDDD" width="15%"><div align="left">建设投资(%)：</div>
         </td>
         <td width="25%"><input type="text" name="jstz" value="<%=jstz1 %>" style="width:130px" disabled="true"/></td>
      </tr>
      <tr class="form_label">
      	
         <td height="8%" bgcolor="#DDDDDD" width="15%"><div align="left">营业(%)：</div>
         </td>
         <td width="25%"><input type="text" name="yy" value="<%=yy1 %>" style="width:130px" disabled="true"/></td>
         <td height="8%" bgcolor="#DDDDDD" width="15%"><div align="left">信息化支撑(%)：</div>
         </td>
         <td width="25%"><input type="text" name="xxhzc" value="<%=xxhzc1 %>" style="width:130px" disabled="true"/></td>
          <td height="8%" bgcolor="#DDDDDD" width="15%"><div align="left">代垫电费(%)：</div>
         </td>
         <td width="25%"><input type="text" name="dddf" value="<%=dddf1 %>" style="width:130px" disabled="true"/></td>
	  ...
	  <tr bgcolor="F9F9F9" class="form_label">
       ...
       <tr class="form_label">
         ...
         <td width="25%">
         	<select name="dfzflx" class="selected_font" title="必填项，根据电费支付类型在下拉值中进行选择，其中月结 表示“普通后付费站点的电表” 插卡、预支、合同 表示“预付费的三种类型”" disabled="true">
         		<option value="0">请选择</option>
         		<%
         ...
         <td width="25%">
         	<select name="fkfs" class="selected_font" disabled="true">
         		<option value="0">请选择</option>
         		<%
         ...
         <td width="26%">
         		<select name="fkzq" class="selected_font" title="必填项，根据下拉值来选择付款的周期" disabled="true">
         		<option value="0">请选择</option>
         		<%
         ...
         <td width="25%">
         	...
         	<input type="text" name="zbdyhh" value="<%=form.getZbdyhh() !=null ? form.getZbdyhh():"" %>"   class="selected_font" disabled/>
         </td>
         ...
         <td width="25%">
         	<select name="watchcost" class="selected_font" disabled="true">
         	    <option value="0">请选择</option>
         ...
         <td width="25%"><input type="text" name="beilv" value="<%=form.getBeilv() !=null ? form.getBeilv():""%>"  class="selected_font" title="必填项，根据电表的实际情况进行填写" disabled="true"/>
         </td>
         </tr>
         ...
         <td width="25%">
         	<select name="linelosstype" class="selected_font" onchange="xsl()" disabled="true">
         		<option value="0">请选择</option>
         		<%
         ...
         <td width="25%"><input type="text" name="linelossvalue" value="<%=form.getLinelossvalue() !=null ? form.getLinelossvalue():""%>"   class="selected_font" disabled="true"/></td>
     
         <td height="8%" bgcolor="#DDDDDD" width="15%"><div align="left">电表用途：</div>
         </td>
         <td width="25%">
        
         <select name="dbyt" class="selected_font" title="必填项，根据电表的用途在下拉值中进行选择，其中 管理 表示“不涉及费用，只做抄表的站点电表” 结算 表示“涉及费用支出的站点电表”" disabled="true">
       ...
       <tr class="form_label">
         ...
         <td width="26%">
         		<input type="text" name="csds" value="<%=form.getCsds()!=null ? form.getCsds():"0" %>"  class="form" style="width:130px" title="必填项，初始的电表读数" disabled="true"/>
         </td>
       <td height="8%" bgcolor="#DDDDDD" width="15%"><div align="left">初始使用时间：</div>
         </td>
         <td width="25%">
         <input type="text" name="cssytime" value="<%=form.getCssytime()!=null ? form.getCssytime():"" %>" style="width:130px"  title="必填项，初始的电表使用时间" disabled="true"/>
         </td>
       <td height="8%" bgcolor="#DDDDDD" width="15%"><div align="left">原电表ID：</div>
         </td>
         <td width="26%">
         		<input type="text" name="ydbid" value="<%=form.getYdbid()!=null ? form.getYdbid():"" %>"  class="form" style="width:130px" disabled="true"/>
         </td>
      </tr>
          ...
          -->
			...
			<tr class="form_label">
			    <td height="8%" bgcolor="#DDDDDD" width="15%"><div align="left">地域属性：</div>
         		...
         		<td width="25%">
         		<select name="dytype" class="selected_font" disabled="true">
         		<option value="0">请选择</option>
         		<%
        		 ...
        		 <td width="25%">
         	  	 <select name="g2" class="selected_font" disabled="true">
         	 ...
         	 <td width="25%">
         	 <select name="g3" class="selected_font" disabled="true">
         		<option value="0">无</option>
         		<option value="1">有</option>
			...
			<tr class="form_label">
				<td height="8%" bgcolor="#DDDDDD" width="15%"><div align="left">宽带设备：</div>
        		 ...
        		 <td width="25%">
         	  	 <select name="kdsb" class="selected_font" disabled="true">
         		 <option value="0">无</option>
         		 <option value="1">有</option>
         	 ...
         	 <td width="25%">
         	 <select name="yysb" class="selected_font" disabled="true">
         		<option value="0">无</option>
         		<option value="1">有</option>
			...
			<td height="8%" bgcolor="#DDDDDD" width="15%"><div align="left">载频数量：</div>
        	 ...
        	 <td width="25%"><input type="text" name="zpslzd" value="<%= form.getZpslzd()%>" class="selected_font" disabled="true"/></td>
			
			</tr>
			<tr class="form_label">
				<td height="8%" bgcolor="#DDDDDD" width="15%"><div align="left">载扇数量：</div>
         		 </td>
        		 <td width="25%"><input type="text" name="zssl" value="<%= form.getZssl()%>" class="selected_font" disabled="true"/></td>
        		 <td height="8%" bgcolor="#DDDDDD" width="15%"><div align="left">宽带设备数量：</div>
         		 </td>
        		 <td width="25%"><input type="text" name="kdsbsl" value="" class="selected_font" disabled="true"/></td>
        		 <td height="8%" bgcolor="#DDDDDD" width="15%"><div align="left">语音设备数量：</div>
         		 </td>
        		 <td width="25%"><input type="text" name="yysbsl" value="" class="selected_font" disabled="true" /></td>
			...
			<tr class="form_label">
				<td height="8%" bgcolor="#DDDDDD" width="15%"><div align="left">空调1：</div>
        		 ...
        		 <td width="25%">
         	  	 <select name="kt1" class="selected_font" disabled="true">
         		 <option value="0">无</option>
         		 <option value="1">有</option>
         		 ...
         		 <td width="25%">
         		 <select name="kt2" class="selected_font" disabled="true">
         		 <option value="0">无</option>
         		 <option value="1">有</option>
                 ...
                 <td height="8%" bgcolor="#DDDDDD" width="15%"><div align="left">直供电：</div>
         		 ...
         		 <td width="25%">
         		 <select name="zgd" class="selected_font" disabled="true">
         		 <option value="0">否</option>
         		 <option value="1">是</option>
      	...
      	<td colspan=6>
      		<div align="center" id="JZKZ"  style="display:none;">
               ...
               <div style="width:15px;display:inline"></div><div style="width:50px;display:inline;"><hr></div>&nbsp;基站扩展信息&nbsp;<div style="width:300px;display:inline;"><hr></div>
      				...
      				<tr class="form_label">
      					<td>共站情况：<input type="text" name="gzqk" value="<%=form.getGzqk()%>" class="selected_font" disabled="true"></td>
      					<td>能耗占比：<input type="text" name="nhzb" value="<%=form.getNhzb()%>" class="selected_font" disabled="true"/></td>
      					<td>载频数量：<input type="text" name="zpsl" value="<%=form.getZpsl()%>" class="selected_font" disabled="true"/></td>
      				<td >逻辑站数：<input type="text" name="ljzs" value="<%=form.getLjzs()%>" class="selected_font" disabled="true"/></td>
    ...
    <tr class="form_label" >
     <!--     <td colspan="8">
		<div id="cancelBtnaq" style="width:60px;height:23px;cursor:pointer;float:right;position:relative;right:26px">
			 <img src="<%=path %>/images/baocun.png" width="100%" height="100%">
			 
			 <span style="font-size:12px;position: absolute;left:27px;top:5px;color:white" onclick="javascript:window.history.go(-1);"></span>      
		</div>
        </td>
        -->
      </tr>
...
<input type="hidden" name="shi2" value="<%=shi2%>"/>
<input type="hidden" name="xian2" value="<%=xian2%>"/>
<input type="hidden" name="xiaoqu2" value="<%=xiaoqu2%>"/>
<input type="hidden" name="szdcode2" value="<%=szdcode2%>"/>
<input type="hidden" name="jzproperty2" value="<%=jzproperty2%>"/>
<input type="hidden" name="jztype2" value="<%=jztype2%>"/>
<input type="hidden" name="lrrq2" value="<%=lrrq2%>"/>
<input type="hidden" name="sname2" value="<%=sname2%>"/>
<input type="hidden" name="stationtype2" value="<%=stationtype2%>"/>
</form>
</body>
</html>
