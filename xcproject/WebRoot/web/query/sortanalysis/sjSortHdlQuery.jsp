...
function ShowHideSearchRegion(trObject,SelfObject)
{
   ...
   function queryDegree(){
                   ...
                   document.form1.action=path+"/web/query/sortanalysis/sjSortHdlQuery.jsp"+canshuStr;
...
<tr bgcolor="F9F9F9">
         ...
         &nbsp;&nbsp;&nbsp;
	       ...
	       &nbsp;&nbsp;&nbsp;&nbsp;
	          &nbsp;&nbsp;&nbsp;
          排名：<select name="sort" style="width:130" onchange="javascript:document.form1.sort.value=document.form1.sort.value">
...
<tr bgcolor="F9F9F9">
  ...
  <table width="100%" border="0" cellspacing="1" cellpadding="1" bgcolor="#cbd5de">
           <tr>
             ...
             <div align="center" class="bttcn">排名</div>
             </td>
             <td width="7%" height="23" bgcolor="#888888">
             <div align="center" class="bttcn">省份</div>
             </td>
                                <td width="7%" height="23" bgcolor="#888888">                              
       ...
       <%
         ...
         String whereStr = "";
			...
			if(beginTimeQ != null && !beginTimeQ.equals("")){
				whereStr=whereStr+" and to_char(a.startmonth,'yyyy-mm')='"+beginTimeQ+"'";
         ...
         SortAnalysisBean bean = new SortAnalysisBean();
       	 ...
       	 fylist = bean.getPageDatahdl(curpage,whereStr,sort);
        ...
        int j=1;
        if(fylist.size()>7){
          for(int i=7;i<fylist.size();i++){   
	           if(j%6==0){
	           paiming.append(fylist.get(i)+";");
	           }
	           else{
	            paiming.append(fylist.get(i)+",");
	           }
		 ...
		 if(fylist!=null)
		{
		    ...
		    shi = (String)fylist.get(k+fylist.indexOf("SHINAME"));
		    shi = shi != null ? shi : "";
		    
		    sum_hj = (String)fylist.get(k+fylist.indexOf("DL"));
		    sum_hj = sum_hj != null ? sum_hj : "";
         
           sumdl+=Double.parseDouble(sum_hj);
			String color=null;
       ...
       <tr bgcolor="<%=color%>">   
           <td>
       		...
       		<td>
       			<div align="center" ><%=beginTimeQ%></div>
       		</td>
       		<td>
  	 ...
  	 <% if(fylist.size()>7) {%>
  	  <img src="<%=path%>/servlet/ShengjihdlpaimingServlet?fylist=<%=URLEncoder.encode(paiming.toString(),"utf-8") %>"/>
       <%}%>
...
<script language="javascript">
   document.form1.sort.value = "<%=request.getParameter("sort") %>";
	var path = '<%=path%>';
	function gopage()
     {
      document.form1.submit();
     ...
     function previouspage()
     {
      ...
      else
      {
        ...
        var curpage='<%=(curpage-1)%>';
      		document.form1.action=path+"/web/query/sortanalysis/sjSortHdlQuery.jsp?curpage="+curpage;
        document.form1.submit();
      }
     }
     function nextpage()
     ...
     function gopagebyno(pageno)
     {
      document.form1.page.value = pageno;
      		...
      		document.form1.action=path+"/web/query/sortanalysis/sjSortHdlQuery.jsp?curpage="+pageno;
      document.form1.submit();
     }
...
function exportad(){
            ...
            //alert(canshuStr);
        	document.form1.action=path+"/web/query/sortanalysis/省级耗电量（总）排名.jsp?curpage="+curpage+"&whereStr="+whereStr;
            document.form1.submit();
   }
   function printad(){
            var curpage = <%=curpage%>;
            var whereStr ="<%=whereStr%>";
            //alert(canshuStr);
            window.open(path+"/web/query/sortanalysis/sjSortHdlQuery_print.jsp?curpage="+curpage+"&whereStr="+whereStr,'','width=800,height=500,status=yes,scrollbars=yes,resizable=yes,left=200,top=200')
