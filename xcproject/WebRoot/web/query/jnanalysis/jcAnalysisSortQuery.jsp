...
function ShowHideSearchRegion(trObject,SelfObject)
{
   ...
   function queryDegree(){
                   ...
                   var sort = document.form1.sort2.value;
                   var begintime = document.form1.BeginTime.value;
                   var endtime = document.form1.EndTime.value;
                   //alert(sort);
                   var canshuStr = "?shi="+shi+"&sort="+sort+"&begintime="+begintime+"&endtime="+endtime; 

                   //alert(canshuStr);
                   document.form1.action=path+"/web/query/jnanalysis/jcAnalysisSortQuery.jsp"+canshuStr;
...
<%
permissionRights=commBean.getPermissionRight(roleId,"1802");
...
<tr bgcolor="F9F9F9">
  		 ...
  		 <td width="1000">
           &nbsp;&nbsp;&nbsp;
            起始月份：<input type="text" name="BeginTime" value="<%if(null!=request.getParameter("BeginTime")) out.print(request.getParameter("BeginTime")); %>" onFocus="getDatenyString(this,oCalendarChsny)" class="form" style="width: 130px;"/>
           &nbsp;&nbsp;&nbsp;
            结束月份：<input type="text" name="EndTime" value="<%if(null!=request.getParameter("EndTime")) out.print(request.getParameter("EndTime")); %>" onFocus="getDateString(this,oCalendarChsny)"  class="form" style="width: 130px;"/>
          &nbsp;&nbsp;&nbsp;
...
<tr bgcolor="F9F9F9">
  ...
  <table width="100%" border="0" cellspacing="1" cellpadding="1" bgcolor="#cbd5de">
           <tr>
                                 <td nowrap height="23" bgcolor="#888888"><div align="center" class="bttcn">序号</div>
            			        </td> 
                                 <td nowrap height="23" bgcolor="#888888"><div align="center" class="bttcn">地市</div>
            			        </td>                             
                                <td nowrap height="23" bgcolor="#888888"><div align="center" class="bttcn">站点数合计</div>
            			        </td>
            			         <td nowrap height="23" bgcolor="#888888"><div align="center" class="bttcn">耗电量合计</div>                                                        
            			        </td>
       ...
       <%
           ...
           if(begintime != null && !begintime.equals("") && !begintime.equals("0")){
			...
			if(begintime != null && !begintime.equals("") && !begintime.equals("0")){
				whereStr=whereStr+" and to_char(ad.endmonth,'yyyy-mm')>='"+begintime+"'";
			}
         ...
         JcAnalysisBean bean = new JcAnalysisBean();
       	 ...
       	 fylist = bean.getPageDataSort(curpage,whereStr,sort);
		...
		zdnum_jn= ""; 
		int sumzd=0;   
		Double dl=0.0,sumdl=0.0;
		    
        int ii=1;
        for(int i=5;i<fylist.size();i++)
        {
            paiming.append((fylist.get(i).equals("")?"0":fylist.get(i))+",");
            
        }
        System.out.println("====="+paiming.toString());
        System.out.println(paiming.toString()+"*****");
		 ...
		 if(fylist!=null)
		{
            ...
            zdnum_jn = (String)fylist.get(k+fylist.indexOf("HDL_SUM"));	
            zdnum_jn = zdnum_jn != null ? zdnum_jn : "";
            
	         sumzd+=Integer.parseInt(zdnum_sum);
	         if(zdnum_jn=="")zdnum_jn="0";
	        sumdl+=Double.parseDouble(zdnum_jn);
	        System.out.println("--"+zdnum_sum+"*"+zdnum_jn+"*");
	        dl=Double.parseDouble(zdnum_jn);
	        DecimalFormat mat=new DecimalFormat("0.0");
	        zdnum_jn=mat.format(dl);
	        sum_dl=mat.format(sumdl);
			String color = "";
	      if(intnum%2==0){
			    color="#DDDDDD";
       ...
       <%}%>
  	  ...
  	  <img src="<%=path%>/servlet/YongdianpaimingServlet?fylist=<%=URLEncoder.encode(paiming.toString(),"utf-8") %>"/>
	...
	function gopage()
     ...
     function previouspage()
     {
      ...
      else
      {
        ...
        var curpage='<%=(curpage-1)%>';
      		document.form1.action=path+"/web/query/jnanalysis/jcAnalysisSortQuery.jsp?curpage="+curpage;
        document.form1.submit();
      }
     }
     function nextpage()
     ...
     function gopagebyno(pageno)
     {
      document.form1.page.value = pageno;
      		...
      		document.form1.action=path+"/web/query/jnanalysis/jcAnalysisSortQuery.jsp?curpage="+pageno;
      document.form1.submit();
     }
...
function exportad(){
            ...
            var sort ="<%=sort%>";
            //alert(canshuStr);
        	document.form1.action=path+"/web/query/jnanalysis/jcAnalysisSortQuery_xml.jsp?curpage="+curpage+"&whereStr="+whereStr+"&sort"+sort;
            document.form1.submit();
   }
   function printad(){
            var curpage = <%=curpage%>;
            ...
            var sort ="<%=sort%>";
            //alert(canshuStr);
            window.open(path+"/web/query/jnanalysis/jcAnalysisSortQuery_print.jsp?curpage="+curpage+"&whereStr="+whereStr+"&sort="+sort,'','width=800,height=500,status=yes,scrollbars=yes,resizable=yes,left=200,top=200')
