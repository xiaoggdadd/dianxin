...
.relativeTag   {     
  ...
  <table border="0" width="100%" cellspacing="1" cellpadding="1" bgcolor="#cbd5de" class="form_label">
     <tr height = "10px" class= "relativeTag">
       <td width="4%" height="23" bgcolor="#DDDDDD"><div align="center" style="font-weight:bold">序号</div></td>
         <td width="10%" height="23" bgcolor="#DDDDDD"><div align="center" style="font-weight:bold">报账月份</div></td>
          <td width="12%" height="23" bgcolor="#DDDDDD"><div align="center" style="font-weight:bold">上次抄表时间</div></td>
           <td width="12%" height="23" bgcolor="#DDDDDD"><div align="center" style="font-weight:bold">本次抄表时间</div></td>
            <td width="4%" height="23" bgcolor="#DDDDDD"><div align="center" style="font-weight:bold">周期</div></td>
         <td width="10%" height="23" bgcolor="#DDDDDD"><div align="center" style="font-weight:bold">月耗电量</div></td>        
          <td width="10%" height="23" bgcolor="#DDDDDD"><div align="center" style="font-weight:bold">日均耗电量</div></td>
     <td width="10%" height="23" bgcolor="#DDDDDD"><div align="center" style="font-weight:bold">实际金额</div></td>
    ...
    -->
                ...
                //  for(int i=0;i<fylist.size();i++){
                   //   System.out.println(fylist.get(i).toString());
      ...
      <tr bgcolor="<%=color%>" class="form_label">
          <td><div align="center" ><%=intnum-1%></div></td> 
            ...
            <td><div align="right" ><%=ts%></div></td>
            <td><div align="right" ><%=hdl%></div></td>
             <td><div align="right" ><%=rhdl%></div></td> 
            <td><div align="right" ><%=m%></div></td> 
           
           <!--  <td><div align="left"  ><a href="javascript:lookDetails('<%=id%>')"><%=sname%></a></div></div></td> 
            <td><div align="right" ><a href="javascript:dfinfo9('<%=dbyt%>','<%=dbid%>','<%=bla%>','<%=yf%>','<%=bzmonth%>','<%=sjyf%>','<%=shia%>','<%=xiana%>','<%=xiaoqua%>','<%=bili%>')"><%=bili%>%</a></div></td>        
         -->
        </tr>   
        <%
       }%> 
...
<script language="javascript">
var path = '<%=path%>';   
    function modifyad(id,dbid,mon){
			 var c=path+"/web/tongjichaxun/collectQueryMap.jsp?id="+id+"&dbid="+dbid+"&mon="+mon+"&";			
			c = c.substring(0,b.length-1);
			 var a = " <a href="+c+" target='treeMap2' id='tmpTree'></a>";
			$("#tmpTree").remove();
			$("body").append(a);
			$("#tmpTree")[0].click();   
    } 
