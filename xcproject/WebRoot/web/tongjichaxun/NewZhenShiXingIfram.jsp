...
<%@page import="com.noki.tongjichaxu.ZhenShiXingDao"%>
...
<% 
	...
	String bzmonth = request.getParameter("bzmonth")!=null?request.getParameter("bzmonth"):"0";
	
	String property=request.getParameter("property")!=null?request.getParameter("property"):"0";
	String shi1=request.getParameter("shi1")!=null?request.getParameter("shi1"):"0";
 ...
 <script language="javascript">
  ...
  <body>
   <form action="" name="form1" method="POST">
    ...
    <table width="2000px" border="0" cellspacing="1" cellpadding="1">
           <tr class="relativeTag" >
             <td width="2%" bgcolor="#DDDDDD" class="form_label"><div align="center" class="bttcn">ID</div></td>
              <td  bgcolor="#DDDDDD" class="form_label" ><div align="center" class="bttcn">所在地市</div></td>
              <td  bgcolor="#DDDDDD" class="form_label" ><div align="center" class="bttcn">所在区县</div></td>
             ...
             <td  bgcolor="#DDDDDD" class="form_label" ><div align="center" class="bttcn">分摊</div></td>
              ...
              <td  bgcolor="#DDDDDD" class="form_label" ><div align="center" class="bttcn">超标站点</div></td>
               ...
               <td  bgcolor="#DDDDDD" class="form_label" ><div align="center" class="bttcn">实际电量和</div></td>
       ...
       <%
         ...
         if(shi1 != null && !shi1.equals("") && !shi1.equals("0")){
				//whereStr=whereStr+" AND zd.shi='"+shi1+"'";
				zsxstr = zsxstr+" AND da.agcode='"+shi1+"'";
		 }
          if(xian != null && !xian.equals("") && !xian.equals("0")){
        	  zsxstr = zsxstr+" AND d.agcode='"+xian+"'";
	 	 }
		 if(bzyf != null && !bzyf.equals("") && !bzyf.equals("0")&&bzmonth != null && !bzmonth.equals("") && !bzmonth.equals("0")){
		 ...
		 ZhenShiXingDao zsxd = new ZhenShiXingDao(); 
		
		List<cbJzBean> fylist = null;
		boolean b = false;
		if(bzw != null && !bzw.equals("") && !bzw.equals("0")){
        String str="";
        String shicode = "",xiancode="";
        String shiname="",xianname="",jzsxcount="0",scnhcount="0",cbgkcount="0",cbzdcount="0",lxcbcount="0",lxzzcbcount="0",jsdbcount="0",bzdlcount="0",szdlcount="0",bzpldu="0",waqzgcount="0",
               zsx="0",pldcount="0",cbjzzgcount="0",zf="0",zsxcount="0"; 
        String cbdlzz = "0";
        String xzsgsdldb = "0";
        //报账月份
        b = zsxd.getJcsjzsxHistory(bzyf,property);
       if(b){
       		fylist = zsxd.getJcsjzsxHistoryData(bzyf,loginId,property);
       }else{
           ...
           // JzxxDao dao = new JzxxDao();
            ...
            //JzxxDao dao = new JzxxDao();
       		...
       		fylist = zsxd.getData(whereStr,zsxstr,property,whereyf,whereyf01,whereyf02,month,pro,bzyf,loginId,loginName);
       }       
		  ...
		  if(fylist!=null){
			...
			request.getSession().setAttribute("fylist",fylist);
             ...
             xianname = beans.getXian();
             xiancode = beans.getXiancode();
            ...
            System.out.println("bzdl:"+bzdlcount);
       ...
       <tr bgcolor="<%=color%>" class="form_label">
       		...
       		<td>
       		<div align="center" id="2<%=intnum%>"><input id="shicode<%=intnum%>" type="hidden" value="<%=shicode%>"/><%=shiname%></div>
       		</td>
       		<td>
       		<div align="center" id="2<%=intnum%>"><input id="xiancode<%=intnum%>" type="hidden" value="<%=xiancode%>"/><%=xianname%></div>
       		</td>
       		<td>
       		<div align="center" id="3<%=intnum%>"><a href="javascript:tz('<%=shicode%>','<%=xiancode%>','<%=loginId%>')"><%=jzsxcount%></a></div>
       		</td>
       		<td>
       		<div align="center" id="4<%=intnum%>"><a href="javascript:sc('<%=shicode%>','<%=xiancode%>','<%=loginId%>','<%=bzyf%>')"><%=scnhcount%></a></div>
       		</td>
       		<td>
        	<div align="center" id="5<%=intnum%>"><a href="javascript:cbgk('<%=shicode%>','<%=xiancode%>','<%=bzyf%>','<%=property%>','<%=loginId%>')"><%=cbgkcount%></a></div>
       		</td>
       		<td>
       			<div align="center" id="6<%=intnum%>"><a href="javascript:cbzd('<%=shicode%>','<%=xiancode%>','<%=bzyf%>','<%=property%>','<%=cbbl%>','<%=loginId%>')"><%=cbzdcount%></a></div>
       		</td>
       		<td>
       		<div align="center" id="7<%=intnum%>"><a href="javascript:jsdb('<%=shicode%>','<%=xiancode%>','<%=bzyf%>','<%=property%>','<%=bzyf02%>','<%=cbbl%>','<%=cbblsyc%>','<%=loginId%>')"><%=jsdbcount%></a></div>
       		</td>
       		<td>
       		<div align="center" id="8<%=intnum%>"><a href="javascript:lxcb('<%=shicode%>','<%=xiancode%>','<%=bzyf%>','<%=property%>','<%=bzyf02%>','<%=cbbl%>','<%=cbblsyc%>','<%=loginId%>')"><%=lxcbcount%></a></div>
       		</td>
       		<td>
       		<div  align="center"id="9<%=intnum%>"><a href="javascript:lxcbzj('<%=shicode%>','<%=xiancode%>','<%=bzyf%>','<%=property%>','<%=bzyf02%>','<%=cbbl%>','<%=cbblsyc%>','<%=loginId%>')"><%=lxzzcbcount%></a></div>
       		</td>
       		<td>
       		...
       		<td>
       		<div align="center" id="16<%=intnum%>"><a href="javascript:waqzg('<%=shicode%>','<%=xiancode%>','<%=bzyf%>','<%=property%>','<%=cbbl%>','<%=loginId%>')"><%=waqzgcount%></a></div>
       		</td>
       		<td>
      ...
      <% }}else if(!(intnum > 266)){
    	  for(int i=((intnum-1)%266);i<266;i++){
            if(i%2==0)
			    color="#FFFFFF" ;
            else
			    color="#DDDDDD";
	...
	<script type="text/javascript">
		function tz(shicode,xiancode,loginId) {
			var path = '<%=path%>';
			window.open(path +  "/web/tongjichaxun/NewsxcwQuery.jsp?shiname="+shicode+"&xianname="+xiancode+"&loginId="+loginId,'','width=1200,height=500,status=yes,scrollbars=yes,resizable=yes,left=100,top=100')
		}
		function sc(shicode,xiancode,loginId,bzyf) {
			var path = '<%=path%>';
			window.open(path +  "/web/tongjichaxun/newScFenTan.jsp?shiname="+shicode+"&xianname="+xiancode+"&loginId="+loginId+"&bzyf="+bzyf,'','width=1200,height=500,status=yes,scrollbars=yes,resizable=yes,left=100,top=100')
		}
		function cbgk(shicode,xiancode,bzyf,property,loginId) {
			var path = '<%=path%>';
			window.open(path +  "/web/tongjichaxun/newCbgk.jsp?shiname="+shicode+"&xianname="+xiancode+"&bzyf="+bzyf+"&property="+property+"&loginId="+loginId,'','width=1200,height=500,status=yes,scrollbars=yes,resizable=yes,left=100,top=100')
		}
		function cbzd(shicode,xiancode,bzyf,property,cbbl,loginId) {
			var path = '<%=path%>';
			window.open(path +  "/web/tongjichaxun/newCbzd.jsp?shiname="+shicode+"&xianname="+xiancode+"&bzyf="+bzyf+"&property="+property+"&cbbl="+cbbl+"&loginId="+loginId,'','width=1200,height=500,status=yes,scrollbars=yes,resizable=yes,left=100,top=100')
		}
		function jsdb(shicode,xiancode,bzyf,property,bzyf02,cbbl,cbblsyc,loginId) {	
			var path = '<%=path%>';
			window.open(path +  "/web/tongjichaxun/newJsdb.jsp?shiname="+shicode+"&xianname="+xiancode+"&bzyf="+bzyf+"&property="+property+"&bzyf02="+bzyf02+"&cbbl="+cbbl+"&cbblsyc="+cbblsyc+"&loginId="+loginId,'','width=1200,height=500,status=yes,scrollbars=yes,resizable=yes,left=100,top=100')
		}
		function lxcb(shicode,xiancode,bzyf,property,bzyf02,cbbl,cbblsyc,loginId) {	
			var path = '<%=path%>';
			window.open(path +  "/web/tongjichaxun/newLxcb.jsp?shiname="+shicode+"&xianname="+xiancode+"&bzyf="+bzyf+"&property="+property+"&bzyf02="+bzyf02+"&cbbl="+cbbl+"&cbblsyc="+cbblsyc+"&loginId="+loginId,'','width=1200,height=500,status=yes,scrollbars=yes,resizable=yes,left=100,top=100')
		}
		function lxcbzj(shicode,xiancode,bzyf,property,bzyf02,cbbl,cbblsyc,loginId) {	
			var path = '<%=path%>';
			window.open(path +  "/web/tongjichaxun/newLxcbzj.jsp?shiname="+shicode+"&xianname="+xiancode+"&bzyf="+bzyf+"&property="+property+"&bzyf02="+bzyf02+"&cbbl="+cbbl+"&cbblsyc="+cbblsyc+"&loginId="+loginId,'','width=1200,height=500,status=yes,scrollbars=yes,resizable=yes,left=100,top=100')
		}
		function waqzg(shicode,xiancode,bzyf,property,cbbl,loginId) {
			var path = '<%=path%>';
			window.open(path +  "/web/tongjichaxun/newWaqzg.jsp?shiname="+shicode+"&xianname="+xiancode+"&bzyf="+bzyf+"&property="+property+"&cbbl="+cbbl+"&loginId="+loginId,'','width=1200,height=500,status=yes,scrollbars=yes,resizable=yes,left=100,top=100')
		}
	</script>
</html>
