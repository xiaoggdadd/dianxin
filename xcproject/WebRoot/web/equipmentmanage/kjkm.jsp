...
function getDictList(type,maxrow,minrow){
	...
	if (type==1){
	 Url = getPublicPath()+"/servlet/FinanceSubject?action=ajaxareaData";
	  params ="&fkmcode="+ document.getElementById("yijikm").value;
	}else if (type==2){
	  Url = getPublicPath()+"/servlet/FinanceSubject?action=ajaxareaData";
	  params ="&fkmcode="+ document.getElementById("erjikm").value;
	}else if (type==3){
		  Url = getPublicPath()+"/servlet/FinanceSubject?action=ajaxareaData";
	      params ="&fkmcode="+ document.getElementById("sanjikm").value;
	}else{
...
//选择项信息
   ...
   var fullname="";
    ...
    window.parent.form1.kjkm.value = fullname;
	window.parent.form1.kjkmcode.value = id;
	closeDiv();
  }
	function closeDiv(){
		var currentDiv = window.parent.$("#kjkmDiv");
		currentDiv.remove();
	}
//分页
