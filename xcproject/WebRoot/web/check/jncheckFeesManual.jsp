		...
		for ( var i = 0; i < l; i++) {
			...
			if (bzw1 == "1") {
				if (count1 <= r
					...
					document.form1.action = path + "/servlet/CountryElecServlet?command=checkcity6&chooseIdStr1=" + chooseIdStr1 
...
function lookDetailss1(dbid,dlid,electricfee_id,edhdl,qsdbdl,jszq){ 
      ...
      window.open(path+"/servlet/EnhanceCountryHeadServlet?action=xiangxi&shi1="+shi+"&xian1="+xian+"&xiaoqu1="+xiaoqu+"&bzyf1="+bzyf+"&lrrq1="+lrrq+"&dbid="+dbid+"&dlid="+dlid+"&electricfee_id="+electricfee_id+"&edhdl="+edhdl+"&qsdbdl="+qsdbdl+"&jszq="+jszq+"&sjtzbz=0")
}

function queryDegree(){
       document.form1.action=path+"/web/check/jncheckFeesManual.jsp?command=chaxun";
       document.form1.submit();
}
...
<body  class="body" style="overflow-x:hidden;">
	<form action="" name="form1" method="POST">
		<table width="100%"  border="0" cellspacing="0" cellpadding="0" height="18%">
			...
			<div style="width:700px;height:50px">
			       ...
			       <span style="font-size:14px;font-weight:bold;position: absolute;left:25px;top:15px;color:black">人工电费审核（加强功能）</span>	
			 </div>
	    	</td></tr>	    	
		...
		<p id="box3" style="display:none">
			<table>
				...
				<td>录入日期：</td><td><input type="text" name="lrrq" value="<%if(null!=request.getParameter("lrrq")) out.print(request.getParameter("lrrq")); %>" onFocus="getDateString(this,oCalendarChs)" onpropertychange="endtimezq()" class="selected_font"/></td>
			...
			<%	       
		        ...
		        ElectricFeesBean bean=new ElectricFeesBean();
					...
					if(beginTime1 != null && beginTime1 != "" && !beginTime1.equals("0")){
						whereStr=whereStr+" and ad.STARTMONTH ='"+beginTime1+"'";
						str=str+" and ad.STARTMONTH ='"+beginTime1+"'";
					}
					if(endTime1 != null && endTime1 != "" && !endTime1.equals("0")){
						whereStr=whereStr+" and ad.ENDMONTH ='"+endTime1+"'";
						str=str+" and ad.ENDMONTH ='"+endTime1+"'";
					}
					...
					if(manualauditstatus3 != null && !manualauditstatus3.equals("") && !manualauditstatus3.equals("0")){
					     ...
					     if(manualauditstatus3.equals("1")){
							 ...
							 str=str+" and (ef.manualauditstatus='"+manualauditstatus3+"' or ef.manualauditstatus='-1')";
					     }else{
  ...
  %>
		...
		<%		         
            ...
            fylist = bean.getPageDataCheckjn(whereStr,loginId,lrbz1,lrbz2);
           ...
           fylist=null;
				 ...
				 if(fylist!=null)
				{
					...
					for(ElectricFeesFormBean elecbean:fylist){
						...
						String dlid = elecbean.getDlid();
						String electricfee_id = elecbean.getElectricfeeid();
		       ...
		       <tr bgcolor="<%=color%>" title="<%=tishi%>">
		            ...
		            <td><div align="center"><%=quxian%></div></td>
		       		...
		       		<td>
		       			<div align="center">
		       				<a href="javascript:lookDetailss1('<%=dbid%>','<%=dlid%>','<%=electricfee_id%>','<%=edhdl%>','<%=qsdbdl%>','<%=jszq%>')"><%=tbtzsq%></a>
		       			</div>
		       		</td>
		       		<td><div align="right"><%=csbdfjdz%></div></td>
					<td><div align="right"><%=cshibdjdz%></div></td>
					<td><div align="right"><%=csbl%>%</div></td>
					<td><div align="right"><%=csbbl%>%</div></td>
					<td><div align="right"><%=csdbdfbz%></div></td>
					<td><div align="right"><%=cshidbdfbz%></div></td>
					<td><div align="right"><%=csbdld%></div></td>
					<td><div align="right"><%=cshibdld%></div></td>
					...
					<td><div align="center"><%=floatdegreeandbl%></div></td>
					<td><div align="center"><%=lastactualdegree%></div></td>
					<td><div align="center"><%=lastfloatdegreeandbl%></div></td>
					<td><div align="right"><%=lineandchangeandbl%></div></td>
					<td><div align="right"><%=bzrj%></div></td>
					<td><div align="right"><%=beilv%></div></td>
		      	 </tr>
...
var path = '<%=path%>';
            ...
            var whereStr ="<%=str%>";
            var lrrq='<%=lrrq%>';
        	document.form1.action=path+"/web/check/加强版人工电费审核信息.jsp?zdname="+zdname+"&whereStr="+whereStr+"&lrrq="+lrrq+"&command=daochu";
...
function passCheck1(chooseIdStr1, chooseIdStr2, manpassmemo) {//通过240
	manpassmemo = encodeURIComponent(manpassmemo);
	sendRequest1(path + "/servlet/CountryElecServlet?command=checkcity6&chooseIdStr1=" + chooseIdStr1 
				+ "&chooseIdStr2=" + chooseIdStr2 + "&manpassmemo=" + manpassmemo, "checkcity16");
}
...
function passCheckNo1(chooseIdStr1, chooseIdStr2) {//取消通过1
	sendRequest1(path + "/servlet/CountryElecServlet?command=checkcityno17&chooseIdStr1="
...
function passCheckNo2(chooseIdStr1, chooseIdStr2,cause) {//审核不通过1
	...
	sendRequest1(path + "/servlet/CountryElecServlet?command=checkcityno16&chooseIdStr1="
				...
				if (bzw1 == "1") {
					if (count1 <= r
						...
						document.form1.action = path
								+ "/servlet/CountryElecServlet?command=checkcityno6&chooseIdStr1="
				...
				for ( var i = 0; i < l; i++) {
					...
					if (bzw1 == "1") {
						if (count1 <= r
							...
							document.form1.action = path
									+ "/servlet/CountryElecServlet?command=checkcityno26&chooseIdStr1="
