...
var path = '<%=path%>';    
    function modifyad(xianid,shiid){
    	var startmonth ='<%=startmonth%>';
    	var endmonth ='<%=endmonth%>';
    	
    	var b=path+"/web/dfdf/dfdfchaxun02.jsp?bz=1&xianid="+xianid+"&shiid="+shiid+"&startmonth="+startmonth+"&endmonth="+endmonth+"&";			
...
<body  class="body" >
  ...
  <table width="100%" border="0" cellspacing="1" cellpadding="1" bgcolor="#cbd5de" class="form_label">
     ...
     <tr height = "20" class="relativeTag">
         <td width="10%" bgcolor="#DDDDDD"><div align="center" style="font-weight:bold;">地市</div></td>     
       ...
       String str="";
		...
		System.out.println("whereStr01111"+whereStr);
       ...
       <tr bgcolor="<%=color%>" class="form_label">
       		...
       		<td>
       			<div align="center"><a href="javascript:modifyad(<%=xianid%>,<%=shiid%>)" ><%=jsydl%></div>
       		</td>
       		<td>
              ...
              <input type="hidden" id="em" name="em" value="33"> 
  	          <input type="hidden" id="tt" name="tt" value="<%=whereStr %>"> 
  	          <input type="hidden" id="shit" name="shit" value="<%=shi %>"> 
       		</td>                      		
...
function exportad(){
	        var path = '<%=path%>';
            var whereSrc = document.getElementById("tt").value;
            var shi = document.getElementById("shit").value;
            //alert(path+"/web/query/basequery/dfdfExt.jsp?whereStr="+whereSrc+"&command=daochu");
        	document.form1.action=path+"/web/query/basequery/dfdfExt.jsp?whereStr="+whereSrc+"&command=daochu"+"&shi="+shi;
