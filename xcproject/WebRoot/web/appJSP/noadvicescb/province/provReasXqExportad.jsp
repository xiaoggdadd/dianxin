...
<%
    	...
    	String bzw = request.getParameter("bzwq")!=null?request.getParameter("bzwq"):"";
    	String whereStr="";
%>
...
<table x:str border=0 cellpadding=0 cellspacing=0 width=705 style='border-collapse:
 ...
 <tr class=xl24 height=19 style='height:14.25pt'>  
  ...
  <td class=xl25 width=165 style='border-left:none;width:124pt'>无建议生产标个数</td>
 </tr>
 <%   
		...
		ArrayList<NoAdvScbProReason> fylist = null;
		if("1".equals(bzw)){
			fylist = bean.getProCity1(whereStr,loginId);
		}else if("2".equals(bzw)){
			fylist = bean.getProCity2(whereStr,loginId);
		}else if("3".equals(bzw)){
			fylist = bean.getProCity3(whereStr,loginId);
		}
		String city = "",num = "0";
		double hj1 = 0;
		if(fylist!=null){
			for(NoAdvScbProReason bean1:fylist){
				city = bean1.getCity();
				num = bean1.getNum();
       %>
 <tr height=19 style='height:14.25pt'>  
  <td class=xl26 align=center style='border-top:none;border-left:none' x:num>&nbsp;<%=city%></td>  
  <td class=xl26 align=center style='border-top:none;border-left:none' x:num>&nbsp;<%=num%></td>
 </tr>
 <%
    }}
 %>
