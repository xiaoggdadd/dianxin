...
<script language="javascript">
	...
	function queryDegree(command) {
		...
		if(shi=='-1'){
			...
			document.form1.action = path+"/servlet/OneSiteDbShiServlet?command="+command;
	...
	function xiangqing1(xian,fwzbzw){
		//fwzbzw 1:一站多表  2:非外租回收一站多表
		var zdqyzt = '${state}';
		var dbqyzt = '${ammerterStyleId}';
		window.open(path+"/servlet/OneSiteDbShiServlet?command=xiangqing1&xian="+xian+"&zdqyzt="+zdqyzt+"&dbqyzt="+dbqyzt+"&fwzbzw="+fwzbzw);
	}
	 			...
	 			<tr>
      				<td colspan="4" width="50">
						<div style="width:700px;height:50px">
							...
							<span style="font-size:14px;font-weight:bold;position: absolute;left:25px;top:15px;color:black">一站多表站点市级查询</span>	
						</div>
					</td>
    			</tr>
				<tr>
				...
				<table width="100%" height="80%" border="0" cellspacing="1"
					...
					<c:forEach items="${emcsList}" var="list" varStatus="status">
						<tr bgcolor="${(status.count)%2==0?'#DDDDDD':'#FFFFFF'}">
							...
							<td>
								<div align="center">
									<a href="javascript:xiangqing1('${list.xiancode}','1')">${list.yzdb}</a>
								</div>
							</td>
							<td>
								<div align="center">
									<a href="javascript:xiangqing1('${list.xiancode}','2')">${list.fhsyzdb}</a>
								</div>
							</td>
							<td>
								<div align="center">
