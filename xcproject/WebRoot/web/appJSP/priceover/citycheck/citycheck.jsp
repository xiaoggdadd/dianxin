...
<%	
	...
	String shengzt =request.getParameter("shengzt")!=null?request.getParameter("shengzt"):"0";//省审核状态
    	...
    	if (document.getElementById("shi").value == ""
			...
			if(isNaN(bl1)|| isNaN(bl2)){
				alert("比例必须为数字!");
		 		...
		 		document.form1.action=path+"/servlet/CityCheckServlet?command=" + command;
						...
						<td><select name="state"  class="selected_font">
         				...
         				<td>报账月份：</td>
                  		...
                  		<td> <%if(permissionRights.indexOf("PERMISSION_SEARCH")>=0){%>
					             <div id="chaxun" style="position:relative;width:60px;height:23px;cursor: pointer;left:100%;float:right;top:0px;right:-340px">
		...
		<div style="width:100%;height:300px;overflow-x:auto;overflow-y:auto;border:1px">
			<table width="1500px" height="70%"  border="0" cellspacing="1" cellpadding="1" bgcolor="#cbd5de" class="form_label">
				<tr height = "23" class="relativeTag">
            			...
            			<td width="" height="23" bgcolor="#DDDDDD"><div align="center" class="bttcn">市审核状态</div></td>
            			<td width="" height="23" bgcolor="#DDDDDD"><div align="center" class="bttcn">省审核状态</div></td>
				</tr>
				
		   <c:forEach items="${list}" var="list" varStatus="status">
		       <tr  bgcolor="${(list.shbzw)=='2'?'#FFFF33':'#FFFFFF'}" title="">
		       		<td>
		            ...
		            <td>
					 ...
					 <td>
		       		...
		       		<td>
		       			<div align="center" >
		       				<c:choose>
								<c:when test="${list.shbzw eq 0}">未审核</c:when>
								<c:when test="${list.shbzw eq 1}">结单</c:when>
								<c:when test="${list.shbzw eq 2}">退单</c:when>
							</c:choose>
		       			</div>
		       		</td>
		       </tr>
      ...
      <% }}else if(!(intnum > 15)){
		...
		<table width="100%" height="8%" border="0" cellspacing="0" cellpadding="0">
		    ...
		    <tr>
				<td>
					<p>
						<font color="red" size="2">说明：省级退单显示为：黄色<br/></font>
					</p>
				</td>
			</tr>		                         
		</table>
...
function passCheck1(chooseIdStr) {//市级审核通过240
    sendRequest(path+"/servlet/CityCheckServlet?command=tongguo1&chooseIdStr="+chooseIdStr, "checkpass1");
}
function passnoCheck1(chooseIdStr) {//市级退单240
	sendRequest(path+"/servlet/CityCheckServlet?command=tuidan1&chooseIdStr="+chooseIdStr, "checknopass1");
} 
