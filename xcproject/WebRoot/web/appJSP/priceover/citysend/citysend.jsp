...
<%	
	...
	String xian = request.getParameter("xian")!=null?request.getParameter("xian"):"0";//区县	
  	...
  	String state=request.getParameter("state")!=null?request.getParameter("state"):"-1";//市派单状态
    ...
    int ss;
   	...
   	CitySendDaoImp dao = new CitySendDaoImp();
   	ss = dao.Check(loginId);
    if(bzw.equals("2")){
    	CitySendDaoImp dao1 = new CitySendDaoImp();
...
var path = '<%=path%>';
    ...
    function queryDegree(command){
    	...
    	if (document.getElementById("shi").value == ""
			...
			var bl1 = document.getElementById("beginbl").value;
			var bl2 = document.getElementById("endbl").value;
			if(isNaN(bl1)|| isNaN(bl2)){
				alert("比例必须为数字!");
			}else{
				document.form1.action=path+"/servlet/CitySendServlet?command=" + command;
...
function paidan(){
	       	...
	       	if(count!=0){
		        ...
		        for(var i = 0; i < l; i++){
		          ...
		          if(count1<=240*n||((bz+count2)>=239&&(bz+count2)<=241)){
			          ...
			          chooseIdStr=chooseIdStr.substring(0,chooseIdStr.length-1);
				      bzw=0;
			          document.form1.action=path+"/servlet/CitySendServlet?command=paidan&chooseIdStr="+chooseIdStr;
			          document.form1.submit(); 
			       }            
		        } 
	        }else{
    ...
    function qianshou(){
		document.form1.action=path+"/web/appJSP/priceover/citysend/citysend.jsp?bzw=2";
        document.form1.submit();
	}
...
<% }else{%>
		...
		<table width="100%"  border="0" cellspacing="0" cellpadding="0" height="18%">
			...
			<div style="width:700px;height:50px">
			       ...
			       <span style="font-size:14px;font-weight:bold;position: absolute;left:25px;top:15px;color:black">地市签收及派单</span>	
			 </div>
	    	</td></tr>	    	
	    	...
	    	<tr><td height="8%" width="1200">
	    		<table>
					...
					<tr class="form_label">
						...
						<td>比例(%)：</td>
	        		 	<td>
							<input type="text" name="beginbl" id="beginbl"
						...
						<td> <%if(permissionRights.indexOf("PERMISSION_SEARCH")>=0){%>
					             <div id="chaxun" style="position:relative;width:60px;height:23px;cursor: pointer;left:300%;float:right;top:0px;right:-340px">
		...
		<div style="width:100%;height:300px;overflow-x:auto;overflow-y:auto;border:1px">
			<table width="1500px" height="70%"  border="0" cellspacing="1" cellpadding="1" bgcolor="#cbd5de" class="form_label">
				<tr height = "23" class="relativeTag">
            			...
            			<td width="" height="23" bgcolor="#DDDDDD"><div align="center" class="bttcn">市派单状态</div></td>
				</tr>
		   ...
		   <c:forEach items="${list}" var="list" varStatus="status">
		       <tr  bgcolor="${(status.count)%2==0?'#DDDDDD':'#FFFFFF'}" title="">
		            ...
		            <td>
		       		...
		       		<td>
		       			<div align="center" ><a href="javascript:lookDetails('${list.zdid}','<%=accountmonth %>')">${list.zdname}</a></div>
		       		</td>
		            <td>
		    ...
		    <tr>
		    	<td align="right">
		    		<div id="daochu"
			      	...
			      	<div id="paidan" style="width:60px;height:23px;cursor:pointer;float:right;position:relative;right:20px">
								 <img src="<%=path %>/images/daochu.png" width="100%" height="100%">
							 <span style="font-size:12px;position: absolute;left:27px;top:5px;color:white">派单</span>      
					</div>  
		      	</td>
		    </tr>		                         
		</table>
	...
	function processResponse_checkcity1() {
    	if (XMLHttpReq.readyState == 4) { // 判断对象状态
    		if (XMLHttpReq.status == 200) { // 信息已经成功返回，开始处理信息
    			document.form1.bzw1.value = XMLHttpReq.responseText;
    		} else { //页面不正常
 	...
 	function passCheck1(chooseIdStr) {
    	sendRequest(path+"/servlet/CitySendServlet?command=paidan1&chooseIdStr="+chooseIdStr, "checkpass1");
	}
...
//-->
 ...
 <script language="javascript">
 var path = '<%=path%>';
 function lookDetails(zdid,bzyf){ 
	 window.open(path+"/servlet/CitySendServlet?command=info&zdid="+zdid+"&bzyf="+bzyf);	
 }
</script>
</html>
