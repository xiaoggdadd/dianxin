...
<%
	...
	String dfmc = (String) request.getParameter("zdbh");//站点名称
	String agcode1 = "";
...
function changeMC() {
	...
	var stationname1 = stationname.options[stationname.selectedIndex].value;//获取被选中的value
	if(stationname1==0){
		alert("请选择正确的站点名称！");
	}else if (0 == shi) {
		alert("城市不能为空");
	} else {
		document.form1.action = path+ "/servlet/LeaseBargainFeesServlet?action=zdMessage&loginId="+ loginId;
																		...
																		<select name="stationname"
																			...
																			<%
																				...
																				if (stationnamea != null) {
																					...
																					String leasename = "";
																					String leaseid = "";
																					if (stationnamea.size() == 5) {
																			%>
																			<option value="0">未查询到站点信息</option>
																			<%
																				} else {
																						...
																						for (int i = cscount; i < stationnamea.size() - 1; i += cscount) {
																							scode = (String) stationnamea.get(i
																									+ stationnamea.indexOf("ZDID"));
																							name = (String) stationnamea.get(i
																									+ stationnamea.indexOf("JZNAME"));
																							leasename = (String) stationnamea.get(i
																									+ stationnamea.indexOf("LEASENAME"));
																							leaseid = (String) stationnamea.get(i
																									+ stationnamea.indexOf("LEASEID"));
																							name = name + "--" + leasename;
																			%>
																			<option value="<%=scode%>,<%=leaseid%>"><%=name%></option>
																			<%}}}%>
																		</select> </span>
																	<%if (dfmc != null && !"".equals(dfmc) && stationnamea.size() > 5
																				) {
																	%>
																	<input type="text" name="zdbh"
																		style="width: 230px; position: absolute; left: 0px;"
																		value="<%=(String) stationnamea.get(4 + stationnamea.indexOf("JZNAME"))%>" />
																	<%} else {%>
																	<input type="text" name="zdbh"
																		style="width: 230px; position: absolute; left: 0px;"
																		value="" />
