...
function passCheck() {//通过
			...
			if (bzw1 == "1") {
				if (count1 <= r || (t >= 239 && t <= 241) {//满240条
					...
					document.form1.action = path
							+ "/servlet/LeaseFeesCityAuditServlet?action=tongguo&chooseIdStr1="
							+ chooseIdStr1 ;
	...
	sendRequest1(path
			+ "/servlet/LeaseFeesCityAuditServlet?action=tongguoajax&chooseIdStr1="
			+ chooseIdStr1);
}
...
function passCheckNoPass() {//通过
		...
		for ( var i = 0; i < l; i++) {
			...
			if (bzw1 == "1") {
				if (count1 <= r || (t >= 239 && t <= 241) {//满240条
					...
					bzw = 0;
					document.form1.action = path
							+ "/servlet/LeaseFeesCityAuditServlet?action=butongguo&chooseIdStr1="
							+ chooseIdStr1 ;
					document.form1.submit();
				}
			} else if (bzw1 == "0") {
	...
	sendRequest1(path
			+ "/servlet/LeaseFeesCityAuditServlet?action=butongguoajax&chooseIdStr1="
			+ chooseIdStr1);
}
            //取消审核
function passCheckCancel() {//取消审核
		...
		for ( var i = 0; i < l; i++) {
			...
			if (bzw1 == "1") {
				if (count1 <= r || (t >= 239 && t <= 241) {//满240条
					...
					bzw = 0;
					document.form1.action = path
							+ "/servlet/LeaseFeesCityAuditServlet?action=quxiao&chooseIdStr1="
							+ chooseIdStr1 ;
					document.form1.submit();
				}
			} else if (bzw1 == "0") {
	...
	sendRequest1(path
			+ "/servlet/LeaseFeesCityAuditServlet?action=quxiaoajax&chooseIdStr1="
			+ chooseIdStr1);
}

	 function queryDegree(action){
        document.form1.action=path+"/servlet/LeaseFeesCityAuditServlet?action=" + action;
        document.form1.submit();
        if(action=="chaxun"){
        	showdiv("请稍等..........");
        }
           ...
           <td width="50" >
             <div style="width:700px;height:50px">
                <img src="<%=path%>/images/btt.bmp" width="100%" height="100%" />
			     <span style="font-size:14px;font-weight:bold;position: absolute;left:25px;top:15px;color:black">租赁费用市审核</span>
	         </div></td>
		</tr>
		<tr><td height="20" >
                ...
                <td><select name="shi" class="selected_font" id="shi" onchange="changeCity()">
                 ...
                 <td><select name="xiaoqu" class="selected_font" onchange="javascript:document.form1.xiaoqu2.value=document.form1.xiaoqu.value">
                  ...
                  <td colspan="5">
				   ...
				   <option value="0">市级未审核</option>
				   <option value="1">市级审核通过</option>
				   <option value="2">市级审核不通过</option>
         	        </select>        
                 </td>
                  </tr>
                 </table>
