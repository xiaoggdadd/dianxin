			...
			if (bzw1 == "1") {
				if (count1 <= r
					...
					document.form1.action = path
							+ "/servlet/CountryHeadActServlet?command=checkcity&chooseIdStr1="
...
function queryDegree(command){
	if (document.getElementById("shi").value == ""
		...
		document.form1.action = path + "/servlet/CountryHeadServlet?action="+command;
		document.form1.submit();
		if("query" == command){
			showdiv("请稍等..............");
		}
	 			...
	 			<tr>
      				<td colspan="4" width="50">
						<div style="width:700px;height:50px">
							...
							<span style="font-size:14px;font-weight:bold;position: absolute;left:25px;top:15px;color:black">区县主任审核</span>	
						</div>
					</td>
    			</tr>
				<tr>
						...
						<table>
							<tr class="form_label">
								...
								<td>
	         					...
	         					<td>
							    	<div id="query" style="position:relative;width:59px;height:23px;right:-95px;cursor: pointer;TOP:0PX">
				...
				<table width="4000px" height="60%" border="0" cellspacing="1" cellpadding="1" bgcolor="#cbd5de" class="form_label">
					...
					<tr bgcolor="${(status.count)%2==0?'#DDDDDD':'#FFFFFF'}" title="${list.description}">
						...
						<td><div align="right" >${list.cshibl}</div></td>
   						...
   						<td><div align="center" >${list.sort}</div></td>
						...
						<td><div align="center" >${list.csdbdfe}</div></td> 
						<td><div align="center" >${list.csdbdfbz}%</div></td> 
...
function getMoreInfo(dbid,zflxflag,dfzflx,accountmonth){  	
    ...
    window.open(path+"/servlet/CountryHeadServlet?action=getMoreInfo&dbid="+dbid+"&biaozhi="+zflxflag+"&dfzflx="+dfzflx1+"&accountmonth="+accountmonth,
...
function passCheck1(chooseIdStr1, chooseIdStr2) {//通过240
	sendRequest1(path + "/servlet/CountryHeadActServlet?command=checkcity1&chooseIdStr1="
...
function passCheckNo1(chooseIdStr1, chooseIdStr2) {//取消通过1
	sendRequest1(path + "/servlet/CountryHeadActServlet?command=checkcityno1&chooseIdStr1="
			+ chooseIdStr1 + "&chooseIdStr2=" + chooseIdStr2, "checkcityno1");
}
function passCheckNo2(chooseIdStr1, chooseIdStr2) {//审核不通过1
	sendRequest1(path + "/servlet/CountryHeadActServlet?command=checkcityno11&chooseIdStr1="
			+ chooseIdStr1 + "&chooseIdStr2=" + chooseIdStr2, "checkcityno1");
}
//响应函数
		...
		for ( var i = 0; i < l; i++) {
			...
			if (bzw1 == "1") {
				if (count1 <= r
					...
					bzw = 0;
					document.form1.action = path
							+ "/servlet/CountryHeadActServlet?command=checkcityno&chooseIdStr1="
							+ chooseIdStr1 + "&chooseIdStr2=" + chooseIdStr2;
					document.form1.submit();
					showdiv("请稍等..............");
				}
		...
		for ( var i = 0; i < l; i++) {
			...
			if (bzw1 == "1") {
				if (count1 <= r
					...
					bzw = 0;
					document.form1.action = path
							+ "/servlet/CountryHeadActServlet?command=checkcityno2&chooseIdStr1="
							+ chooseIdStr1 + "&chooseIdStr2=" + chooseIdStr2;
					document.form1.submit();
					showdiv("请稍等..............");
				}
