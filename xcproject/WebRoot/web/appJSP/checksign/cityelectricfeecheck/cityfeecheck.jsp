			...
			if (bzw1 == "1") {
				if (count1<= r || (t >= 239 && t <= 241) {
					...
					document.form1.action = path
							+ "/servlet/CityFeeCheck?command=checkcity&chooseIdStr1="
...
function queryDegree(command) {
	if (document.getElementById("shi").value == ""
		...
		document.form1.action = path + "/servlet/CityFeeCheck?command="
				+ command;
		;
		document.form1.submit();
		if ("chaxun" == command) {
			...
			<div style="width: 100%; height: 300px; overflow-x: auto; overflow-y: auto; border: 1px">
				<table width="4000px" height="80%" border="0" cellspacing="1"
					...
					<c:forEach items="${list}" var="list" varStatus="status">
						<c:if test="${list.gdtx eq '上级退单'}">
						<tr bgcolor="red">
						</c:if>
						<c:if test="${list.gdtx != '上级退单'}">
...
function lookDetailss(statusi) {//查看详情
	...
	document.form1.attributes["action"].value = path
			+ "/servlet/CityFeeCheck?command=xiangqing&statusi=" + statusi;
	document.form1.submit();
}
var XMLHttpReq1;
...
function passCheck1(chooseIdStr1, chooseIdStr2) {//通过240
	sendRequest1(path
			+ "/servlet/CityFeeCheck?command=checkcity1&chooseIdStr1="
...
function passCheckNo1(chooseIdStr1, chooseIdStr2) {//取消通过1
	sendRequest1(path
			+ "/servlet/CityFeeCheck?command=checkcityno1&chooseIdStr1="
			+ chooseIdStr1 + "&chooseIdStr2=" + chooseIdStr2, "checkcityno1");
}
function passCheckNo2(chooseIdStr1, chooseIdStr2) {//审核不通过1
	sendRequest1(path
			+ "/servlet/CityFeeCheck?command=checkcityno11&chooseIdStr1="
			+ chooseIdStr1 + "&chooseIdStr2=" + chooseIdStr2, "checkcityno1");
}
//响应函数
			...
			for ( var i = 0; i < l; i++) {
				...
				if (bzw1 == "1") {
					if (count1<= r || (t >= 239 && t <= 241) {
						...
						bzw = 0;
						document.form1.action = path
								+ "/servlet/CityFeeCheck?command=checkcityno&chooseIdStr1="
			...
			for ( var i = 0; i < l; i++) {
				...
				if (bzw1 == "1") {
					if (count1<= r || (t >= 239 && t <= 241) {
						...
						bzw = 0;
						document.form1.action = path
								+ "/servlet/CityFeeCheck?command=checkcityno2&chooseIdStr1="
								+ chooseIdStr1 + "&chooseIdStr2="
								+ chooseIdStr2;
						document.form1.submit();
						showdiv("请稍等..............");
