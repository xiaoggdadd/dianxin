...
<style type="text/css">
   ...
   function lookDetailss(command){//电费单录入，修改，调高 详情
	   
	    var shi  = "${shi}";//市
	    var xian = "${xian}";//区县
	   	var beginyue = "${beginyue}";//起始月份
		var endyue = "${endyue}";//结束月份
		var dfshzt = "${dfshzt}";//电费审核状态
		var zdqyzt = "${zdqyzt}";//站点启用状态
		var dbqyzt = "${dbqyzt}";//电表启用状态
	    window.open(path + "/servlet/ModifyUnitPriceServlet?command="+command+"&shi="+shi+"&xian="+xian+"&beginyue="+beginyue+"&endyue="+endyue+"&dfshzt="+dfshzt+"&zdqyzt="+zdqyzt+"&dbqyzt="+dbqyzt);
   }
   
      function lookDetailss1(command){//电费单录入
	   
	    var shi  = "${shi}";//市
	    var xian = "${xian}";//区县
	   	var beginTime = "${beginyue}";//报账起始月份
		var endTime = "${endyue}";//报账结束月份
		var dfshzt = "${dfshzt}";//电费审核状态
		var dbqyzt = "${dbqyzt}";//电表启用状态
		var qyzt = "${zdqyzt}";//站点启用状态
		...
		if(dfshzt == "0"){//市级审核通过
			citystatus = "1";
		}else if(dfshzt == "1"){//人工审核通过
			manauditstatus = "1";
		}else if(dfshzt == "2"){//财务审核通过
			manauditstatus = "2";
		}else if(dfshzt == "3"){//市级主任审核通过
			sjzrsh = "1";
		}
		if(dbqyzt == ""){//电表启用状态
			dbqyzt ="-1";
	    ...
	    window.open(path + "/servlet/ElectricDetail?command="+command+"&shi="+shi+"&xian="+xian+"&beginTime="+beginTime+"&endTime="+endTime+"&citystatus="+citystatus+"&manauditstatus="+manauditstatus+"&sjzrsh="+sjzrsh+"&dbqyzt="+dbqyzt+"&qyzt="+qyzt);
   }
...
if (shi == "" || shi == "0" || shi == null){
		...
		if(command=="chaxun"){
		document.form1.action = path + "/servlet/ModifyUnitPriceServlet?command="+command;
   ...
   <form action="" name="form1" method="POST">
	...
	<tr>
		<td colspan=1 width="700" background="<%=path%>/images/btt.bmp" height=50>
			<span style="color: black; font-weight: bold;">&nbsp;&nbsp;&nbsp;修改单价查询&nbsp;&nbsp;</span>
		</td>
		<td width='380'>
		</td>
	</tr>
           ...
           <td   bgcolor="#DDDDDD" class="form_label"><div align="center" class="bttcn" style="color:black;font-weight:bold;font-size: 12px">城市</div></td>
			...
			<tr height="23">
        		...
        		<td bgcolor="#DDDDDD"></td>
      			<td bgcolor="#DDDDDD"><div align="right" class="bttcn" style="color:black;font-weight:bold;font-size: 12px"><a href="javascript:lookDetailss1('chaxun')">${sumbean.lrtssum}</a></div></td>
      			<td bgcolor="#DDDDDD"><div align="right" class="bttcn" style="color:black;font-weight:bold;font-size: 12px"><a href="javascript:lookDetailss('xiugai')">${sumbean.xgdjtssum}</a></div></td>
      			<td bgcolor="#DDDDDD"><div align="right" class="bttcn" style="color:black;font-weight:bold;font-size: 12px"><a href="javascript:lookDetailss('tiaogao')">${sumbean.djtgtssum}</a></div></td>
