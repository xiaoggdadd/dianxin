...
function queryDegree(command){
        ...
        if(""!=(year)){
	         document.form1.action=path+"/servlet/ProvServlet?command="+command;
           ...
           <td width="50" >
             <div style="width:700px;height:50px">
                <img src="<%=path%>/images/btt.bmp" width="100%" height="100%" />
			     <span style="font-size:14px;font-weight:bold;position: absolute;left:25px;top:15px;color:black">全省站点超标分析</span>
	         </div>
	        </td>
		  </tr>
		  <tr>
			...
			<fieldset>
				<legend><b><font size="2">数据信息</font></b></legend>
	 			<table width="100%" align="center" CellSpacing="0" style="BORDER-COLLAPSE: collapse" borderColor=#eaeaea bgColor=#ffffff border=1>
		           <tr> 
		             <td class="form_label"><div align="center" class="bttcn" style="color:black;font-weight:bold;font-size: 12px">市</div></td>
					...
					<tr>
		       			<td rowspan="3" class="form_label">
		       				<div align="center" >
		       					<a href="javascript:xiangxi('${list.shi}','<%=zdshuxing%>','<%=zdlx%>','<%=year%>')">${list.city}</a>
		       				</div>
		       			</td> 
...
function xiangxi(shi,zdsx,zdlx,year) {//查看详情
	//document.form1.attributes["target"].value = "_blank"; //当target属性为_blank的时候就是给form设置为新窗口提交
	document.form1.attributes["action"].value = path
			+ "/servlet/ProvServlet?command=xiangqing&shi2="+shi+"&zdsx="
