					...
					<div class="tbtitle01"><b>电表查询统计一览</b></div>
						<table width="100%"  border="0" cellpadding="0" cellspacing="0" class="tb_list1" >
									...
									<tr align="center">
										...
										<th width="100">预算责任中心</th>
										<th>操作</th>
									</tr>
							      <%
							       	 ArrayList costlist = new ArrayList();
							      	int totalRow = commBean.getMeterTotalRow(code, name,accountCthrnumber);
							         Page pager = new Page(totalRow, 10);
							         pager.setPage(Integer.parseInt(s_curpage));
							         int curpage = pager.getPage();
							      	costlist = commBean.getMeterPage(code, name ,pager,accountCthrnumber);
							      	int totalPage = pager.getTotalPage();
							      	int intnum = pager.getPageSize()*(pager.getPage()-1)+1;
									String pcode = "",pname = "", dbid="",gszt="",cbzx="",yszrzx="";
									 if(costlist!=null)
									{
										...
										for( int k = costcount;k<costlist.size()-1;k+=costcount){
									    	...
									    	gszt= (String)costlist.get(k+costlist.indexOf("GSZT"));
									    	cbzx=(String)costlist.get(k+costlist.indexOf("CBZX"));
											yszrzx=(String)costlist.get(k+costlist.indexOf("YSZRZX"));
							       ...
							       %>
									...
									<tr align="center">
										...
										<td align="left"><%=gszt%></td>
										<td align="left"><%=cbzx%></td>
										<td align="left"><%=yszrzx%></td>
										<td width="80">
...
<script>
    ...
    function previouspage(){
    	var curPage = "<%=pager.getPreviousPage() %>";
     	document.form1.action=path+"/web/sdttWeb/jizan/Meter.jsp?curpage="+curPage;
        document.form1.submit();
    }
    function nextpage(){
    ...
    function gopagebyno(pageno){
       document.form1.action=path+"/web/sdttWeb/jizan/Meter.jsp?curpage="+pageno;
       document.form1.submit();
    }


function query(){
	document.form1.action=path+"/web/sdttWeb/jizan/Meter.jsp";
	document.form1.submit();
}
