...
//报送时间
   ...
   %>
							...
							<table border="0" cellpadding="1" cellspacing="0"
								...
								<tr>
									...
									<td width="100px">
										...
										name="baosongtime" id="baosongtime" value="" readonly="readonly" class="Wdate"
         ...
         fylist =bean.getPageData(curpage,"ELECTRICFEES",loginId,whereStr);
		...
		int intnum = pagesize*(curpage-1)+1;
		 ...
		 if(fylist!=null)
			...
			for( int k = fycount;k<fylist.size()-1;k+=fycount){
				...
				if(bgdl!=null && !bgdl.equals("") && DIANLIANG!=null && !DIANLIANG.equals("")&&bgpll==""&&bgpll.equals("")){
					bgpll = df1.format(
							(Double.valueOf(DIANLIANG).doubleValue() -
									Double.valueOf(bgdl).doubleValue())
									/(Double.valueOf(bgdl).doubleValue()*100));
				}
				System.out.println(bgpll);
       ...
       %>
								...
								<tr align="center">
									<td width="80">
										<button type="button" class="btn_c1" onclick="banli('<%=workflowId%>')">
												审批
										</button>
										</td>
...
function banli(workFlowId,taskType){
		window.open("Baozhang_sh.jsp?workFlowId="+workFlowId, "newwindow", "height=650, width=1300, toolbar=no, menubar=no, scrollbars=no, resizable=no, location=no, status=no,scrollbars=yes");  
		//showdiv("请稍等..........");
}	
function checkPage() {
...
function plbl(n) {
	...
	function gopage()
     ...
     var rgsh2='<%=rgsh2%>';
      ...
      else
      {
        ...
        var curpage='<%=(curpage - 1)%>';
      		document.form1.action=path+"/web/sdttWeb/baozhang/bzsh.jsp?rgsh="+rgsh+"&caiji="+rgsh2+"&curpage="+curpage;
        document.form1.submit();
        showdiv("请稍等..........");
      }
     }
     ...
     var rgsh2='<%=rgsh2%>';
      ...
      document.form1.action=path+"/web/sdttWeb/baozhang/bzsh.jsp?rgsh="+rgsh+"&caiji="+rgsh2+"&curpage="+pageno;
      document.form1.submit();
      showdiv("请稍等..........");
     }
     function delbz(id){
...
function addbz(){
	window.open("../baozhang/addBaozhang.jsp", "newwindow", "height=500, width=1200, toolbar=no, menubar=no, scrollbars=no, resizable=no, location=no, status=no,scrollbars=yes");  
}
function daochu(){
...
function update(id,zdqyzt){
	if(zdqyzt=="2"){
		...
		window.open("../baozhang/UpdateBaozhang.jsp?id="+id, "newwindow", "height=300, width=1200, toolbar=no, menubar=no, scrollbars=no, resizable=no, location=no, status=no,scrollbars=yes");  
	}
}
function query(){
	document.form1.action=path+"/web/sdttWeb/baozhang/bzsh.jsp";
	document.form1.submit();
}
