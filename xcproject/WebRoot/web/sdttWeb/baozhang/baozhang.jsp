...
<script>
    ...
    String jiaofeifs = request.getParameter("jiaofeifs")!=null?request.getParameter("jiaofeifs"):"";
        ...
        String textdbbm = request.getParameter("textdbbm")!=null?request.getParameter("textdbbm"):"";//电表编码
...
<ul class="tab">
  <li class="first"></li>
  <li  class="cur"><a href="baozhang.jsp">报账信息</a></li>
...
<%
  ...
  if (txtDianbiao != "") {
	...
	if (zdname_key != "") {
		...
		if(textdbbm!=""){
			whereStr = whereStr + " and d.dbbm= '"+ textdbbm + "'";
		}
   %>
	<tr valign="top">
		...
		<td>
				...
				<div class="content01_1">
					<table border="0" cellpadding="1" cellspacing="0" class="tb_select" >
						...
						<td align="right" width="60px">电表名称：</td>
							...
							<td align="left" width="60px">
								<input type="text" name="textdbbm" id="textdbbm" style="box-sizing: border-box; width: 130px;" />
				         	</td>
						...
						<table width="100%"  border="0" cellpadding="0" cellspacing="0" class="tb_list1" >
									<tr align="left">
										<td colspan="15" align="left">
											...
											<input name="button2" type="submit" onclick="daochu()" class="btn_c1" id="button2" value="数据导出" />&nbsp;&nbsp;
										 <input name="button2" type="button" onclick="importExcel()" class="btn_c1" id="button2" value="数据导入" />
										</td>
									</tr>
									<tr align="center">
									    <th width="10">序号</th>
         ...
         fylist = bean.getPageData_baozhang(curpage,loginName,loginId,whereStr);
       ...
       %>
									...
									<tr align="center">
										...
										<td width="80">
										<%--
											<input name="button2" type="button" onclick="ch('<%=ID%>','<%=STATE%>')" class="btn_c1" value="撤回" />
											<input name="button2" type="button" onclick="rulu('<%=ID%>','<%=STATE%>')"" class="btn_c1" value="录入" />
											<input name="button2" type="button" onclick="xiangxi('<%=ID%>','<%=STATE%>')" class="btn_c1" value="详细" />
											<input name="button2" type="button" onclick="update('<%=ID%>','<%=STATE%>')" class="btn_c1" value="修改" />
										 	--%>
...
<script>
	...
	function gopage()
     ...
     var rgsh2='<%=rgsh2%>';
      ...
      else
      {
        ...
        var curpage='<%=(curpage - 1)%>';
      		document.form1.action=path+"/web/sdttWeb/baozhang/baozhang.jsp?rgsh="+rgsh+"&caiji="+rgsh2+"&curpage="+curpage;
        document.form1.submit();
      }
     }
     function nextpage()
     ...
     var rgsh2='<%=rgsh2%>';
      ...
      document.form1.action=path+"/web/sdttWeb/baozhang/baozhang.jsp?rgsh="+rgsh+"&caiji="+rgsh2+"&curpage="+pageno;
      document.form1.submit();
     }
     function delbz(id,s){ 
   if(s!=0){
     ...
     function ch(id,s){	
    	//alert("报账："+id);
    	 if(s!=1 && s!=3){   		   
    			alert("对不起！您只能撤回审核中或未通过的数据！");
        			return;   			
    		}
     
		if(confirm("您确认要撤回本条信息？")){
		     		document.form1.action=path+"/servlet/dianbiao?action=chbaozhang&id="+id;
		      		document.form1.submit();
		      		showdiv("请稍等..............");
		      	}
     }  
    ...
    if(s=="2"){
		...
		alert("未通过的数据请撤回之后再进行上报！");
	}
	else{
    	window.open("../baozhang/xzquery.jsp?id="+id, "newwindow", "height=500, width=1200, toolbar=no, menubar=no, scrollbars=no, resizable=no, location=no, status=no,scrollbars=yes");  
    }
    }
    function xiangxi(id){
    	window.open("../baozhang/xxquery.jsp?id="+id, "newwindow", "height=500, width=1200, toolbar=no, menubar=no, scrollbars=no, resizable=no, location=no, status=no,scrollbars=yes");  
    }
...
function addbz(){
	window.open("../baozhang/addBaozhang.jsp", "newwindow", "height=800, width=1400, toolbar=no, menubar=no, scrollbars=no, resizable=no, location=no, status=no,scrollbars=yes");  
}
function daochu(){
	document.form1.action=path+"/servlet/CbDownload?bz=baozhang";
...
function update(id,s){
	if(s=="2"){
		...
		alert("未通过的信息请先撤回之后进行修改");
	}
	else{
		window.open("../baozhang/UpdateBaozhang.jsp?id="+id, "newwindow", "height=460, width=1200, toolbar=no, menubar=no, scrollbars=no, resizable=no, location=no, status=no,scrollbars=yes");  
	}
}
...
function query(){
	document.form1.action=path+"/web/sdttWeb/baozhang/baozhang.jsp";
	document.form1.submit();
}

function importExcel(){
	window.open("../baozhang/importExcel.jsp", "newwindowImport", "height=460, width=1200, toolbar=no, menubar=no, scrollbars=no, resizable=no, location=no, status=no,scrollbars=yes");  
}
...
document.form1.textdbbm.value='<%=textdbbm%>';
