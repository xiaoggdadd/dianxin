...
<%@page import="com.noki.biaogan.servlet.ComputeUtil"%>
...
<script>
    ...
    String cthrnumber = account.getCthrnumber();//按角色查询
...
<ul class="tab">
  ...
  <li class="cur"><a href="AllTietabz.jsp">铁塔报账查询</a></li>
	...
	<tr valign="top">
		...
		<td>
			<div class="content01">
				...
				<div class="content01_1">
					...
					<div class="tbtitle01"><b>铁塔查询统计一览</b></div>
         ...
         fylist = bean.getAllTietabz(curpage,loginName,loginId,whereStr);
       	...
       	String FTDL="",BHSJE="",FTJE="",FTBL="",SFYZ="";
		...
		int intnum = pagesize*(curpage-1)+1;
		 ...
		 if(fylist!=null)
			...
			for( int k = fycount;k<fylist.size()-1;k+=fycount){
				FTJE = (String) fylist.get(k + fylist.indexOf("FTJE"));
				SFYZ = (String) fylist.get(k + fylist.indexOf("SFYZ"));
       			FTBL = (String) fylist.get(k + fylist.indexOf("FTBL"));
		       ...
		       BHSJE=""+(Double.valueOf(allmoney).doubleValue()*Double.valueOf(FTBL).doubleValue()/100);
		 
       			FTDL = ""+(Double.valueOf(dianliang).doubleValue() * Double.valueOf(FTBL).doubleValue()/100);
       			if(FTJE==null||FTJE.equals("")||FTJE==""){
       			
       			FTJE = ""+(Double.valueOf(allmoney).doubleValue()*Double.valueOf(FTBL).doubleValue()/1000*(1+Double.valueOf(SFYZ).doubleValue()));
       ...
       %>
									...
									<tr align="center">
										...
										<td align="center"><%=bean.number(zdname)%></td>
										<td align="center"><%=bean.number(dbcode)%></td>
										<td align="center"><%=bean.number(dianliang)%></td>
										<td align="center"><%=bean.number(FTBL)%></td>
										<td align="center"><%=bean.number(SFYZ)%></td>
										<td align="center"><%=bean.number(FTDL)%></td>
										<td align="center"><%=price%></td>
										<td align="center"><%=bean.number(allmoney)%></td>
										<td align="center"><%=bean.number(FTJE)%></td>
										<td align="center"><%=bean.number(BHSJE)%></td>
										<td align="center"><%=bean.number(fptype)%></td>
										<td align="center"><%=bean.number(jffs)%></td>
...
<script>
	...
	function gopage()
     ...
     var rgsh2='<%=rgsh2%>';
      ...
      else
      {
        ...
        var curpage='<%=(curpage - 1)%>';
      		document.form1.action=path+"/web/sdttWeb/baozhang/AllTietabz.jsp?rgsh="+rgsh+"&caiji="+rgsh2+"&curpage="+curpage;
        document.form1.submit();
      }
     }
     function nextpage()
     ...
     var rgsh2='<%=rgsh2%>';
      ...
      document.form1.action=path+"/web/sdttWeb/baozhang/AllTietabz.jsp?rgsh="+rgsh+"&caiji="+rgsh2+"&curpage="+pageno;
      document.form1.submit();
     }
     function delzd(id){
    // alert(id);
...
function addmss(){
	...
	if(isComp==1){
		...
		//	document.form1.submit();
			window.open("addbz.jsp?ids="+s, "newwindow", "height=530, width=1250, toolbar=no, menubar=no, scrollbars=no, resizable=no, location=no, status=no,scrollbars=yes");  
