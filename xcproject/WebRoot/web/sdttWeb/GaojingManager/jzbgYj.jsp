...
function delacc(accountId) {
	...
	if (confirm("确定要删除此账户么！")) {
		document.form1.action = path + "/servlet/account?action=del&accountId="
				+ accountId;
		document.form1.submit();
	} else {
		return;
...
function fzqy() {
	...
	if (i == 1) {
		...
		window
				.open('../sys/per_area.jsp?accountid=' + accountid, '',
			...
			<div class="content01_1">
				...
				<table width="100%" border="0" cellpadding="0" cellspacing="0"
					...
					<%
						...
						String yjid="",zdmc="",dccbsj="",sccbsj="",dccbdl="",sccbdl="",bgz="";
						BGYJDao dao = new BGYJDao();
						ArrayList list = new ArrayList();
						  list = dao.SelectBGYJ(curpage);
						  	 allpage=dao.getAllPage();
						...
						if (list != null) {
								...
								for( int k = fycount;k<list.size()-1;k+=fycount){
								 ...
								 yjid = (String) list.get(k + list.indexOf("ID"));
								 zdmc = (String) list.get(k + list.indexOf("JZNAME"));
								 dccbsj = (String) list.get(k + list.indexOf("DCCBSJ"));
								 sccbsj = (String) list.get(k + list.indexOf("SCCBSJ"));
								 dccbdl = (String) list.get(k + list.indexOf("DCCBDL"));
								 sccbdl = (String) list.get(k + list.indexOf("SCCBDL"));
								 bgz = (String) list.get(k + list.indexOf("BGZ"));
					%>
					<!-- 数据加载  Start-->
					<tr align="center">
						<td width="10"><%=intnum++%></td>
						<td align="left"><%=zdmc%></td>
						<td align="left"><%=dccbsj%></td>
						<td align="right"><%=sccbsj%></td>
						<td align="right"><%=dccbdl%></td>
						<td align="right"><%=sccbdl%></td>
						<td align="right"><%=bgz%></td>
						<td width="80" align="center">
							<div>
								<a href="#" onclick="delet('<%=yjid%>')"> <img
...
function nextpage(){
 ...
 function gopagebyno(pageno){
    ...
    document.form1.action=path+"/web/sdttWeb/GaojingManager/jzbgYj.jsp?curpage="+pageno;
    document.form1.submit();
}
...
function daochu() {
	document.form1.action = path + "/servlet/PlDownload?bz=JZBGYJ";
	document.form1.submit();
}
