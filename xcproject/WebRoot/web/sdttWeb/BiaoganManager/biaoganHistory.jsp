...
<body>
	...
	<table width="100%" border="0" cellspacing="0" cellpadding="0">
		<%
			...
			if (!xiaoqu.equals("0")) {
				whereStr = whereStr + " and c.xiaoqu='" + xiaoqu + "'";
				str = str + " and c.xiaoqu='" + xiaoqu + "'";
			} else if (!xian.equals("0")) {
				whereStr = whereStr + " and c.xian='" + xian + "'";
				str = str + " and c.xian='" + xian + "'";
			} else if (!shi.equals("0")) {
				whereStr = whereStr + " and c.shi='" + shi + "'";
				str = str + " and c.shi='" + shi + "'";
			} else if (!sheng.equals("0")) {
				whereStr = whereStr + " and c.sheng='" + sheng + "'";
				str = str + " and c.sheng='" + sheng + "'";
			}
			if (sname.length() > 0 && sname != null) {
				whereStr = whereStr + " and c.jzname like '%" + sname + "%'";
				str = str + " and c.jzname like '%" + sname + "%'";
			}
			...
			if (yearmonth != null && !yearmonth.equals("")) {
				whereStr = whereStr + " and a.YEARMONTH = '"+yearmonth+"'";
				str = whereStr + " and a.YEARMONTH = '"+yearmonth+"'";
			}
			
			if(gjz !=null&&!gjz.equals("")){
				whereStr = whereStr + " and (c.JZNAME like '%"+gjz+"%' or c.JZCODE like '%"+gjz+"%')";
				str = whereStr + " and (c.JZNAME = '%"+gjz+"%' or c.JZCODE like '%"+gjz+"%')";
			}
		%>
		<tr valign="top">
	...
	关键字：&nbsp;<input name="gjz" type="text" id="gjz"
		style="width:80px" value="<%=gjz %>" /></br> ※ 关键字只包括基站名称</td>
</tr>
<tr>
	<td align="right"><input name="button2" type="submit"	class="btn_c1"  id="button2" onclick="query()" value="查询" />&nbsp;
		</td>
	</tr>
</table>
...
<table width="100%" border="0" cellpadding="0" cellspacing="0"
	...
	<tr align="right">
		...
		<input name="daochuExcel" type="button" class="btn_c1" id="daochuExcel" value="新增" onclick="addBG();"/>&nbsp;
...
<%
	...
	biaoganlist = manage.searchBgList2(curpage, loginName, loginId,whereStr);
	...
	if (biaoganlist != null) {
		...
		for (int k = 0; k < biaoganlist.size(); k++) {
			...
			/* String sql2 = "select spzt from TBL_TT_BIAOGAN_LISHI where id ='"+id+"'";
			...
			a=Double.valueOf(ydxs);
			b=Double.valueOf(ktxs);
			c=Double.valueOf(biaoganvalue);
							%>
							<tr align="center">
								<td  align="right"><%=k+intnum%></td>
								<td  align="right">&nbsp;<%=_yearmonth %></td>
								<td  align="right">&nbsp;<%=jzname %></td>
								<td  align="right">&nbsp;<%=df1.format(a) %></td>
								<td  align="right">&nbsp;<%=df1.format(b) %></td>
								<td  align="right">&nbsp;<%=df1.format(c) %></td>
								<td > </td>
								<td width="80" align="right">
									<a href="#" onclick="update('<%=id%>')"><img src="../images/accept.png" width="16" height="16"  title="修改" /></a><a href="#" onclick="delzd('<%=id%>')"><img src="../images/delete.png" width="16" height="16"  title="删除" /></a>&nbsp; &nbsp; &nbsp;&nbsp; 
								</td>
							</tr>
							<%
								}
...
function addBG(){
	window.open("../BiaoganManager/biaoganAdd.jsp", "newwindow", "height=500, width=1200, toolbar=no, menubar=no, scrollbars=no, resizable=no, location=no, status=no");  
}

function update(id){
	window.open("../BiaoganManager/biaoganupdate.jsp?id="+id, "newwindow", "height=500, width=1200, toolbar=no, menubar=no, scrollbars=no, resizable=no, location=no, status=no");  
}
  function delzd(id){
     if(confirm("您确定删除此站点信息？")){
     	//<if (permissionRights.indexOf("PERMISSION_DELETE") >= 0) {%> //权限
     		document.form1.action=path+"/servlet/BGServlet?action=delBG&id="+id;
      		document.form1.submit();
      //<} else {%>
      //alert("您没有删除站点的权限");
    //<}%>
