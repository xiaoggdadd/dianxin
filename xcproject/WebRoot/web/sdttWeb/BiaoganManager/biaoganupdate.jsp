...
<%
	...
	String idd="0", zdid="0",yearmonth="",dbbm="0",city="",cityLow="",dbid="",zdbm="",zdmc="0",spzt="",
			sh="0",xx="0",xiaoqu="0",cccccc="",xxxxx="",qqqq="";
	double zlfh=0.00,ktxs=0.00,ydxs=0.00,bg=0.00;
	if(bz!=null&&bz.equals("1")&&id!=null){
		String sql = "select * from tbl_tt_biaogan_lishi where id ='"+id+"'";
	    ...
	    if(rs.next()){
	    	...
	    	bg=Double.valueOf(rs.getString("biaoganvalue")); 
	    	zlfh=Double.valueOf(rs.getString("zlfh")); 
	    	ydxs=Double.valueOf(rs.getString("ydxs")); 
	    	ktxs=Double.valueOf(rs.getString("ktxs")); 
	    	spzt=rs.getString("spzt"); 
	    	zdid=rs.getString("zdid"); 
	    	dbid=rs.getString("dbid"); 
	    }
	    System.out.println(zdid);
	    //已获得站点ID,用站点ID去获取市县小区
	    ResultSet rs1=null;
	    ResultSet rscity=null;//市的结果集
	    ResultSet rscitl=null;//县的结果集
	    ResultSet rscitll=null;//镇的结果集
	    String sql1 = "select shi,xian,jzname,xiaoqu from  zhandian  where id='"+zdid+"'";
	    rs1 = db.queryAll(sql1);
	    if(rs1.next()){
			city=rs1.getString(1);
			cityLow=rs1.getString(2);
			zdmc=rs1.getString("jzname");
			xiaoqu=rs1.getString("xiaoqu");
		}
		//根据省市ID查出名字
		String citt = " select agname from d_area_grade where agcode='"+city+"'";
		rscity = db.queryAll(citt);
	    if(rscity.next()){
			cccccc=rscity.getString("agname");
		}
		String citl = " select agname from d_area_grade where agcode='"+cityLow+"'";
		rscitl = db.queryAll(citl);
	    if(rscitl.next()){
			xxxxx=rscitl.getString("agname");
		}
		String citll = " select agname from d_area_grade where agcode='"+xiaoqu+"'";
		rscitll = db.queryAll(citll);
	    if(rscitll.next()){
			qqqq=rscitll.getString("agname");
		}
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
	<tr valign="top">
		...
		<td>
			<div class="content01">
			<%if("1".equals(bz)){ %>
				<div class="tit01">电价信息修改</div>
				<%}else{ %>
				<div class="tit01">电价信息新增</div>
				<%} %>
				<div class="content01_1">
					<table width="1000px" border="0" cellpadding="2" cellspacing="0" class="tb_select" >
						...
						<td align="right" width="100px">用电系数</td>
							<td><input type="text" name="ydxs" value="<%=ydxs%>" maxlength="20"/></td>
						<td align="right" width="100px">空调系数</td>
							<td><input type="text" name="ktxs" value="<%=ktxs%>" maxlength="20"/></td>
						<td align="right" width="100px">标杆值</td>
							<td><input type="text" name="bg" value="<%=bg%>" maxlength="20"/></td>
						</tr>	
						<tr>
						<td align="right" width="100px">直流负荷</td>
							<td><input type="text" name="zlfh" value="<%=zlfh%>" maxlength="20"/>
							</td>
...
<!-- 								<input name="button2" type="submit" class="btn_c1" id="button2" value="临时保存" />&nbsp;  -->
								<%	if(spzt.equals("0")){
									//未审核
								%>
								<input onclick="saveAccount()" type="button" class="btn_c1" id="button2" value="修改" />&nbsp; 
								<%	}else if(spzt.equals("1")){
									//审核通过
								%>
								<input onclick="over()" type="button" class="btn_c1" id="button2" value="修改" />&nbsp; 
								<%
									}else if(spzt.equals("-1")){
										//审核不通过
								%>
...
function addzhandian(){
    var bz=1;
    if("1"==bz){
    	if(confirm("您将要修改标杆信息！确认信息正确！")){      		
      		document.form1.action=path+"/servlet/BGServlet?action=updateBG&id="+'<%=id%>';
...
function over(){
	...
	<%-- document.form1.shi.value='<%=city%>';
	document.form1.xian.value='<%=cityLow%>';
