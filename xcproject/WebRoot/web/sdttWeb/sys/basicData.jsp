...
<script>
	...
	String keyword = request.getParameter("txtKeyword")!=null?request.getParameter("txtKeyword"):""; //获取模糊查询值
    ...
    curpage=Integer.parseInt(s_curpage);
   	...
   	//新改字典表遍历查询方法
   	SiteManage bean = new SiteManage();	//调用DAO层
    ArrayList<BasicDataBean> fylist = new ArrayList<BasicDataBean>();	//创建接受数据集合
	...
	allpage = bean.CountBasicData(keyword);//获取共多少页
	System.out.println("allpage:"+allpage);
	fylist = bean.ListBasicData(curpage, keyword); //获取字典表集合
%>
</script>
</head>
		 ...
		 if(fylist!=null)
		  ...
		  //for( int k = fycount;k<fylist.size()-1;k+=fycount){
			for( int k = 0;k<fylist.size();k++){
       		...
       		*/
       		//遍历获取DAO方法集合内的值
       		int zidianbiaoid = fylist.get(k).getId();
       		String ne = fylist.get(k).getName();
       		String ce = fylist.get(k).getCode();
       		String de = fylist.get(k).getDescribe();
       %>
				<!-- 数据加载  Start-->
									<tr align="center">
										...
										<td align="left"><%=de%></td>
										<td width="80">
											<a href="#" onclick="update('<%=zidianbiaoid%>')"><img src="../images/accept.png" width="16" height="16"  title="修改" /></a>
											<!--  <a href="#" onclick="delzd('<%=zidianbiaoid%>')"><img src="../images/delete.png" width="16" height="16"  title="删除" /></a>-->
										</td>
									</tr>
									<% } %>
									<!-- 数据加载 End -->
...
<script>
	...
	function gopage()
     ...
     var rgsh2='<%=rgsh2%>';
      ...
      else
      {
        ...
        var curpage='<%=(curpage - 1)%>';
      		document.form1.action=path+"/web/sdttWeb/sys/basicData.jsp?rgsh="+rgsh+"&caiji="+rgsh2+"&curpage="+curpage;
        document.form1.submit();
      }
     }
     function nextpage()
     ...
     var rgsh2='<%=rgsh2%>';
      ...
      document.form1.action=path+"/web/sdttWeb/sys/basicData.jsp?rgsh="+rgsh+"&caiji="+rgsh2+"&curpage="+pageno;
      document.form1.submit();
     }
     function delzd(id){
    // alert(id);
...
function addjz(){
	window.open("../sys/addSite.jsp", "newwindow", "height=500, width=1200, toolbar=no, menubar=no, scrollbars=no, resizable=no, location=no, status=no,scrollbars=yes");  
}
function update(id){
	window.open("../sys/addBasicData.jsp?id="+id+"", "newwindow", "height=400, width=1100, toolbar=no, menubar=no, scrollbars=no, resizable=no, location=no, status=no,scrollbars=yes");  
}
function query(){
	document.form1.action=path+"/web/sdttWeb/sys/basicData.jsp";
}
 function daochu(){
