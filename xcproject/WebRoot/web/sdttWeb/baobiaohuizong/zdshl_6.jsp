...
<%@ page import="com.noki.baobiaohuizong.bean.Zdshl"%>
...
<%	
		...
		//数据遍历获取
		Zdshl stl = new Zdshl();//实体类
		BaoBiaoHuiZongDao dao = new BaoBiaoHuiZongDao();//DAO方法
		List<Zdshl> al = new ArrayList<Zdshl>();//泛型集合
		//获取共有几页
		allpage = dao.pagesZdshl(shi,xian,xiaoqu); //共有几页
		...
		al = dao.queryZdshl(curpage,shi,xian,xiaoqu);//根据条件获取要显示的内容 
...
<script type="text/javascript">
			...
			function previouspage() {
				...
				document.form1.action = path +
					"/web/sdttWeb/baobiaohuizong/zdshl_6.jsp?rgsh=" + rgsh +
			...
			function daochu(){
				...
				var xian = '<%=xian%>';
				var xiaoqu = '<%=xiaoqu%>';
				document.form1.action=path+"/servlet/CbDownload?bz=zdshl&shi="+shi+"&xian="+xian+"&xiaoqu="+xiaoqu+"";
				document.form1.submit();
			}
			function query(){
				document.form1.action=path+"/web/sdttWeb/baobiaohuizong/zdshl_6.jsp";
				document.form1.submit();
			}
		</script>
</head>
...
<body>
	<ul class="tab">
		<li class="first"></li>
		<li class="cur"><a href="zdshl_6.jsp">自动审核率</a></li>
		<li><a href="ydxzzdslcx_7.jsp">月度新增站点数量查询</a></li>
						...
						<div class="content01_1">
							...
							<table width="100%" border="0" cellpadding="0" cellspacing="0"
								...
								<tr align="center">
									...
									<th width="50">乡镇</th>
									<th width="50">缴费站点</th>
									<th width="50">自动审核数量</th>
									<th width="50">自动审核通过率</th>
								</tr>
								<%
										for(int i=0;i<al.size();i++){
										//为null的话默认为空
										String createdate = al.get(i).getCreatedate()!= null ?al.get(i).getCreatedate():"";//年月
										...
										String zdshs = al.get(i).getZdshs()!= null ? al.get(i).getZdshs():"";//自动审核数量
										String zdshl = al.get(i).getZdshl()!= null ? al.get(i).getZdshl():"";//自动审核通过率
										...
										//把审核通过率转换为百分数显示 
										if(zdshl != "" && !zdshl.equals("")){
											int a = 0;
											a = Integer.parseInt(zdshl);	//将字符串转换为int
								...
								<tr align="center">
									...
									<td align="left"><%=zdshs%></td>
									<td align="left"><%=zdshl%></td>
								</tr>
								<%
									}
									 %>
