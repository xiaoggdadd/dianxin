...
<%@ page import="com.noki.baobiaohuizong.bean.Zhdjbb"%>
...
<html xmlns="http://www.w3.org/1999/xhtml">
	<head>
		...
		//数据遍历获取
		Zhdjbb stl = new Zhdjbb();//实体类
		BaoBiaoHuiZongDao dao = new BaoBiaoHuiZongDao();//DAO方法
		List<Zhdjbb> al = new ArrayList<Zhdjbb>();//泛型集合
		//获取共有几页
		allpage = dao.pagesZhdjbb(shi,xian,xiaoqu); //共有几页
		...
		al = dao.queryZhdjbb(curpage,shi,xian,xiaoqu);//根据条件获取要显示的内容 
		...
		<script type="text/javascript">
			...
			function previouspage() {
				...
				document.form1.action = path +
					"/web/sdttWeb/baobiaohuizong/zhdjbb_1.jsp?rgsh=" + rgsh +
			...
			function daochu(){
				...
				var xian = '<%=xian%>';
				var xiaoqu = '<%=xiaoqu%>';
				document.form1.action=path+"/servlet/CbDownload?bz=zhdjbb&shi="+shi+"&xian="+xian+"&xiaoqu="+xiaoqu+"";
				document.form1.submit();
			}
			function query(){
				document.form1.action=path+"/web/sdttWeb/baobiaohuizong/zhdjbb_1.jsp";
				document.form1.submit();
			}
		</script>
	</head>
							...
							<div class="content01_1">
								...
								<table width="100%" border="0" cellpadding="0" cellspacing="0" class="tb_list1">
									<tr align="center">
										...
										<th width="50">局站名称</th>
										<th width="50">电表名称</th>
										<th width="50">综合电价（总电费/总电量）</th>
									</tr>
									<%
										for(int i=0;i<al.size();i++){
										//为null的话默认为空
										String costtime = al.get(i).getCosttime()!= null ?al.get(i).getCosttime():"";//日期
										...
										String zhdanjia = al.get(i).getZhdanjia() != null ? al.get(i).getZhdanjia():"";//综合电价（总电费/总电量）
										...
										//单价精确到小数点后两位
										double d = Double.parseDouble(zhdanjia);// 这个是转为double类型  
										DecimalFormat df = new DecimalFormat("0.0000");   
										zhdanjia = df.format(d);  //这个是字符串，但已经是我要的两位小数了  
									 %>
									<tr align="center">
