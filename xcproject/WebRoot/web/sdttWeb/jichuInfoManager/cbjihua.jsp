	...
	<tr valign="top">
		...
		<td>
			<div class="content01">
				...
				<div class="content01_1">
					...
					<div class="tbtitle01"><b>抄表计划信息查询统计一览</b></div>
						<table width="100%"  border="0" cellpadding="0" cellspacing="0" class="tb_list1" >
									...
									<tr align="center">
										...
										<th width="80">姓名</th>
										<th width="80">分管基站</th>
										<th width="80">抄表计划日期</th> 									
										<th width="50">操作</th>
									</tr>
						<%
		 SiteManage bean = new SiteManage();
       	 ArrayList fylist = new ArrayList();
         fylist = bean.getCbjihua(curpage,loginName,loginId,whereStr);
       	 DecimalFormat df1 = new DecimalFormat("0.00");
       	 allpage=bean.getAllPage();
       	
		String id="", userid="",name="",jzname="",accountname="",cbjhrq="";
		int intnum = pagesize*(curpage-1)+1;
		 ...
		 if(fylist!=null)
			...
			for( int k = fycount;k<fylist.size()-1;k+=fycount){
       			...
       			accountname = (String) fylist.get(k + fylist.indexOf("ACCOUNTNAME"));
       			userid = (String) fylist.get(k + fylist.indexOf("USERID"));   			
       ...
       %>
									...
									<tr align="center">
										...
										<td align="center"><%=cbjhrq%></td>
<!-- 										<td align="center"><=beilv%></td> -->
<!-- 										<td align="center"><=danjia%></td> -->
										<td width="80">
...
<script>
	...
	function gopage()
     ...
     var rgsh2='<%=rgsh2%>';
      ...
      else
      {
        ...
        var curpage='<%=(curpage - 1)%>';
      		document.form1.action=path+"/web/sdttWeb/jichuInfoManager/cbyuanmanage.jsp?rgsh="+rgsh+"&caiji="+rgsh2+"&curpage="+curpage;
        document.form1.submit();
      }
     }
     function nextpage()
     ...
     var rgsh2='<%=rgsh2%>';
      ...
      document.form1.action=path+"/web/sdttWeb/jichuInfoManager/cbyuanmanage.jsp?rgsh="+rgsh+"&caiji="+rgsh2+"&curpage="+pageno;
      document.form1.submit();
     }
     function delzd(id){
     if(confirm("您确定删除此条信息？")){
     
     		document.form1.action=path+"/servlet/Cbjihuaservlet?action=delete&id="+id;
      		document.form1.submit();
   		 }
    }
    function xiangxi(id){
...
function addjz(){
	window.open("../jichuInfoManager/addCbjihua.jsp", "newwindow", "height=500, width=1200, toolbar=no, menubar=no, scrollbars=no, resizable=no, location=no, status=no");  
}
function daochu(){
	document.form1.action=path+"/servlet/CbDownload?bz=cbjh";
	document.form1.submit();
}
function update(id){
	window.open("../jichuInfoManager/addCbjihua.jsp?id="+id+"+&bz="+"1", "newwindow", "height=500, width=1200, toolbar=no, menubar=no, scrollbars=no, resizable=no, location=no, status=no");  
}
function query(){
	document.form1.action=path+"/web/sdttWeb/jichuInfoManager/cbjihua.jsp";
}
