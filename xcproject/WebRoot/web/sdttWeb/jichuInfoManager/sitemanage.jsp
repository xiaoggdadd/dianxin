...
<ul class="tab">
  <li class="first"></li>
  <li class="cur"><a href="sitemanage.jsp">局站管理</a></li>
  <li><a href="dianbiaolist.jsp">电表管理</a></li>
	...
	<tr valign="top">
		...
		<td>
			<div class="content01">
				...
				<div class="content01_1">
					...
					<div class="tbtitle01"><b>局站查询统计一览</b></div>
						<table width="100%"  border="0" cellpadding="0" cellspacing="0" class="tb_list1" >
									<tr align="left">
										<td colspan="10" align="left">
										    <input name="button2" type="hidden" onclick="addjz()" class="btn_c1" value="新增"  />&nbsp;&nbsp;
											<input name="button2" type="submit" onclick="daochu()" class="btn_c1" id="button2" value="导出Excel" />
										</td>
									</tr>
									<tr align="center">
									    <th width="10">序号</th>
										<th width="100">局站名称</th>
										<th width="100">监控中心</th>
										<th width="80">地市</th>
										<th width="80">区县</th>
										<th width="80">乡镇</th>
										<th width="80">局站完整编码</th>
										<th width="80">业务等级</th>
										<th width="80">局站等级</th>
										<th>操作</th>
									</tr>
         ...
         fylist = bean.getPageData_new(curpage,loginName,loginId,whereStr);
       	 DecimalFormat df1 = new DecimalFormat("0.00");
       	 allpage=bean.getAllPage();
       	double a=0;
		String zdqyzt="", zlfh="",jlfh="", jzname = "",szdq = "",jzproperty = "", jztype= "",yflx = "",jnglmk="",gdfs="",area="",id="",dianfei="",zdcode="",xunisign="",shsignxs="",dbid="",xlx="";
		String dlbg="",zldy="";
		String LSCID="",STATION_NO="",AREA_ID="",DISTRICT_ID="",COUNTRY_ID="",FULL_STATION_CODE="",STATION_NAME="",STATION_CODE="",SERVICE_ID="",RANK_ID="",SERVICE_NAME="",RANK_NAME="";
		int intnum = pagesize*(curpage-1)+1;
		 ...
		 if(fylist!=null)
			...
			for( int k = fycount;k<fylist.size()-1;k+=fycount){
				...
				zdqyzt = (String) fylist.get(k + fylist.indexOf("QYZT"));
				STATION_NO = (String) fylist.get(k + fylist.indexOf("STATION_NO"));
				LSCID = (String) fylist.get(k + fylist.indexOf("LSCID"));
				AREA_ID = (String) fylist.get(k + fylist.indexOf("SHI"));
				DISTRICT_ID = (String) fylist.get(k + fylist.indexOf("XIAN"));
				COUNTRY_ID = (String) fylist.get(k + fylist.indexOf("XIAOQU"));
				FULL_STATION_CODE = (String) fylist.get(k + fylist.indexOf("FULL_STATION_CODE"));
				STATION_NAME = (String) fylist.get(k + fylist.indexOf("JZNAME"));
				STATION_CODE = (String) fylist.get(k + fylist.indexOf("STATION_CODE"));
				SERVICE_ID = (String) fylist.get(k + fylist.indexOf("SERVICE_ID"));
				SERVICE_NAME = (String) fylist.get(k + fylist.indexOf("SERVICE_NAME"));
				RANK_ID = (String) fylist.get(k + fylist.indexOf("RANK_ID"));
				RANK_NAME = (String) fylist.get(k + fylist.indexOf("RANK_NAME"));
       ...
       %>
									...
									<tr align="center">
										...
										<%--<td width="10"><%=STATION_NO%></td>--%>
										<td align="center"><%=STATION_NAME%></td>
										<td align="center"><%=LSCID%></td>
										<td align="center"><%=AREA_ID%></td>
										<td align="center"><%=DISTRICT_ID%></td>
										<td align="center"><%=COUNTRY_ID%></td>
										<td align="center"><%=FULL_STATION_CODE%></td>
										<td align="center"><%=SERVICE_NAME%></td>
										<td align="center"><%=RANK_NAME%></td>
										<td width="80">
											<a href="#" onclick="xiangxi('<%=STATION_NO%>')"><img src="../images/lefticon3.png" width="16" height="16"  title="详细" /></a>
											<a href="#" onclick="update('<%=STATION_NO%>','<%=zdqyzt%>')"><!-- <img src="../images/accept.png" width="16" height="16"  title="修改" /></a>
											<a href="#" onclick="delzd('<%=STATION_NO%>')"><img src="../images/delete.png" width="16" height="16"  title="删除" /></a> -->
										</td>
									</tr><% } %>
...
function addjz(){
	window.open("../jizan/addSite.jsp", "newwindow", "height=530, width=1250, toolbar=no, menubar=no, scrollbars=no, resizable=no, location=no, status=no,scrollbars=yes");  
}
function daochu(){
	document.form1.action=path+"/servlet/CbDownload?bz=site";
	document.form1.submit();
}
function update(id,zdqyzt){
	if(zdqyzt=="1"){
		alert("审核通过的不能再修改");
	}else{
	window.open("../jizan/UpdateSite.jsp?id="+id, "newwindow", "height=500, width=1200, toolbar=no, menubar=no, scrollbars=no, resizable=no, location=no, status=no,scrollbars=yes");  
}
}
function query(){
	document.form1.action=path+"/web/sdttWeb/jichuInfoManager/sitemanage.jsp";
	document.form1.submit();
}
