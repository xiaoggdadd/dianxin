...
<%
	...
	session.setAttribute("sfyz", id);
	String bz = "0";
	SiteModifyBean form = new SiteModifyBean();
	bz=request.getParameter("bz");
	String idd="0", zdid="0",yearmonth="",dbbm="0",sfyz="",sh="0",xx="0",zdmc="0",xiaoqu="0";
	if(bz!=null&&bz.equals("1")&&id!=null){
		String sql = "select zdid,yearmonth,sfyz,dbbm from sfyz where id = '"+id+"'";
...
<%if(!"1".equals(bz)){ %>
<title>新增税负因子信息</title>
<% }else{%>
<title>修改税负因子信息</title>
<%}%>
	...
	<tr valign="top">
		...
		<td>
			<div class="content01">
				...
				<%if(!"1".equals(bz)){ %>
				<div class="tit01">税负因子信息新增</div>
				<% }else{%>
				<div class="tit01">税负因子信息修改</div>
				<%}%>
				<div class="content01_1">
					<table width="1000px" border="0" cellpadding="2" cellspacing="0" class="tb_select" >
						...
						<td align="right" width="100px">税负因子</td>
						<td width="60px">
							<input name="sfyz" value='<%=sfyz %>' />
						</td>
						</tr>							
						<tr>
...
<!-- 								<input name="button2" type="submit" class="btn_c1" id="button2" value="临时保存" />&nbsp;  -->
	...
	function saveAccount(){
		          if(checkNotSelected(document.form1.shi,"城市")&&
						...
						checkNotnull(document.form1.sfyz,"税负因子")
						
					){	
					var a = document.form1.sfyz.value;
					if(isNaN(a)){
						alert("税负因子必须为数字！");
						return false;
					}
        	...
        	var bz='<%=bz%>';
        	if("1"==bz){
        		if(confirm("您将要修改税负因子信息！确认信息正确！")){      		
          		document.form1.action=path+"/servlet/SfyzServlet?action=addsfyz&bz="+'<%=bz%>';
				document.form1.submit();
				showdiv("请稍等..............");
           		 }
        	}else{
        	if(confirm("您将要添加税负因子信息！确认信息正确！")){      		
          		document.form1.action=path+"/servlet/SfyzServlet?action=addsfyz&bz="+'<%=bz%>';
				document.form1.submit();
				showdiv("请稍等..............");
           		 }
