...
<%
	...
	String yddl="",ltdl="",dxdl="",qtdl="",zdl="", idd="0", zdid="",yearmonth="",dbbm="0",yidong="",liantong="",dianxin="",qita="",sh="0",xx="0",zdmc="0",xiaoqu="0";
	if(bz!=null&&bz.equals("1")&&id!=null){
		String sql = "select zdid,yearmonth,yidong,liantong,dianxin,qita,dbbm,yddl,ltdl,dxdl,qtdl,zdl from ftbl where id = '"+id+"'";
	    ...
	    if(rs.next()){
	    	...
	    	yidong=rs.getString("yidong");
	    	liantong=rs.getString("liantong");
	    	dianxin=rs.getString("dianxin");
	    	qita=rs.getString("qita");  
	    	dbbm=rs.getString("dbbm");  	
	    	yddl=rs.getString("yddl")==null?"0":rs.getString("yddl");
	    	ltdl=rs.getString("ltdl")==null?"0":rs.getString("ltdl");
	    	dxdl=rs.getString("dxdl")==null?"0":rs.getString("dxdl");
	    	qtdl=rs.getString("qtdl")==null?"0":rs.getString("qtdl");
	    	zdl=rs.getString("zdl")==null?"0":rs.getString("zdl");
	    }
...
<meta http-equiv="Content-Type" content="text/html; charset=utf-8" />
	<%if(!"1".equals(bz)){ %>
		<title>新增分摊比例信息</title>
		<% }else{%>
		<title>修改分摊比例信息</title>
		<%}%>
	...
	<tr valign="top">
		...
		<td>
			<div class="content01">
				...
				<%if(!"1".equals(bz)){ %>
				<div class="tit01">新增分摊比例信息</div>
		<% }else{%>
				<div class="tit01">修改分摊比例信息</div>
		<%}%>
				<div class="content01_1">
					<table width="1000px" border="0" cellpadding="2" cellspacing="0" class="tb_select" >
						...
						<td width="60px">
							<input onfocus="WdatePicker({skin:'default',dateFmt:'yyyy-MM'})" name="yearmonth" value='<%=yearmonth %>' />
						</td>
						<td align="right" width="100px">移动电流</td>
						<td width="60px">
							<input name="yddl" value='<%=yddl%>' onchange="jszdl()" />
						</td>
						<td align="right" width="100px">联通电流</td>
						<td width="60px">
							<input name="ltdl" value='<%=ltdl%>' onchange="jszdl()" />
						</td>
						</tr>	
						<tr>
						<td align="right" width="100px">电信电流</td>
						<td width="60px">
							<input name="dxdl" value='<%=dxdl %>' onchange="jszdl()" />
						</td>
						<td align="right" width="100px">其他电流</td>
						<td width="60px">
							<input name="qtdl" value='<%=qtdl %>' onchange="jszdl()" />
						</td>
						<td align="right" width="100px">总电流</td>
						<td width="60px" >
							<input name="zdl" readonly="readonly" value='<%=zdl%>'/>
						</td>
						</tr>
						<tr>
...
<script>
	function jszdl(){
		...
		var a = isNaN(parseFloat(document.form1.yddl.value))==false?parseFloat(document.form1.yddl.value):"0";  
		var b = isNaN(parseFloat(document.form1.ltdl.value))==false?parseFloat(document.form1.ltdl.value):"0";			          		
		var c = isNaN(parseFloat(document.form1.dxdl.value))==false?parseFloat(document.form1.dxdl.value):"0";		          	
		var d = isNaN(parseFloat(document.form1.qtdl.value))==false?parseFloat(document.form1.qtdl.value):"0";	
		// alert(b);
		var e=parseFloat(a).toFixed(2);
		var f=parseFloat(b).toFixed(2);
		var g=parseFloat(c).toFixed(2);
		var h=parseFloat(d).toFixed(2);
		document.form1.yddl.value=e;
		document.form1.ltdl.value=f;
		document.form1.dxdl.value=g;
		document.form1.qtdl.value=h;
		
		document.form1.zdl.value=(parseFloat(e)+parseFloat(f)+parseFloat(g)+parseFloat(h)).toFixed(2);
	...
	function saveAccount(){
					if(checkNotSelected(document.form1.shi,"城市")&&
		          	...
		          	var y = parseFloat(document.form1.yddl.value);  
		          	var l = parseFloat(document.form1.ltdl.value);			          		
		          	var dd = parseFloat(document.form1.dxdl.value);		          	
		          	var o = parseFloat(document.form1.qtdl.value);	
		          	var z = parseFloat(document.form1.zdl.value);
		          	var aa = parseFloat(y+l+dd+o);
// 		          	alert(aa);
// 		          	alert(z);
		          	if(aa!=z){
		          		alert("各家运营商电流总和要等于总电流，请核算！");
		          	}else{
		          		var yd = parseFloat(y/z).toFixed(4);
		          		var lt = parseFloat(l/z).toFixed(4);
		          		var dx = parseFloat(dd/z).toFixed(4);
		          		var qt = parseFloat(o/z).toFixed(4);
		          		var a=parseFloat(yd);
		          		var b=parseFloat(lt);
		          		var c=parseFloat(dx);
		          		var d=parseFloat(qt);
		          		...
		          		if(a!=0&&b!=0&&c!=0&&d!=0){
		          			var hh=parseFloat(1-a-b-c).toFixed(4);
		          			d=parseFloat(hh);
		          		}
// 		          		alert(a);
// 		          		alert(b);
...
// 		          		alert(d);
        	...
        	function addzhandian(a,b,c,d){
        	var bz='<%=bz%>';
        	if("1"==bz){
        		if(confirm("您将要修改分摊比例信息！确认信息正确！")){      		
          		document.form1.action=path+"/servlet/FtblServlet?action=addftbl&bz="+'<%=bz%>'+"&a="+a+"&b="+b+"&c="+c+"&d="+d;
				document.form1.submit();
				showdiv("请稍等..............");
           		 }
           		 }else{
           		 if(confirm("您将要添加分摊比例信息！确认信息正确！")){      		
          		document.form1.action=path+"/servlet/FtblServlet?action=addftbl&bz="+'<%=bz%>'+"&a="+a+"&b="+b+"&c="+c+"&d="+d;
				document.form1.submit();
				showdiv("请稍等..............");
           		 }
