...
<ul class="tab">
  <li class="first"></li>
  <li class="cur"><a href="ftblmanage.jsp">分摊比例管理</a></li>
  <li><a href="sfyzmanage.jsp">税负因子管理</a></li>
  <li class="end"></li>
</ul>
		...
		<td>
			<div class="content01">
				...
				<div class="content01_1">
					...
					<div class="tbtitle01"><b>分摊比例查询统计一览</b></div>
						<table width="100%"  border="0" cellpadding="0" cellspacing="0" class="tb_list1" >
									...
									<tr align="center">
									    <th width="10">序号</th>
										<th width="120">基站名称</th>
										<th width="100">年度月份</th>
										<th width="80">移动运营商</th>
										<th width="80">联通运营商</th>
										<th width="80">电信运营商</th>
										<th width="80">其他</th>
										<th>操作</th>
									</tr>
						<%
         ...
         fylist = bean.getFtbl(curpage,loginName,loginId,whereStr);
       	 DecimalFormat df1 = new DecimalFormat("0.00");
       	 allpage=bean.getAllPage();
       	 double a=0,b=0,c=0,d=0;
		String id="", yearmonth="",yidong="",liantong="",dianxin="",qita="",zdmc="";
		int intnum = pagesize*(curpage-1)+1;
		 ...
		 if(fylist!=null)
			...
			for( int k = fycount;k<fylist.size()-1;k+=fycount){
       			...
       			yidong = (String) fylist.get(k + fylist.indexOf("YIDONG"))!=null?(String) fylist.get(k + fylist.indexOf("YIDONG")):"0.00";
       			liantong = (String) fylist.get(k + fylist.indexOf("LIANTONG"))!=null?(String) fylist.get(k + fylist.indexOf("LIANTONG")):"0.00";
       			dianxin = (String) fylist.get(k + fylist.indexOf("DIANXIN"))!=null?(String) fylist.get(k + fylist.indexOf("DIANXIN")):"0.00";
       			qita = (String) fylist.get(k+ fylist.indexOf("QITA"))!=null?(String) fylist.get(k+ fylist.indexOf("QITA")):"0.00";
       			zdmc = (String) fylist.get(k + fylist.indexOf("JZNAME"));
       			if(yidong!=null&&yidong!=""){
				a=Double.valueOf(yidong);
				}
				if(liantong!=null&&liantong!=""){
				b=Double.valueOf(liantong);
				}
				if(dianxin!=null&&dianxin!=""){
				c=Double.valueOf(dianxin);
				}
				if(qita!=null&&qita!=""){
				d=Double.valueOf(qita);  
				}     			
       %>
...
<script>
	...
	function gopage()
     ...
     var rgsh2='<%=rgsh2%>';
      ...
      else
      {
        ...
        var curpage='<%=(curpage - 1)%>';
      		document.form1.action=path+"/web/sdttWeb/jichuInfoManager/ftblmanage.jsp?rgsh="+rgsh+"&caiji="+rgsh2+"&curpage="+curpage;
        document.form1.submit();
      }
     }
     function nextpage()
     ...
     var rgsh2='<%=rgsh2%>';
      ...
      document.form1.action=path+"/web/sdttWeb/jichuInfoManager/ftblmanage.jsp?rgsh="+rgsh+"&caiji="+rgsh2+"&curpage="+pageno;
      document.form1.submit();
     }
     function delzd(id){
     if(confirm("您确定删除此条信息？")){
     	//<if (permissionRights.indexOf("PERMISSION_DELETE") >= 0) {%> //权限
     		document.form1.action=path+"/servlet/FtblServlet?action=delete&id="+id;
      		document.form1.submit();
    }
    }
    function xiangxi(id){
...
function addjz(){
	window.open("../jichuInfoManager/addFtbl.jsp", "newwindow", "height=500, width=1200, toolbar=no, menubar=no, scrollbars=no, resizable=no, location=no, status=no");  
}
function daochu(){
	document.form1.action=path+"/servlet/CbDownload?bz=ftbl";
	document.form1.submit();
}
function update(id){
	window.open("../jichuInfoManager/addFtbl.jsp?id="+id+"&bz="+"1", "newwindow", "height=500, width=1200, toolbar=no, menubar=no, scrollbars=no, resizable=no, location=no, status=no");  
}
function query(){
	document.form1.action=path+"/web/sdttWeb/jichuInfoManager/ftblmanage.jsp";
}
