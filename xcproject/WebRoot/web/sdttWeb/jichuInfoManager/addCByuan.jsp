...
<%
	...
	session.setAttribute("cbyuan", id);
	SiteModifyBean form = new SiteModifyBean();
	String bz = "0";
	bz=request.getParameter("bz");
	String zdid="", zdmc="0",username="0",zdbm="",sh="0",xia="0",xiaoqu="0",jzcode="0",dbbm="0";
	if(bz!=null&&bz.equals("1")&&id!=null){
		String sql = "select c.userid,c.dbbm,a.name,z.id,z.jzname,z.jzcode from cbyuan c,account a,zhandian z where c.userid=a.accountid and c.zdid=z.id and c.id="+id+"";
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
	<tr valign="top">
		...
		<td>
			<div class="content01">
			<%if("1".equals(bz)){ %>
				<div class="tit01">抄表员信息修改</div>
				<%}else{ %>
				<div class="tit01">抄表员信息新增</div>
				<%} %>
...
var path = '<%=path%>';
        	...
        	function addzhandian(){
        		var bz='<%=bz%>';
        		if("1"==bz){
        			if(confirm("您将要修改抄表员信息！确认信息正确！")){       		
          		document.form1.action=path+"/servlet/Cbyuanservlet?action=addCbyuan&bz="+'<%=bz%>';
				document.form1.submit();
           		 }
        		}else{
        			if(confirm("您将要添加抄表员信息！确认信息正确！")){       		
          		document.form1.action=path+"/servlet/Cbyuanservlet?action=addCbyuan&bz="+'<%=bz%>';
				document.form1.submit();
           		 }
        		}
