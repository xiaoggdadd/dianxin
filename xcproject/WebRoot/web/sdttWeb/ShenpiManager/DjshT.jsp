...
<link href="../css/content.css" rel="stylesheet" type="text/css" />
	...
	//获取本页查询的信息 
	String usename = request.getParameter("usename")!=null?request.getParameter("usename"):"";
	String adminsname = request.getParameter("adminsname")!=null?request.getParameter("adminsname"):"";
	// String sheng = (String)session.getAttribute("accountSheng");
	String shi = request.getParameter("shi")!=null?request.getParameter("shi"):"13701";
...
function chaxun() {
	document.form1.action = path + "/web/sdttWeb/ShenpiManager/DjshT.jsp";
	document.form1.submit();
}
function accountOn() {
	var i = 0;
...
function sh(id){
window.open("../ShenpiManager/DanjiaSh_xx.jsp?id="+id, "newwindow", "height=500, width=1200, toolbar=no, menubar=no, scrollbars=no, resizable=no, location=no, status=no");
}
</script>
<body>
					...
					<table width="800px" border="0" cellpadding="1" cellspacing="0" class="tb_select" >
						...
						<tr>
							<td align="right" colspan="8">
								<input type="button" class="btn_c1" onclick="chaxun()" value="查询" /> 
							</td>
						</tr>
					</table>
					
					<div class="tbtitle01"><b>电价变更审批一览</b></div>
...
<!-- 									       <input name="button2" type="submit" class="btn_c1" id="button2" value="批量审批" />&nbsp;  -->
						...
						String whereStr = "";
		                   ...
		                   list = bean.getAllDjSh(whereStr, usename, adminsname, curpage,ssagcode);//获取 用户的分页数据  ssagcode登录用户所属的区域 编码 
		                             allpage = bean.getAllPage();
											int intnum = pagesize*(curpage-1)+1;;
									//DJ.ID, DJ.YEARMONTH,DJ.DBBM,DB.DBNAME,RTNAME(DB.DFZFLX) DFZFLX,
									//DB.BEILV,DJ.DANJIA,DJ.SHENHE_FLAG		
											String accountId = "", accountName = "", name = "", mobile = "", roleName = "", sign = "", tel = "";
											String rsheng = "", rshi = "", rxian = "", rxiaoqu = "";
											
											String zdmc="", id="", yf="",dbbm="",dbname="",dfzflx="",bl="",dj="",shzt="";//,yd="",qt="",zt="";
										String color = "F3F3F3";
		                                      DecimalFormat de = new DecimalFormat("0.00");
		                                      DecimalFormat dee = new DecimalFormat("0.0000");
		                                      double a=0,b=0;
			                                  if(list!=null){
											...
											for (int i = countColum; i < list.size() - 1; i += countColum) {
												...
												dbbm=(String) list.get(i + list.indexOf("DBBM"));
												dbname=(String) list.get(i + list.indexOf("DBNAME"));
												dfzflx=(String) list.get(i + list.indexOf("DFZFLX"));
												bl=(String) list.get(i + list.indexOf("BEILV"));
												dj=(String) list.get(i + list.indexOf("DANJIA"));
												shzt=(String) list.get(i + list.indexOf("SHENHE_FLAG"));
												zdmc=(String) list.get(i + list.indexOf("JZNAME"));
												a=Double.valueOf(bl.toString());
												b=Double.valueOf(dj.toString());
												if("1".equals(shzt)){
												shzt="通过";
       ...
       %>
									...
									<tr align="center">
										...
										<td align="left"><%=yf%></td>
										<td align="left"><%=dbbm%></td>
										<td align="center"><%=dbname%></td>
										<td align="center"><%=dfzflx%></td>
										<td align="right"><%=de.format(a)%></td>
										<td align="right"><%=dee.format(b)%></td>
										<td align="center"><%=shzt%></td>
										<%if(shzt.equals("通过")){ %>
										<td width="80" align="center"><div >
											&nbsp;<input name="button2" type="button" class="btn_c1" onclick="sh(<%=id%>)" id="button2" disabled="disabled" value="审批" />&nbsp; 
												</div></td>
												<%}else{ %>
												<td width="80" align="center"><div >
	...
	function gopage()
     ...
     var rgsh2='<%=rgsh2%>';
      ...
      else
      {
        ...
        var curpage='<%=(curpage - 1)%>';
      		document.form1.action=path+"/web/sdttWeb/ShenpiManager/DjshT.jsp?rgsh="+rgsh+"&caiji="+rgsh2+"&curpage="+curpage;
        document.form1.submit();
      }
     }
     function nextpage()
     ...
     var rgsh2='<%=rgsh2%>';
      ...
      document.form1.action=path+"/web/sdttWeb/ShenpiManager/DjshT.jsp?rgsh="+rgsh+"&caiji="+rgsh2+"&curpage="+pageno;
      document.form1.submit();
     }
function exportad() {
	var usename = "<%=usename%>";
...
function daochu(){
	document.form1.action=path+"/servlet/PlDownload?bz=DJBGSP";
	document.form1.submit();
}
