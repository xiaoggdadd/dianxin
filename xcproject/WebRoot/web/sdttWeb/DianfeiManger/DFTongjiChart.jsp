...
<%@ page import="java.text.SimpleDateFormat,java.util.ArrayList,java.util.Date"%>
<%
	...
	String zdmc = request.getParameter("jzList")!=null?request.getParameter("jzList"):"0";
...
var path = "<%=path%>";
function changeChart(txt,pj,value){
...
//             },
            ...
            series: [{
                ...
                name: '平均值',
                data: [pj,pj,pj,pj,pj,pj,pj,pj,pj,pj,pj,pj]
            }]
        });
}

// $(function () {
// 	changeChart('[枣庄峄城大安村东口基站]2016年度电费数据分析图',12,[12,13,25,21,23,21,25,26,23,18,13,9]);
// });

function searchDianfeiChart(){
	var smc = $("#shi option:selected").text();
	var qxmc = $("#xian option:selected").text();
	var zdmc = $("#jzList option:selected").text();
	var zdid = document.form1.jzList.value;
	...
	$.ajax({
		...
		url:path+'/servlet/DianfeiFenxiServlet',
		dataType:'json',
		success:function(data) {
			var dataValue = [];
			var isAllNull = true;
			for(var i=0;i<data.value.length;i++){
				var value = data.value[i].beilv;
				dataValue[i] = Number(value);
				if(Number(value)!=0){
					isAllNull = false;
				}
			}
			...
			changeChart('['+smc+qxmc+zdmc+']'+year+'年度电费数据分析图',data.pjValue,dataValue);
		}
	});
}
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
	<tr valign="top">
		...
		<td>
			<div class="content01">
				<div class="tit01">2016年度各基站电费数据分析图</div>
					...
					<table width="100%" border="0" cellpadding="2" cellspacing="0" class="tb_select" >
						<tr >
							<td>
								...
								基站名称：
								<select name="jzList" id="jzList" style="width:160px;">
									<option value="0">请选择</option>
									<%
										SystemManage systemManage = new SystemManage();
										List<ZhandianBean> jzList = systemManage.searchZhandianZX(xian);
								...
								&nbsp;&nbsp;
								年度：<input type="text" id="year" name="year" value="<%=year %>" onFocus="WdatePicker({skin:'default',dateFmt:'yyyy'})" style="width:80px;"/>	
							</td>
						</tr>
						<tr>
							<td align="right">
								<input id="biaoganChartSubmit" name="biaoganChartSubmit" type="button" class="btn_c1"  value="输出图表" onclick="searchDianfeiChart()"/>&nbsp; 
							</td>
						</tr>
					</table>
...
function changeZd(){
	document.form1.action=path+"/web/sdttWeb/DianfeiManger/DFTongjiChart.jsp";
	document.form1.submit();
}

function updateZd(res){
	var jzlist = document.all.jizhan;
...
document.form1.jzList.value='<%=zdmc%>';
</script>