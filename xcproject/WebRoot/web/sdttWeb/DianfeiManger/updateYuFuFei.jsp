...
<%
	...
	String yearmonth="",dbbm="0",sfyz="",sh="0",xx="0",zdmc="0",xiaoqu="0",dbcode="0",zdbm="0";
	SiteModifyBean form = new SiteModifyBean();
	String danjia="",ftbl="",zdid="0";
	String dlid = request.getParameter("dlid")!=null?request.getParameter("dlid"):"0";
	String dfid=request.getParameter("dfid")!=null?request.getParameter("dfid"):"0";
	String bz = request.getParameter("bz")!=null?request.getParameter("bz"):"0";
	String  jfqsrq="",jfzzrq="",qm="",zm="",dl="",pjlx="",zdf="",ftje="",dj="";
	DecimalFormat de = new DecimalFormat("0.00");
	double a=0,b=0;
	DataBase db = new DataBase();
	if(bz!=null&&bz.equals("1")&&dlid!=null&&dfid!=null){
		String sql = "select STARTMONTHOLD,ENDMONTHOLD,LASTDEGREE,THISDEGREE,BLHDL from AMMETERDEGREES where AMMETERDEGREEID = '"+dlid+"'";
	    String sq = "select NOTETYPEID,ACTUALPAY,ACTUALPAY_D,UNITPRICE from ELECTRICFEES where ELECTRICFEE_ID = '"+dfid+"'";
	    ...
	    if(rs.next()){
	    	jfqsrq = rs.getString("STARTMONTHOLD")!=null?rs.getString("STARTMONTHOLD"):"";
	    	jfzzrq= rs.getString("ENDMONTHOLD")!=null?rs.getString("ENDMONTHOLD"):"";	 
	    	qm=rs.getString("LASTDEGREE");  
	    	zm=rs.getString("THISDEGREE");	
	    	dl=rs.getString("BLHDL");
	    }	 
	    rs1=db.queryAll(sq.toString());
	    while(rs1.next()){
	    	pjlx=rs1.getString("NOTETYPEID");
	    	zdf=rs1.getString("ACTUALPAY")!=null?rs1.getString("ACTUALPAY"):"";
	    	ftje=rs1.getString("ACTUALPAY_D")!=null?rs1.getString("ACTUALPAY_D"):"";
	    	dj=rs1.getString("UNITPRICE")!=null?rs1.getString("UNITPRICE"):"";
	    }   
	}
	if(zdf!=""&&zdf!=null&&!zdf.equals("")){
		a=Double.valueOf(zdf);
	}
	if(ftje!=null&&ftje!=""&&!ftje.equals("")){
		b=Double.valueOf(ftje);
	}
	String sql1="select zdid,dbbm from dianbiao where dbid=(select AMMETERID_FK from AMMETERDEGREES where AMMETERDEGREEID = '"+dlid+"')";
	ResultSet rs2 = null;
	db.connectDb();
	rs2=db.queryAll(sql1.toString());
	while(rs2.next()){
		zdid=rs2.getString("zdid");
		dbcode=rs2.getString("dbbm");
	}
	...
	zdbm = form.getJzcode();
	String aaa = "select dianxin from ftbl where dbbm='"+dbcode+"' and yearmonth='"+entrytime+"'";
	ResultSet rs3 = null;
	rs3=db.queryAll(aaa.toString());
	while(rs3.next()){
	ftbl=rs3.getString("dianxin");
	}
	%>
					...
					<table width="1000px" border="0" cellpadding="2" cellspacing="0" class="tb_select" >
						...
						<tr>
							...
							<td width="80px" align="left">
								<input type="text" name="t_jfqsrq" value='<%=jfqsrq %>' onfocus="WdatePicker({skin:'default',dateFmt:'yyyy-MM-dd'})"/>
							</td>
							<td align="right" width="80px">缴费终止日期</td>
							<td width="80px" align="left">
								<input type="text" name="t_jfzzrq" value='<%=jfzzrq %>' onfocus="WdatePicker({skin:'default',dateFmt:'yyyy-MM-dd'})" onchange="jfrqchange()"/>
							</td>
							<td align="right" width="80px">起码</td>
							<td width="80px">
								<input type="text" name="t_qm" value="<%=qm %>" class="selected_font"/>
							</td>
							<td align="right" width="80px">止码</td>
							<td width="80px">
								<input type="text" name="t_zm" value="<%=zm %>" class="selected_font" onchange="jisdl()"/>
							</td>
						</tr>	
						<tr>
							<td align="right" width="80px">电量</td>
							<td width="80px">
								<input type="text" name="t_dliang" value="<%=dl %>" class="selected_font"/>
							</td>
							<td align="right" width="80px">电费单价（元）</td>
							<td width="80px">
								<input type="text" name="t_dj" value="<%=dj %>" class="selected_font"/>
							</td>
							<td align="right" width="80px">缴费日期</td>
							<td width="80px">
								<input type="text" name="t_jfrq" value="<%=jfzzrq%>"/>
							</td>
						...
						<tr>
							...
							<td width="80px">
								<input type="text" name="t_yg" value="<%=de.format(a) %>" class="selected_font" onchange="ftje()"/>
							</td>
						</tr>
						<tr bgcolor="F9F9F9">
						...
						<tr>
							...
							<td width="80px">
								<input type="text"  name="t_ftje" value="<%=de.format(b) %>" class="selected_font" />
							</td>
						</tr>		
						<tr>
	...
	function saveAccount(){
        	...
        	function addzhandian(){
        		if(confirm("您将要添加或修改预付费信息！确认信息正确！")){      		
          		document.form1.action=path+"/servlet/YufufeiServlet?action=updateyff&bz="+'<%=bz%>'+"&dlid="+'<%=dlid%>'+"&&dfid="+'<%=dfid%>';
