...
function chaxun() {
	document.form1.action = path + "/web/sdttWeb/DianfeiManger/YuFuFeiQuery.jsp";
	document.form1.submit();
}
function accountOn() {
	var i = 0;
	...
	<tr valign="top">
		...
		<td>
			<div class="content01">
				...
				<div class="content01_1">
					...
					<div class="tbtitle01"><b>预付费一览</b></div>
						...
						String whereStr = "";
		                   ...
		                   list = bean.getYufufei(whereStr, usename, adminsname, curpage,ssagcode);//获取 用户的分页数据  ssagcode登录用户所属的区域 编码 
		                             allpage = bean.getAllPage();
											...
											double a=0,b=0,c=0;
											String qs="",zz="", dlid="", electricfee_id="",accountmonth="",jzname="",blhdl="",bg="",actualpay="";
										String color = "F3F3F3";
			                                  if(list!=null){
			                                  int countColum = ((Integer) list.get(0)).intValue();
											for (int i = countColum; i < list.size() - 1; i += countColum) {
												qs=(String) list.get(i + list.indexOf("STARTMONTHOLD"))==null?"":(String) list.get(i + list.indexOf("STARTMONTHOLD"));
												zz=(String) list.get(i + list.indexOf("ENDMONTHOLD"))==null?"":(String) list.get(i + list.indexOf("ENDMONTHOLD"));
												dlid=(String) list.get(i + list.indexOf("AMMETERDEGREEID"));
												electricfee_id=(String) list.get(i + list.indexOf("ELECTRICFEE_ID"));
												jzname=(String) list.get(i + list.indexOf("JZNAME"));
												accountmonth=(String) list.get(i + list.indexOf("ACCOUNTMONTH"));
												blhdl=(String) list.get(i + list.indexOf("BLHDL"));
												bg=(String) list.get(i + list.indexOf("BG"))==null?"0":(String) list.get(i + list.indexOf("BG"));
												actualpay=(String) list.get(i + list.indexOf("ACTUALPAY"));
												a=Double.valueOf(blhdl.toString());
												b=Double.valueOf(bg.toString());
												c=Double.valueOf(actualpay.toString());
												if (mobile == null)
													mobile = "";
       ...
       %>
									...
									<tr align="center">
										...
										<td align="left"><%=electricfee_id%></td>
										<td align="right"><%=accountmonth%></td>
										<%if(qs==""&&zz==""){ %>
										<td></td>
										<%}else{ %>
										<td align="center"><%=qs %>至<%=zz %></td>
										<%} %>
										...
										<td width="80">
											<a href="#" onclick="update('<%=dlid%>','<%=electricfee_id%>')"><img src="../images/accept.png" width="16" height="16"  title="修改" /></a>
											<a href="#" onclick="delzd('<%=dlid%>','<%=electricfee_id%>')"><img src="../images/delete.png" width="16" height="16"  title="删除" /></a>
										</td>
									</tr><% } %>
...
function daochu(){
	document.form1.action=path+"/servlet/PlDownload?bz=YFF";
	document.form1.submit();
}
function addjz(){
	window.open("../DianfeiManger/addYuFuFei.jsp", "newwindow", "height=500, width=1200, toolbar=no, menubar=no, scrollbars=no, resizable=no, location=no, status=no");  
}
function delzd(a,b){
	if(confirm("您确定删除此预付费信息？")){
		document.form1.action=path+"/servlet/YufufeiServlet?action=delete&dlid="+a+"&dfid="+b;
		document.form1.submit();
}
}
function update(a,b){
	window.open("../DianfeiManger/updateYuFuFei.jsp?bz=1&&dlid="+a+"&&dfid="+b, "newwindow", "height=500, width=1200, toolbar=no, menubar=no, scrollbars=no, resizable=no, location=no, status=no");  
}
