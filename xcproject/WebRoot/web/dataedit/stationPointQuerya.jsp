...
<%
	...
	String zdlx1 = request.getParameter("zdlx1") != null ? request
			.getParameter("zdlx1") : "";
	String entrytimeQS = request.getParameter("entrytimeQS") != null ? request
			.getParameter("entrytimeQS")
			: "";//开始录入时间
	String entrytimeJS = request.getParameter("entrytimeJS") != null ? request
			.getParameter("entrytimeJS")
			: "";//结束录入时间         
	String sydateks = request.getParameter("sydateks") != null ? request
			.getParameter("sydateks")
			: "";//投入使用开始日期
	String sydatejs = request.getParameter("sydatejs") != null ? request
			.getParameter("sydatejs")
			: "";//投入使用结束日期
	String entrypensonnel = request.getParameter("entrypensonnel") != null ? request
			.getParameter("entrypensonnel")
			: "";//录入人员
	...
	map = (HashMap) se.getAttribute("lst");
...
function delLogs() {
	...
	if (checkNotnull(document.form1.delBeginTime, "开始时间")
        ...
        if(lie!=27){
         ...
         for(var i =1 ;i<=allselect;i++){
        	...
        	//alert(arr2.length);
        	//var atest = "aaaa";
        	//dataUpdateService.test(arr2);
        	dataUpdateService.dataSave(arr1,arr2,callback); 
        	//alert(i2);
        	//var mes=chestr+";"+checkid;                
        	//向servlet中传值                
        	//编码encodeURI()向servlet中传值中文乱码的问题解决方式    
        	//var url =path+"/servlet/dataUpdate?arr1="+arr1;     
        	//alert(url);
  ...
  function qk(){
  	if (confirm("您确定清空数据？")) {
  document.form1.action=path+"/servlet/DeleteId?action=del";
 document.form1.submit();
  }
  }
   $(function(){
			...
			<div
				...
				<table width="2300px" height="60%" border="0" cellspacing="1"
					...
					<%
						...
						if (map != null) {
							...
							while (iter.hasNext()) {
								...
								rru = zd.getRru();
								ydgxsb = zd.getYdgxsbsl();
								dxgxsb = zd.getDxgxsbsl();
					...
					<tr bgcolor="<%=color%>">
						...
						<td>
							<select class="selected_font" id="zdcq" name="zdcq">
								<%
									if (zdcqaa != null) {
												String code = "", name = "";
												int cscount = ((Integer) zdcqaa.get(0)).intValue();
												for (int i = cscount; i < zdcqaa.size() - 1; i += cscount) {
													code = (String) zdcqaa.get(i
															+ zdcqaa.indexOf("CODE"));
													name = (String) zdcqaa.get(i
															+ zdcqaa.indexOf("NAME"));
								%>
								<%
									if (zdcq.equals(name)) {
						...
						<td>
							<input type="text" id="kd<%=countxh%>" name="kd" value="<%=d%>"
								class="selected_font" />
						</td>
						<td>
							<input type="text" id="yy<%=countxh%>" name="yy" value="<%=y%>"
								class="selected_font" />
						</td>
						<td>
							<input type="text" id="kty<%=countxh%>" name="kty"
								value="<%=kty%>" class="selected_font" />
						</td>

						<td>
							<input type="text" id="kte<%=countxh%>" name="kte"
								value="<%=kte%>" class="selected_font" />
						</td>
						...
						<td>
							<select class="selected_font">
								<%
									if (gxxxaa != null) {
												String code = "", name = "";
												int cscount = ((Integer) gxxxaa.get(0)).intValue();
												for (int i = cscount; i < gxxxaa.size() - 1; i += cscount) {
													code = (String) gxxxaa.get(i
															+ gxxxaa.indexOf("CODE"));
													name = (String) gxxxaa.get(i
															+ gxxxaa.indexOf("NAME"));
								%>
								<%
									if (gxxx.equals(name)) {
						...
						<td>
							<input type="text" id="rru<%=countxh%>" name="rrt"
								value="<%=rru1%>" class="selected_font" />
						</td>

						<td>
							<input type="text" id="yd<%=countxh%>" name="yd" value="<%=yd1%>"
								class="selected_font" />
						</td>

						<td>
							<input type="text" id="dx<%=countxh%>" name="dx" value="<%=dx1%>"
								class="selected_font" />
						</td>
								...
								<%
									if (zdsxaa != null) {
												...
												for (int i = cscount; i < zdsxaa.size() - 1; i += cscount) {
													code = (String) zdsxaa.get(i
															+ zdsxaa.indexOf("CODE"));
													name = (String) zdsxaa.get(i
															+ zdsxaa.indexOf("NAME"));
								%>
								<%
									if (zdsx.equals(name)) {
...
function countzf01(num) {
	...
	jlbl.value = ((((Number(jlfh.value) * 220 * 24 / 1000) - (Number(edhdl.value))) / ((Number(edhdl.value)))) * 100)
			.toFixed(2)
			+ "%";
	Number(zlfh.value) * 54 * 24 / 1000 / 0.85 //直流电量
	zlbl.value = ((((Number(zlfh.value) * 54 * 24 / 1000 / 0.85) - (Number(edhdl.value))) / (Number(edhdl.value))) * 100)
			.toFixed(2)
			+ "%";
}
</script>
