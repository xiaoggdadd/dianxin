...
function getDictList(type,maxrow,minrow){
	...
	if (type==1){
	if(document.form1.yiji.value=="0"){
	  var sz="1";
	}else{
	 var sz="2";
	}
	 Url = getPublicPath()+"/servlet/Quanchengben1?action=ajaxareaData";
	  params ="&fqcbcode="+ document.getElementById("yiji").value+"&sz="+sz;
	}else if (type==2){
	sz="2";
	  Url = getPublicPath()+"/servlet/Quanchengben1?action=ajaxareaData";
	  params ="&fqcbcode="+ document.getElementById("erji").value+"&sz="+sz;
	}else if (type==3){
		  Url = getPublicPath()+"/servlet/Quanchengben1?action=ajaxareaData";
	      params ="&fqcbcode="+ document.getElementById("sanji").value;
	}else{
       sz="1";
	  Url = getPublicPath()+"/servlet/Quanchengben1?action=select";
	  params ="&fqcbcode=0";
	 				 ...
	 				 if (rowDate!=""){
		 				  for(var i=0;i<rowDate.length;i++){
			 			    	...
			 			    	curRow = cellDate[5];
				 			     ...
				 			     var url="'"+"quanchengbenupdate1.jsp?qcbCode="+cellDate[0]+"&fqcbCode="+cellDate[3]+"&idd="+cellDate[6]+"&bzw="+cellDate[2]+"'";
...
function getSelOpen(type){
  	var Url = getPublicPath()+"/servlet/Quanchengben1?action=selOption";
	var params ="&key=value";
	if (type==1){
	var sz="2";
	  params ="&fqcbcode="+ document.getElementById("yiji").value+"&sz="+sz;
...
//增加数据
 ...
 //  var yibanzuTxt = sanji.options[i].text;
  var fatherAreaName = "";
   if (gongsiVal==0 && bumenVal == 0 ){
     fatherAreaName = "1";
     fatherCode = "0";
   }else if (gongsiVal!=0 && bumenVal == 0){
      fatherAreaName = "2";
    ...
    window.open("quanchengbenadd1.jsp?fatherAreaName="+fatherAreaName+"&fatherCode="+fatherCode, 'newwindow', 'height=500, width=750,top=0,left=200,toolbar=no,menubar=no, scrollbars=no, resizable=no,location=n o, status=no');
 } 
   function open1(url){																						
...
//
   function del(id){
     if(confirm("确定删除")){
       var params = "?&id="+id;
       var Url=getPublicPath()+"/servlet/Quanchengben1?action=del";
	    new Ajax.Request(Url,{
	      		method : 'post',
				asynchronous: false,
			    parameters : params,
...
function getDate(){
  ...
  // var quxianTxt = sanji.options[i].text;
   ...
   var fatherAreaName = "";
   if (shengVal==0 && shiVal == 0){
     fatherAreaName = "";
   }else if (shengVal!=0 && shiVal == 0){
      fatherAreaName = "1";
   }else if (shengVal!=0 && shiVal != 0){
      fatherAreaName = "2";
   }
						...
						<td colspan="4">
					    	...
					    	<td colspan="4" width="50" >
											 <div style="width:700px;height:50px">
											  ...
											  <span style="font-size:14px;font-weight:bold;position: absolute;left:25px;top:15px;color:black">专业明细管理</span>	
												</div>
											</td>
