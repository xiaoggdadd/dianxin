...
function getDictList(type,maxrow,minrow){
	...
	if (type==1){
	 Url = getPublicPath()+"/servlet/FinanceSubject?action=ajaxareaData";
	  ...
	  Url = getPublicPath()+"/servlet/FinanceSubject?action=select";
	  params ="&fkmcode=0";
	 				 ...
	 				 if (rowDate!=""){
		 				  for(var i=0;i<rowDate.length;i++){
			 			    	...
			 			    	curRow = cellDate[5]
				 			     ...
				 			     var url="'"+"financesubjectupdate.jsp?kmCode="+cellDate[0]+"&fkmCode="+cellDate[2]+"'";
...
function getSelOpen(type){
  	var Url = getPublicPath()+"/servlet/FinanceSubject?action=selOption";
	var params ="&key=value";
	if (type==1){
	  params ="&fkmcode="+ document.getElementById("yiji").value;
	}else if (type==2){
...
//
   function del(id){
     if(confirm("确定删除")){
       var params = "?&kmCode="+id;
       var Url=getPublicPath()+"/servlet/FinanceSubject?action=del";
	    new Ajax.Request(Url,{
	      		method : 'post',
				asynchronous: false,
			    parameters : params,
...
$(function(){
  ...
  <body  class="body" style="overflow-x:hidden;" onload="waitThenDoIt();">
	...
	<div style="position:relative;height:30px;width:800px;top:0px;">
  		...
  		<span style="position:absolute;font:微软雅黑;font-size:16px;height:30px;left:50px;top:20px;"><b>会计科目管理</b></span>
  	</div>
