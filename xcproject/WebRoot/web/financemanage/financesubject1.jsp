...
function getDictList(type,maxrow,minrow){
   var sz="";
	...
	if (type==1){
	 ...
	 Url = getPublicPath()+"/servlet/FinanceSubject1?action=ajaxareaData";
	  params ="&fkmcode="+ document.getElementById("yiji").value+"&sz="+sz;
	}else if (type==2){
		sz="1";
	  Url = getPublicPath()+"/servlet/FinanceSubject1?action=ajaxareaData";
	  params ="&fkmcode="+ document.getElementById("erji").value+"&sz="+sz;
	}else if (type==3){
		  Url = getPublicPath()+"/servlet/FinanceSubject1?action=ajaxareaData";
	      params ="&fkmcode="+ document.getElementById("sanji").value;
	}else{
      var sz="1";
	  Url = getPublicPath()+"/servlet/FinanceSubject1?action=select";
	  params ="&fkmcode=0"+"&sz="+sz;
				  ...
				  RemoveItems(dataType);
	 				 ...
	 				 if (rowDate!=""){
		 				  for(var i=0;i<rowDate.length;i++){
				 			  ...
				 			  //   alert("idd:"+cellDate[6]);
				 			   //  alert("bzw:"+cellDate[2]);
				 			     //var url="'"+"quanchengbenupdate.jsp?qcbCode="+cellDate[0]+"&fqcbCode="+cellDate[3]+"&idd="+cellDate[6]+"&bzw="+cellDate[2]+"'";-->
				 			     var url="'"+"financesubjectupdate1.jsp?qcbCode="+cellDate[0]+"&fqcbCode="+cellDate[3]+"&idd="+cellDate[6]+"&bzw="+cellDate[2]+"'";
                                 var str ="<a href=\"javascript:open1(" + url + ")\">修改</a>";  
				 			     ...
				 			     var str ="<a href=\"javascript:del(" + cellDate[6] + ")\">删除</a>";
...
function getSelOpen(type){
  	var Url = getPublicPath()+"/servlet/FinanceSubject1?action=selOption";
	var params ="&key=value";
	...
	document.getElementById("erji").value="0";
	}else{
	var sz="1";
	  params ="&fkmcode="+ document.getElementById("erji").value+"&sz="+sz;
	  }
...
//增加数据
 function add(){
   ...
   if (gongsiVal==0 && bumenVal == 0 && yibanzuVal ==0){
    ...
    window.open("financesubjectadd.jsp?fatherAreaName="+fatherAreaName+"&fatherCode="+fatherCode, 'newwindow', 'height=500, width=750,top=0,left=200,toolbar=no,menubar=no, scrollbars=no, resizable=no,location=n o, status=no');
 } 
   function open1(url){
...
//
   function del(id){
     if(confirm("确定删除")){
       var params = "?&idde="+id;
       var Url=getPublicPath()+"/servlet/FinanceSubject1?action=del";
	    new Ajax.Request(Url,{
	      		method : 'post',
				asynchronous: false,
			    parameters : params,
	...
	<tr >
					    	...
					    	<td colspan="4" width="50" >
											 <div style="width:700px;height:50px">
											  ...
											  <span style="font-size:14px;font-weight:bold;position: absolute;left:25px;top:15px;color:black">全成本科目管理</span>	
												</div>
											</td>
