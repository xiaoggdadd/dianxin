...
<%
	...
	String qrzt = request.getParameter("qrzt")!=null?request.getParameter("qrzt") : "0";
	String ids = request.getParameter("ids");
	System.out.println(">>>>>>>>>>>>>>>>>"+ids);
	WyManage wyManage = new WyManage();
	if(ids!=null){
		if(ids.length()>22){
			ids = ids.substring(18, ids.length()-4);
			wyManage.updateWydfft_yqr(ids);
		}else{
			wyManage.updateWydfft_yqr(ids);
		}
	}
...
<script language="javascript">
	...
	function chaxun() {
		document.form1.action = path + "/web/wyftdfsh/tt_qxdfftqr_search.jsp";
	...
	function querenjn(){
		...
		if (i > 0) {
			document.form1.action = path + "/web/wyftdfsh/tt_qxdfftqr_search.jsp?ids='"+ids+"'";
			document.form1.submit();
		} else {
			alert("请选择要确认的条目");
			return;
	...
	function exportad() {
		document.form1.action = path+ "/servlet/QxdfftqrDownload";
		document.form1.submit();
	}
	...
	$(function() {
		...
		$("#xiugaiBtn").click(function(){
			...
			if (i > 0) {
				document.form1.action = path + "/web/wyftdfsh/tt_qxdfftqr_update.jsp?id='"+_id+"'";
				document.form1.submit();
			} else {
				alert("请选择要删除的条目");
				return;
...
function nextpage() {
	...
	document.form1.action = path + "/web/wyftdfsh/tt_qxdfftqr_search.jsp?curpage="
			+ curpage;
	document.form1.submit();
}
...
function gopagebyno(pageno) {
	...
	document.form1.action = path + "/web/wyftdfsh/tt_qxdfftqr_search.jsp?curpage=" + pageno;
	document.form1.submit();
}
</script>
...
<body class="body" style="overflow-x:hidden;">
			...
			<tr>
				<td colspan="4">
					<div style="width:700px;height:50px">
						<span
							style="font-size:14px;font-weight:bold;position: absolute;left:25px;top:15px;color:black">区县电费分摊确认</span>
					</div></td>
			</tr>
			<tr>
 ...
 %>
		...
		<table width="2000px" height="60%" border="0" cellspacing="1" cellpadding="1" bgcolor="#cbd5de" class="form_label">
			...
			<%
				List<WydfftBean> wydfftList = wyManage.searchWydfft_fy(loginName,loginId,pageSize,curpage,sheng,shi,xian,xiaoqu,"已核对",qrzt,pc,zdbm,dh,cy);
				allpage = wyManage.searchWydfft_fy(loginName,loginId,9999999,1,sheng,shi,xian,xiaoqu,"已核对",qrzt,pc,zdbm,dh,cy).size()/pageSize+1;
			...
			<tr bgcolor="<%=color%>">
				...
				<td>
					<div align="center"><%=wydfft.getQRZT()%></div></td>
				<td>
