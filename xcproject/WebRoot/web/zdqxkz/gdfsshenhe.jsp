...
function ShowHideSearchRegion(trObject,SelfObject){
    ...
    function queryDegree(){
              document.form1.action=path+"/servlet/GdfServlet?command=chaxun";
	...
	<table width="100%"  border="0" cellspacing="0" cellpadding="0" height="18%">
		<tr>
			...
			<div style="width:700px;height:50px">
			       ...
			       <span style="font-size:14px;font-weight:bold;position: absolute;left:25px;top:15px;color:black">省申请供电方式审核</span>	
			 </div>
		    </td>
			</tr>		
		    ...
		    <table>
				...
				<td>
 				...
 				<td><select name="sqnr" class="selected_font" >
	         		...
	         		<option value="0">由直供改转供</option>
	         		<option value="1">由转供改直供</option>
	         		</select>
	         	</td>	
         		<td>审核状态：</td>
		 ...
		 <c:forEach items="${list}" var="list" varStatus="status">
		  <tr  bgcolor="${(status.count)%2==0?'#DDDDDD':'#FFFFFF'}" title="">
            ...
            <td>
       		...
       		<td>
       			<div align="center" >${list.oldg}</div>
       		</td>
       		<td>
       			<div align="center" >${list.newg}</div>
       		</td>
       		<td>
       			<div><%--<input type="file" id="fileUp1" name="fileUp1" height="19px" width="130px" onchange="fileChange(this);"/>--%>
					<c:choose>
						...
						<a href="#" onclick="modifyjz2('${list.id}','3');return false;">
				...
				if (bzw1 == "1") {
					if (count1 <= 120 * n
						...
						document.form1.action = path
								+ "/servlet/GdfServlet?command=tongguo1&chooseIdStr="
					...
					if (bzw1 == "1") {
						if (count1 <= 120 * n || ((bz) >= 119 && (bz) <= 121)) {
							if (((bz) / 120 == 1)|| ((bz) >= 119 && (bz) <= 121)) {
					          ...
					          chooseIdStr1=chooseIdStr1.substring(0,chooseIdStr1.length-1);
						      bzw=0;
						      document.form1.action=path+"/servlet/GdfServlet?command=butongguo1&chooseIdStr="+chooseIdStr+"&chooseIdStr1="+chooseIdStr1+"&cause="+cause;
					          document.form1.submit(); 
						}
					} else if (bzw1 == "0") {
						document.form1.action = path + "/web/msg.jsp";
		...
		for ( var i = 0; i < l; i++) {
			...
			if (bzw1 == "1") {
				if (count1 <= 120 * n
					...
					bzw = 0;
					document.form1.action = path
							+ "/servlet/GdfServlet?command=chehui1&chooseIdStr="
							+ chooseIdStr + "&chooseIdStr1=" + chooseIdStr1
							;
					document.form1.submit();
				}
