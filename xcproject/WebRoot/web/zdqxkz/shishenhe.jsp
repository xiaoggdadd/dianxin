...
<%
	...
	String whosh = request.getParameter("whosh");//市审核还是省审核
...
String id="";//id
String shishzt = "",bftg="",bfbtg="";
boolean shbz=false;//能不能修改标识
	//获取站点信息
	if(idcz!=null){
        	// 站点信息Z
        	List<Zdqxkz> ls=null;
        	...
        	if("sheng".equals(whosh)){//省
        		bftg = lst.getBftg();
        		bfbtg = lst.getBfbtg();
        		shbz = bftg.contains("9") || bfbtg.contains("9");//省标审核过
        	}else{//市
        	shishzt	= lst.getShishbz();
        	shbz ="1".equals(shishzt) || "2".equals(shishzt);//通过或不通过
        	}
		...
		<form action="" name="form1" method="POST" id="form" enctype=multipart/form-data>
			<table border="0" width="100%">
				<tr>
					<td width="100%" scope="2">
						<table width="100%" border="0" cellspacing="1" cellpadding="1">
							...
							<td bgcolor="#DDDDDD" class="form_label">
											...
											<td>
												<input type="text" name="newqyzt" readonly="true"
											...
											<td>
												<input type="text" name="oldqsdb"  readonly="true"
											...
											<td>
												<input type="text" name="newqsdb" 
													value="<%=newqsdbdl%>" class="form31" style="color: red"/>
											</td>
										<!--  	<td bgcolor="#DDDDDD" class="form_label">
												<div>
													全省定标电量：
					...
					<td align="right">
					<%if(shbz){
							if("sheng".equals(whosh)){%>
						<div style="width:160px;height:23px;cursor:pointer;float:right;position: relative; right:-150px; top: 0px">
						 <span style="font-size: 12px; position: absolute; left: 35px; top: 5px; color: red">省标已审核,不允许修改</span>
						</div>
							<%
							}else{%>
						<div style="width:160px;height:23px;cursor:pointer;float:right;position: relative; right:-150px; top: 0px">
						 <span style="font-size: 12px; position: absolute; left: 35px; top: 5px; color: red">市级已审核,不允许修改</span>
						</div>
							<% 
							}
					%>
...
<script language="javascript">
	...
	var newqyzt = document.form1.newqyzt.value;
		if("否"==newqyzt){//站点启用状态为否
			if(parseFloat(qsdb)==qsdb&&isNaN(qsdb)==false&&parseFloat(qsdb)>=0){
				var ggbz = Number(oldqsdb)==Number(qsdb)?"1":"2";//1:未改,2改
				document.form1.action=path+"/servlet/zdqxcxxg?action=zdqxsjsh&ggbz="+ggbz+"&id="+idd+"&qsdb="+qsdb;
				document.form1.submit();
			}else{
