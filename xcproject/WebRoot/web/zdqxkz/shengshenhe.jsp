...
function ShowHideSearchRegion(trObject,SelfObject)
{
  ...
  function chaxun(){
   ...
   function queryDegree(){
              document.form1.action=path+"/web/zdqxkz/shengshenhe.jsp?command=chaxun";
              document.form1.submit();
    } 
//审核不通过
 function butongguo() {
			...
			if (bzw1 == "1") {
				if (count1 <= 120 * n || ((bz) >= 119 && (bz) <= 121)) {
					if (((bz) / 120 == 1)|| ((bz) >= 119 && (bz) <= 121)) {
			          ...
			          chooseIdStr1=chooseIdStr1.substring(0,chooseIdStr1.length-1);
				      bzw=0;
				      document.form1.action=path+"/servlet/ShenHe?action=butongguo&chooseIdStr="+chooseIdStr+"&chooseIdStr1="+chooseIdStr1+"&cause="+cause;
			          document.form1.submit(); 
			          showdiv("请稍等..............");
				}
			} else if (bzw1 == "0") {
			...
			if (bzw1 == "1") {
				if (count1 <= 120 * n
					...
					document.form1.action = path
							+ "/servlet/ShenHe?action=chehuicc&chooseIdStr="
			...
			if (bzw1 == "1") {
				if (count1 <= 120 * n
					...
					if (((bz + count2) / 120 == 1)
						...
						chooseIdStr1 = chooseIdStr1.substring(0,
								chooseIdStr1.length - 1);
						chehuic(chooseIdStr, chooseIdStr1);
						chooseIdStr = "";
						chooseIdStr1 = "";
						bz = 0;
						count2 = 0;	          	
					...
					bzw = 0;
					document.form1.action = path
							+ "/servlet/ShenHe?action=chehuia&chooseIdStr="
							+ chooseIdStr + "&chooseIdStr1=" +chooseIdStr1+"&lg="+lg
							;
					document.form1.submit();
				}
			} else if (bzw1 == "0") {
...
function butongguo1(chooseIdStr, chooseIdStr1,cause) {
	sendRequest1(path + "/servlet/ShenHe?action=butongguo1&chooseIdStr="+ chooseIdStr + "&chooseIdStr1=" + chooseIdStr1+"&cause="+cause, "checkcityno2");
}
   //审核通过
function chehuic(chooseIdStr, chooseIdStr1) {
	sendRequest1(path + "/servlet/ShenHe?action=chehuia1&chooseIdStr="
			+ chooseIdStr + "&chooseIdStr1=" + chooseIdStr1, "checkcityno1");
}

    //取消审核（撤回）
function chehuiqx(chooseIdStr, chooseIdStr1) {
	sendRequest1(path + "/servlet/ShenHe?action=chehuicc1&chooseIdStr="
		...
		<table width="100%" height="80%"  border="0" cellspacing="1" cellpadding="1" bgcolor="#cbd5de" class="form_label">
			<tr height = "23" class="relativeTag">
				...
				<td width="6%" height="23" bgcolor="#DDDDDD"><div align="center" class="bttcn"><input type="checkbox" name="test" onclick="chooseAll()"/>全选</div></td>
                 ...
                 <td width="8%" height="23" bgcolor="#DDDDDD"><div align="center" class="bttcn">审核状态</div></td>
                 <td width="8%" height="23" bgcolor="#DDDDDD"><div align="center" class="bttcn">审核时间</div></td>
...
// com.noki.zdqxkz.dao
        ...
        String zdid="",zdnamea="",shi1="",xian1="",xiaoqu1="",shengshsj="",shengshzt="",shengshzt1="",iid="";
        String wjid="";String idd="",dbid="",dj="",noreason;
		if("chaxun".equals(request.getParameter("command"))){
		 fylist=bean.seachSheng(whereStr,loginId);
       ...
       <td>
       		...
       		<td>
              <div align="center"><input type="checkbox" name="test[]" value="<%=iid%>" /><input type="hidden" type="checkbox" name="test1[]" value="<%=zdid%>"/><input type="hidden" type="checkbox" name="test2[]" value="<%=shengshzt1%>" /></div>
            </td>
           <td>
		...
		<!--   <tr bgcolor="F9F9F9">
	             ...
	             -->
	               ...
	               <td align="right">
				        ...
				        <div id="ch" style="width: 57px; height: 23px; cursor: pointer; float: right; position: relative; right: 30px">
...
<script type="text/javascript">
    function lookDetailssz(wjid){ 
	 ...
	 var path='<%=path%>';   
	 window.open(path+"/web/zdqxkz/shishenhe.jsp?whosh=sheng&zdid="+zdid,'','width=1230,height=500,status=yes,scrollbars=yes,resizable=yes,left=100,top=100')	
    }
</script>
<script type="text/javascript">
<!--
