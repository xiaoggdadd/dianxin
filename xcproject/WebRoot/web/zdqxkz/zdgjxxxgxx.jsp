...
<%
	...
	String dbdj;//电表旧单价
	String dfzflxlast = "";//电费支付类型
	String olddbds = "";//电表修改读数(老)
	String oldxgbzw = "";//电表读数修改标识位
	//省级审核
	...
	String bzw="";//标志位 1核实标杆站点 0为非核实标杆站点
	
	double d,dd;
	double qsdb;
	String qd="",scb="";
	String zdidcz = request.getParameter("zdid") != null ? request.getParameter("zdid"): "";//前台传值唯一标识zdid
	String dbid=request.getParameter("dbid")!=null?request.getParameter("dbid"):"";
	// bzw=request.getParameter("bzw")!=null?request.getParameter("bzw"):"0";//标志位 1核实标杆站点 0为非核实标杆站点
	 System.out.println("bzw:"+bzw);
	Zdqxcxxg bean = new Zdqxcxxg();
	System.out.println("---------------------"+zdidcz);
	...
	if(zdidcz!=null){
        ...
        jcnh jc=new jcnh();
        String bz = "1";
        if(request.getParameter("bz1") == "1"||"1".equals(request.getParameter("bz1"))){
        	bz = bean.getZdqxcxxgxxsfcf(zdidcz);
        }
System.out.println("电表ID:"+dbid);
        zdinfo = bean.getZdqxcxxgxx(zdidcz,dbid);
        zdinfoa=bean.getZdqxcxxgxxgx(zdidcz);
        String[] hh = bean.getZggd(zdidcz);
        String gdscb = hh[1];//工单生产标
        String zggdid = hh[0];//整改工单id
        jc=bean.getZdqxkzbl(zdidcz);//能耗基础值  实际值  建议生产标
        if(jc!=null){
        	nhjc=jc.getNhjcz();
    		sjz=jc.getSjz();
    		jyscb=jc.getJyscb();
        ...
        if(zdinfo!=null){
			...
			olddbds = zdinfo.getOlddbds();
			oldxgbzw = zdinfo.getOldxgbzw();
			...
			zlfhlast = zdinfo.getOldzlfh();//交流负荷
			zlfhlast = zlfhlast != null ? zlfhlast : "0";
			if("".equals(zlfhlast)){zlfhlast="0";}
			
			jlfhlast = zdinfo.getOldjlfh();//直流负荷
			jlfhlast = jlfhlast != null ? jlfhlast : "0";
			if("".equals(jlfhlast)){jlfhlast="0";}
			
			jlfhlast=dj.format(Double.parseDouble(jlfhlast));
			zlfhlast=dj.format(Double.parseDouble(zlfhlast));
			d=Double.parseDouble(zlfhlast);
			d=d*54*24/1000/0.85;
			dd=Double.parseDouble(jlfhlast);
            dd=dd*220*24/1000;
            if("0".equals(zlfhlast)&&!"0".equals(jlfhlast)){
             qsdb=dd;
            }else if(!"0".equals(zlfhlast)&&"0".equals(jlfhlast)){
            qsdb=d;
            }else if("0".equals(zlfhlast)&&"0".equals(jlfhlast)){
            qsdb=0;
            }else{
            ...
            qd=th.format(qsdb);		
		//  Number(zl*54*24/1000/0.85).toFixed(2);
		// Number(jl*220*24/1000).toFixed(2);
			...
			//原生产标
	scb=zdinfoa.getScb();
			if("".equals(scb)||null==scb){
				scb="0";
			}
			scb=th.format(Double.parseDouble(scb));
			...
			qsdbdl = zdinfo.getOldqsdbdl();//全省定标电量
			...
			xian = zdinfo.getXian();//县
			xian = xian != null ? xian : "";
			
			xiaoqu = zdinfo.getXiaoqu();//小区
			xiaoqu = xiaoqu != null ? xiaoqu : "";
			
			zdname = zdinfo.getZdname();//站点名称
			zdname = zdname != null ? zdname : "";
			diquzdid =  shi+xian+xiaoqu+"--";	//所在地区的站点		
%>
<html>
	<head>
...
<script language="javascript">
	...
	function shangchuan(){
		...
		var olddbds = document.form1.olddbds.value;
		var oldxgbzw = document.form1.oldxgbzw.value;
		...
		var oldqsdb= document.form1.oldqsdb.value;
		...
		var nhjc=document.form1.nhjc.value;//站点核标标志位
			var sjz=document.form1.sjz.value;//站点核标标志位
				...
				var nh=((Number(nhjc)-Number(qsdb))/Number(qsdb))*100;
				var nhbl=nh.toFixed(2);
				var sj=((Number(sjz)-Number(qsdb))/Number(qsdb))*100;
				var sjbl=sj.toFixed(2);
				var jy=((Number(jyscb)-Number(qsdb))/Number(qsdb))*100;
				...
				var zldl=((Number(zl)-Number(qsdb))/Number(qsdb))*100;
				var jldl=((Number(jl)-Number(qsdb))/Number(qsdb))*100;
				var zlbl=zldl.toFixed(2);var jlbl=jldl.toFixed(2);
				// alert("直流电量:"+zl+"交流电量:"+jl);
		 var lg='<%=accountid%>';
		...
		if(checkNotnull(document.form1.zt,"标题状态")&&checkNotSelected(document.form1.zt,"标题状态")
			...
			&&checkNotnull(document.form1.zdsxnow,"站点属性")
			&&checkNotSelected(document.form1.dfzflxnow,"电费支付类型")&&checkNotSelected(document.form1.zdsxnow,"站点属性")
			...
			&&checkNotnull(document.form1.zdareanow,"站点面积")&&checkNotnull(document.form1.qyztnow,"站点启用状态")
			// &&checkNotnull(document.form1.newqsdbdl,"全省定标电量")
			&&checkNotnull(document.form1.qsdb,"省定标(不含空调)")
	        			...
	        			if(parseInt(rrunow)==rrunow&&isNaN(rrunow)==false){
	        				if(parseInt(ydshebeinow)==ydshebeinow&&isNaN(ydshebeinow)==false){
	        					if(parseInt(gxgwslnow)==gxgwslnow&&isNaN(gxgwslnow)==false){
	        					   if(parseFloat(dianjianow)==dianjianow&&isNaN(dianjianow)==false&&parseFloat(dianjianow)>0){
	        					   		if(qyztAndEdhdl()){
	        					   			if(parseFloat(zlfhnow)==zlfhnow&&isNaN(zlfhnow)==false&&parseFloat(zlfhnow)>0){
	        					   				if(parseFloat(jlfhnow)==jlfhnow&&isNaN(jlfhnow)==false&&parseFloat(jlfhnow)>0){
	        					   					if(parseFloat(zdareanow)==zdareanow&&isNaN(zdareanow)==false){
	        					   						if(quztAndScb()){
	        					   							if(isNaN(dbds)==false&&Number(dbds)>=0){
	        					   						//市级审核标志判断	
	        					   						var	bzw = "0";        					   							
														if(
																(sfbglast!= sfbgnow)||(Number(dianjialast)!= Number(dianjianow))||(Number(edhdllast)!= Number(edhdlnow))||(Number(zlfhlast)!= Number(zlfhnow))||(Number(jlfhlast)!= Number(jlfhnow))||(dfzflxlast !=dfzflxnow)||dbds!=""
														  ){
															bzw1 = "1";
															}
														//省级审核标志判断
														if((zdsxlast!= zdsxnow)||(zdlxlast!= zdlxnow)||(yflxlast!= yflxnow)||(gxxxlast!= gxxxnow)||(gdfslast!= gdfsnow)||(zgdlast!= zgdnow)
														||(Number(zdarealast)!= Number(zdareanow))||(Number(oldqsdb)!=Number(qsdb))||(qyztlast!= qyztnow)||(g2last!= g2now)||(g3last!= g3now)||(zplast!= zpnow)||(zslast!= zsnow)
														...
														//如果市级有申请，省级没有申请 不到省级审核；如果省级有申请，市级有没有申请都要到市级审核
														if(bzw1=="1"&&bzw2=="0"){
															bzw = "1";
														...
														(changjialast== changjianow)&&(jztypelast== jztypenow)&&(shebeilast== shebeinow)&&(bbulast== bbunow)&&(rrulast== rrunow)&&
														(ydshebeilast== ydshebeinow)&&(gxgwsllast== gxgwslnow)&&(dfzflxlast== dfzflxnow)&&(Number(oldqsdb)>Number(qsdb))
														...
														(Number(zdarealast)== Number(zdareanow))&&(qyztlast== qyztnow)&&(g2last== g2now)&&(g3last== g3now)&&(zplast== zpnow)&&(zslast== zsnow)&&
														(changjialast== changjianow)&&(jztypelast== jztypenow)&&(shebeilast== shebeinow)&&(bbulast== bbunow)&&(rrulast== rrunow)&&
														(ydshebeilast== ydshebeinow)&&(gxgwsllast== gxgwslnow)&&(dfzflxlast== dfzflxnow)&&(Number(oldqsdb)>Number(qsdb))&&((Number(zlfhlast)!= Number(zlfhnow))||(Number(jlfhlast)!=Number(jlfhnow)))
													...
													alert("能耗基础值占比:"+Math.abs(nhbl)+"%"+"\n"+"实际值占比:"+Math.abs(sjbl)+"%"+"\n"+"建议生产标占比:"+Math.abs(jybl)+"%"+"\n"+"直流电量占比:"+Math.abs(zlbl)+"%"+"\n"+"交流电量占比:"+Math.abs(jlbl)+"%"+"\n"+"能耗基础值:"+Number(nhjc).toFixed(2)+" 实际值:"+Number(sjz).toFixed(2)+" 建议生产标:"+Number(jyscb).toFixed(2)+"交流电量:"+Number(jl).toFixed(2)+"直流电量"+Number(zl).toFixed(2));
														if(bzw != "0"){
														   ...
														   if(confirm("您将要添加信息！确认信息正确！")){
														         document.form1.action=path+"/servlet/zdqxcxxg?action=upload&zdid="+zdid+"&sfbglast="+sfbglast+"&sfbgnow="+sfbgnow
														         ...
														         +"&zdarealast="+zdarealast+"&zdareanow="+zdareanow+"&oldqsdb="+oldqsdb+"&qsdb="+qsdb+"&qyztlast="+qyztlast+"&qyztnow="+qyztnow
														         +"&g2last="+g2last+"&g2now="+g2now+"&g3last="+g3last+"&g3now="+g3now
																 +"&zplast="+zplast+"&zpnow="+zpnow+"&zslast="+zslast+"&zsnow="+zsnow
														         ...
														         +"&gxgwsllast="+gxgwsllast+"&gxgwslnow="+gxgwslnow+"&bzw="+bzw+"&diquzdid="+diquzdid+"&zt="+zt+"&zdbzw="+zdbzw+"&dbid="+dbid+"&dbds="+dbds
														         +"&lg="+lg+"&fzbzw="+fzbzw+"&zggdid="+zggdid
														         +"&dfzflxlast="+dfzflxlast+"&dfzflxnow="+dfzflxnow
														         +"&oldqsdbdl="+oldqsdbdl+"&olddbds="+olddbds+"&oldxgbzw="+oldxgbzw
														         +"&reasonanalyse="+reasonanalyse+"&reasonanalyseyj="+reasonanalyseyj;
														 	...
														 	alert("您未更改任何信息！");
	        					   							...
	        					   							alert("您输入的信息有误，电表读数必须为数字且大于等于0！");
	        					   						}
														//
														}
													  }else{
							        				...
							        				alert("您输入的信息有误，交流负荷必须为数字且大于0！");
							        		 	  }
											  }else{
							        			alert("您输入的信息有误，直流负荷必须为数字且大于0！");
							        		  }
										}
							        }else{
							        	alert("您输入的信息有误，单价必须为数字 且大于0！");
							        }
								 }else{
	...
	var gdscb = '<%=gdscb%>';	//工单生产标
		if(gdscb!=""){
			//申请生产标—工单生产标）/ 工单生产标*100%。该值绝对值是否在10%以内。
		...
		function exportModel(idcz){
		  var path = '<%=path%>';
	      document.form1.action=path+"/servlet/download?action=downloadzdxgsq&idcz="+idcz+"&templetname="+encodeURIComponent("字段修改申请模板");            
	      document.form1.submit();
	}
...
$(function() {
	...
	<body class="body" style="overflow-x: hidden;">
					...
					<td width="100%" scope="2">
						<table width="100%" border="0" cellspacing="1" cellpadding="1">
							<tr>
								...
								<td colspan="6"><input type="text" name="diquzdid" readonly="true" value="<%=diquzdid%>" class="form1" />
								<select name="zt" class="formm1">
							...
							<tr>
											...
											<td>
												<input type="text" name="dianjianow" onblur="getGraygys('dianjianow');" onfocus="getBlack(this);" value="<%=dianjialast%>" class="formm" />
											</td>
											<td bgcolor="#DDDDDD" class="form_label">
												<div>
													额定耗电量：
											...
											<td>
												<input type="text" name="edhdlnow" onblur="getGraygys('edhdlnow');" onfocus="getBlack(this);" value="<%=edhdllast%>" class="formm" />
											</td>
										</tr>
										<tr>
											<td bgcolor="#DDDDDD" class="form_label">
											...
											<td>
												<input type="text" name="jlfhnow" onblur="getGraygys('jlfhnow');" onfocus="getBlack(this);" value="<%=jlfhlast%>" class="formm" onchange="getJl()"/>
											</td>
											<td bgcolor="#DDDDDD" class="form_label">
												<div>
													直流负荷：
											...
											<td>
												<input type="text" name="zlfhnow" onblur="getGraygys('zlfhnow');" onfocus="getBlack(this);" value="<%=zlfhlast%>" class="formm" onchange="getZl()"/>
										</td>
									...
									<tr> 
										...
										<td>
										<input type="text" name="dbds" class="formm"/>
										</td>
									<td bgcolor="#DDDDDD" class="form_label">站点ID:</td>
										<td>
										<input type="text"  readonly="true" 
													value="<%=zdidcz%>" class="form2" />
													</td>
										<td bgcolor="#DDDDDD" class="form_label">电表ID:</td>
										<td>
										<input type="text"  readonly="true" 
													value="<%=dbid%>" class="form2" />
													<input name="olddbds" type="hidden"  readonly="readonly" value="<%=olddbds%>" />
													<input name="oldxgbzw" type="hidden"  readonly="readonly" value="<%=oldxgbzw%>" />
									</td>
									</tr>
							...
							<tr>
								...
								<td bgcolor="#DDDDDD" class="form_label">
											...
											<td bgcolor="#DDDDDD" class="form_labell">
									         ...
									         <td>
									         	<select name="zdlxnow" class="formr" onblur="getGraygys('zdlxnow');" onfocus="getBlack(this);">
										         	...
										         	stationtype = ztcommon.getZdlx(zdsxlast);
										         	if(stationtype!=null){
										         		String code="",name="";
										...
										<tr>
											...
											<td>
												<input type="text" name="zdareanow" onblur="getGraygys('zdareanow');" onfocus="getBlack(this);" class="formm" value="<%=zdarealast%>"/>
											</td>
											<td bgcolor="#DDDDDD" class="form_label">
												<div>
													站点启用状态：
											...
											<td>
											<input type="text" name="oldqsdb" readonly="readonly" value='<%=scb%>' class= "form2"/>
											</td>
											<td bgcolor="#DDDDDD" class="form_labell">
												<div>
													改为:
											...
											<td>
												...
												<input type="hidden" name="oldqsdbdl" readonly="readonly" value="<%=qsdbdl%>" class="form2" />
											</td>	
								       ...
								       <!--    <td bgcolor="#DDDDDD" class="form_label">
											...
											<td>
												<input type="text" name="newqsdbdl" value="<%=qsdbdl%>" class="formm" onblur="getGrayqsdb(this);" onfocus="getBlack(this);"/>
											</td>	
											 -->						
										</tr>
								...
								<tr>
									<td colspan="10" bgcolor="F9F9F9" class="form_label">
										...
										<tr>
											...
											<td>
												<input type="text" name="zpnow" value="<%=zplast%>" class="formm" onblur="getGrayzp(this);" onfocus="getBlack(this);"/>
											</td>
										</tr>
										<tr>
											<td bgcolor="#DDDDDD" class="form_label">
											...
											<td>
												<input type="text" name="zsnow" value="<%=zslast%>" class="formm" onblur="getGrayzs(this);" onfocus="getBlack(this);"/>
											</td>
											<td bgcolor="#DDDDDD" class="form_label">
												<div>
													厂家：
										...
										<tr>
											...
											<td>
												<input type="text" name="bbunow" value="<%=bbulast%>" class="formm" onblur="getGraybbu(this);" onfocus="getBlack(this);"/>
											</td>
											<td bgcolor="#DDDDDD" class="form_label">
												<div>
													RRU数量：
											...
											<td>
												<input type="text" name="rrunow" value="<%=rrulast%>" class="formm" onblur="getGrayrru(this);" onfocus="getBlack(this);"/>
											</td>
											<td bgcolor="#DDDDDD" class="form_label">
												<div>
													设备编码：
									...
									<td bgcolor="#DDDDDD" class="form_label">
											...
											<td>
												<input type="text" name="ydshebeinow" value="<%=ydshebeilast%>" class="formm" onblur="getGrayyd(this);" onfocus="getBlack(this);"/>
											</td>
											<td bgcolor="#DDDDDD" class="form_label">
												<div>
													共享固网数量：
											...
											<td>
												<input type="text" name="gxgwslnow" value="<%=gxgwsllast%>" class="formm" onblur="getGraygw(this);" onfocus="getBlack(this);" />
											</td>
										</tr>
									</table>
									<table>	
							...
							<tr>
								...
								<td colspan='7'>
									<input type="text" name="reasonanalyseyj" value="" class="formjc" />
								</td>
							</tr>
							<tr height="19px">
								...
								<td><div id="uploading"
									style="position: relative; width: 80px; height: 19px; float: left;">
									<img alt=""
										src="<%=request.getContextPath()%>/images/shangchuan.png"
										width="100%" height="100%" />
									<span
										style="font-size: 12px; position: absolute; left: 28px; top: 3px; color: white">提交</span>
								</div>
								<input type="hidden" name="tidc" value="0" />
								</td>
								<td><div id="daochu"
									style="position: relative; width: 80px; height: 19px; float: left;left: 10px;">
									<img alt=""
										src="<%=request.getContextPath()%>/images/shangchuan.png"
										width="100%" height="100%" />
									<span
		...
		<font size="2px" color="red"> 以下占比都为绝对值占比：能耗基础值占比：(能耗基础值-省定标(不含空调))/省定标(不含空调)</font><br>
		<font size="2px" color="red">实际值占比：(实际值-省定标(不含空调))/省定标(不含空调)</font><br>
			...
			<font size="2px" color="red">交流电量占比：(交流负荷*5.28(交流电量)-省定标(不含空调))/省定标(不含空调)</font>
			<font size="2px" color="red">直流电量占比：(直流负荷*1.52(直流电量)-省定标(不含空调))/省定标(不含空调)</font><br>
			<input type="hidden" value="<%=dbid%>" name="dbid"/>
			<input type="hidden" value="<%=bzw%>" name="zdhbbzw"/>
			...
			<input type="hidden" id="zggdid" name="zggdid" value="<%=zggdid%>"/>
			
		</form>
	<%} }else{%>
		<div style="width: 300px; height: 19px; position: relative; font-size: 12px; color: red">此站点有修改但是未审核的站点信息！
		</div>
	<%}} %>
...
function zdsx(){
	...
	if(sid=="0"){
		...
		sendRequest(path+"/servlet/garea?action=zdsx&pid="+sid,"zdsxnow");
	        ...
	        document.form1.sfbgnow.value='<%=sfbglast%>';
			document.form1.zdsxlast.value='<%=zdsxlast%>';
			document.form1.zdsxnow.value='<%=zdsxlast%>';
			document.form1.zdlxlast.value='<%=zdlxlast%>';
			document.form1.zdlxnow.value='<%=zdlxlast%>';
			document.form1.yflxlast.value='<%=yflxlast%>';
			document.form1.yflxnow.value='<%=yflxlast%>';
			document.form1.gxxxlast.value='<%=gxxxlast%>';
			document.form1.gxxxnow.value='<%=gxxxlast%>';
			document.form1.gdfslast.value='<%=gdfslast%>';
			document.form1.gdfsnow.value='<%=gdfslast%>';
			document.form1.zgdlast.value='<%=zgdlast%>';
			document.form1.zgdnow.value='<%=zgdlast%>';
			document.form1.qyztlast.value='<%=qyztlast%>';
			document.form1.qyztnow.value='<%=qyztlast%>';
			document.form1.g2last.value='<%=g2last%>';
			document.form1.g2now.value='<%=g2last%>';
			document.form1.g3last.value='<%=g3last%>';
			document.form1.g3now.value='<%=g3last%>';
			document.form1.changjialast.value='<%=changjialast%>';
			document.form1.changjianow.value='<%=changjialast%>';
			document.form1.jztypelast.value='<%=jztypelast%>';
			document.form1.jztypenow.value='<%=jztypelast%>';
			document.form1.shebeilast.value='<%=shebeilast%>';
			document.form1.shebeinow.value='<%=shebeilast%>';
			document.form1.dfzflxlast.value='<%=dfzflxlast%>';
			document.form1.dfzflxnow.value='<%=dfzflxlast%>';
</script>
<script type="text/javascript">
